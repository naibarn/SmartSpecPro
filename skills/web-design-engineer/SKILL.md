...
description: |
  Build standalone high-quality visual Web artifacts using HTML/CSS/JavaScript/React — web pages, landing pages, dashboards, interactive prototypes, HTML slide decks, animated demos, UI mockups, data visualizations, and more.
  ...
  Prefer visual-ui-enhancement for existing React/Next/Vite app refactors, shadcn/Tailwind UI polish, accessibility/responsive QA, or product UI changes. Use this skill mainly for standalone visual artifacts, prototypes, decks, animations, and data visualizations.
...
This skill is subordinate to the global Codex frontend guidance and the
`visual-ui-enhancement` skill. If those instructions conflict with this file,
follow the global Codex/frontend rules.

Do not pause for formal design-system approval when the user has already asked
Codex to implement. Instead, briefly state the design direction in the working
update, build a usable first version, and verify it.
...
### Step 3: Declare the Design Direction Before Writing Code

Before writing the first line of code, articulate the design direction in a short
Markdown note. In Codex, do this as a concise working update; do not block for
confirmation unless the user asked for options first or the direction is genuinely ambiguous:

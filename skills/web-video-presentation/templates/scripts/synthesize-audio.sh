...
#
# Usage:
#   bash scripts/synthesize-audio.sh --yes                # incremental
#   bash scripts/synthesize-audio.sh --yes --force        # overwrite all
#   bash scripts/synthesize-audio.sh --yes --voice=<id>   # override voice
# ─────────────────────────────────────────────────────────────────────
set -euo pipefail
...
if [[ "$CONFIRMED" != true ]]; then
  cat >&2 <<EOF
This script sends narration text to the configured mmx TTS provider.
Re-run with --yes after confirming the external audio-generation side effect.
EOF
  exit 2
fi
...
✗ mmx CLI not found in PATH.
  ...
  Login:    mmx auth login --api-key <MINIMAX_API_KEY>

...
description: "Create click-driven 16:9 web video presentations from scripts, articles, lessons, product demos, or talks. Produces a Vite + React + TypeScript presentation with chapter/step narration timing and optional audio synthesis. Use when the user asks to turn content into a video-like web presentation, dynamic deck, screen-recordable explainer, product talk, or lesson. Requires explicit confirmation before scaffolding, npm installs, TTS, or network-backed audio generation."
---
...
- Codex can use this skill, but network/package side effects are gated.
- Ask for explicit confirmation before running `npm create`, `npm install`, `npm install -g`, `mmx`, or any TTS/API call.
- Do not require parallel agents. If independent review or parallel chapter work would help, use it only when the user explicitly asks for parallel agents; otherwise self-review with the same checklist.
- Paths in this skill are relative to `skills/web-video-presentation/` in this portable pack, not editor-specific skill folders.
- After scaffolding, run typecheck and start a dev server only when appropriate for the user's workflow.
...
1. **Independent reviewer, when the host supports it and the user asked for parallel review**：开一个独立的 reviewer agent，给它"产出文件
   路径 + 对应清单 + 关键上下文"，让它逐项核查并**严格汇报结论**
   （哪几条 pass / 哪几条 fail + 证据 + 改写建议）。
2. **Optional sub-agent, only when the user explicitly asked for parallel agents**：没有 Teams 能力但能开 parallel agent 就用 parallel agent
   走同样流程。
...
对 `script.md` / `outline.md` 执行（优先 independent reviewer → optional parallel agent → 自检），
...
接下来一次对齐 5 件事：
  ...
  5. 开发模式选哪个？
     ...
     **第 1 章无论哪种模式都必须主线程做完 + 用户验收**（强制 anchor）。
     ...
     C) 第 1 章后并行开发（parallel agent）
        第 2~N 章用 parallel agent 并行 → 最快 / 用户控并行数（一次几章）
...
脚手架带一个 `01-example` demo。在写第一章真实内容前，先确认目标路径仍在
`presentation/src/chapters/01-example`，再删除该目录：

```bash
rm -r -- presentation/src/chapters/01-example
```
...
#### 模式 C · 第 1 章后并行开发（parallel agent）

用 parallel agent 把第 2~N 章并行做完，最大并行数由用户控制（"一次 4 章"
/ "一次 2 章"）。**最快，但风格各章会有差异** —— 这是预期，因为：

1. 每个 parallel agent 看不到别的 parallel agent 产出，无法机械对齐
...
并行 parallel agent 的 prompt 必须包含：
...
- **完工自检逐项过**，不达标回去改 —— 按上文「硬性自检协议」执行
  （优先 independent reviewer → optional parallel agent → 自检），**改完再向用户汇报本章交付**
...
npm run synthesize-audio -- --yes     # 调 mmx 串行合成；增量、跳过已存在
```

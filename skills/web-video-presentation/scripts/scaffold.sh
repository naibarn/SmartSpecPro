...
#
# 用法：
#   bash scripts/scaffold.sh <target-dir> [--theme=<id>] --yes
#   bash scripts/scaffold.sh --list-themes
#
# 例子：
#   bash skills/web-video-presentation/scripts/scaffold.sh ./presentation --yes
#   bash skills/web-video-presentation/scripts/scaffold.sh ./talk --theme=paper-press --yes
...
#
# 之后切换主题，覆盖一个文件即可：
#   cp skills/web-video-presentation/themes/<id>/tokens.css \
...
if [[ "$CONFIRMED" != true ]]; then
  cat >&2 <<EOF
This scaffold creates a Vite project and runs npm install.
Re-run with --yes after confirming the side effects are intended.
EOF
  exit 2
fi
...
音频合成（可选，录制前做）：
  ...
  npm run synthesize-audio -- --yes      # 调 mmx-cli 合成 → public/audio/<id>/<step>.mp3

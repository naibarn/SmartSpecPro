...
```bash
npm run synthesize-audio -- --yes              # 增量：跳过已存在的 mp3
npm run synthesize-audio -- --yes --force   # 全部重合成
npm run synthesize-audio -- --yes --voice=<voice-id>  # 指定音色
```
...
本机没检测到 mmx-cli。我可以：
  ...
  1. 帮你安装 MiniMax CLI（推荐）
     ...
     运行：npm install -g mmx-cli && mmx auth login --api-key <MINIMAX_API_KEY>
     API key 在 https://platform.minimaxi.com 获取
...
| `401 / unauthorized` | `mmx auth login --api-key <MINIMAX_API_KEY>` 重新登录 |

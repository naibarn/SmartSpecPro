...
```bash
# 默认（midnight-press）
bash scripts/scaffold.sh ./presentation --yes

# 显式指定
bash scripts/scaffold.sh ./talk --theme=newsroom --yes
```
...
```bash
cp skills/web-video-presentation/themes/newsroom/tokens.css \
...
```bash
cd skills/web-video-presentation/themes

...
SmartSpecPro's active package intentionally does **not** include the optional
external `openai-agents-python` orchestrator. The repo skill must work through
Codex/Claude skills and SmartSpecPro sub-agents without `.venv` or external LLM
API wiring.
...
`orchestra` skill and SmartSpecPro sub-agent definitions.

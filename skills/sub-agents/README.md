...
| Agent File | Role | CMD | Registered agent name | Output Format | When to Use |
|---|---|---|---|---|---|
...
| `frontend.md` | React/UI implementer | CMD-1 | `frontend` | Result Report + changed `.tsx`/`.ts` files | Adding/modifying React components, pages, hooks |
| `backend.md` | tRPC/Drizzle implementer | CMD-2 | `backend` | Result Report + changed server `.ts` files | Adding/modifying tRPC routers, Express routes, DB queries |
| `python.md` | FastAPI/Celery implementer | CMD-3 | `python` | Result Report + changed `.py` files | Adding/modifying FastAPI endpoints, Celery tasks |
| `database.md` | Schema/migration specialist | CMD-4 | `database` | Result Report with backup audit trail | Schema changes, migrations, data seeding |
| `test-qa.md` | Test writer and QA reporter | CMD-8 | `test-qa` | Result Report + test plan + pass/fail report | Writing tests, checking coverage |
| `e2e-playwright.md` | Browser workflow tester | CMD-8E | `e2e-playwright` | E2E plan + Playwright results | Browser workflow, responsive viewport, auth flow, screenshot/trace, or flaky E2E work |
...
| `security.md` | General security auditor/fixer | CMD-6 | `security` | Result Report + risk register | Security audit + remediation for HIGH/CRITICAL risk tasks |
| `debugger.md` | Bug investigator and fixer | CMD-7 | `debugger` | Result Report with root cause + attempt log | Multi-file bugs with unclear root cause |
| `error-detective.md` | Audit log investigator (read-only) | CMD-7 | `error-detective` | Result Report with event timeline | LLM/media failures, cost discrepancies, trace investigation |
| `infrastructure.md` | Infra/ops specialist | CMD-5 | `infrastructure` | Result Report + validate-all-configs result | Nginx, Docker, systemd, deployment changes |
| `performance.md` | Performance specialist | CMD-9 | `performance` | Baseline + bottleneck + verification report | Slow endpoints, N+1 queries, cache strategy, bundle/load-test regressions |
| `ci-release.md` | CI/release specialist | CMD-10 | `ci-release` | Workflow patch + release readiness report | GitHub Actions, failing checks, deploy gates, release readiness, rollback |
| `dependency-supply-chain.md` | Dependency/supply-chain specialist | CMD-11 | `dependency-supply-chain` | Dependency findings + lockfile/scanner report | Dependency audit, lockfile drift, vulnerable packages, licenses, package integrity |
| `docs-release.md` | Docs and changelog writer | release | `docs-release` | Result Report + changelog + migration guide | End of feature cycle — release documentation |
...
| `security-trpc.md` | tRPC security auditor (read-only) | CMD-6 | `security` | Security findings table | Pre-merge gate — changed tRPC routers |
| `security-fastapi.md` | FastAPI security auditor (read-only) | CMD-6 | `security` | Security findings table | Pre-merge gate — changed FastAPI endpoints/Celery tasks |
| `security-frontend.md` | Frontend security auditor (read-only) | CMD-6 | `security` | Security findings table | Pre-merge gate — changed React components/pages |
...
| `ui-builder.md` | Tailwind/shadcn UI builder | CMD-12 | `ui-builder` | Result Report + changed UI files | Implement visual UI polish, component states, responsive styling, and dark-mode-friendly UI |
...
| `visual-final-refactor.md` | Final visual refactor implementer | CMD-12 | `visual-final-refactor` | Result Report + final UI patch | Consolidate visual/UX/a11y/responsive findings into a safe final UI patch |
...
| Agent | claude-code (registered name) | standard (template injected) | open-code (sequential) |
|---|---|---|---|
...
| `backend` | `backend` | `general-purpose` + `backend.md` template | Conductor adopts role |
| `python` | `python` | `general-purpose` + `python.md` template | Conductor adopts role |
...
| `security` | `security` | `general-purpose` + `security.md` template | Conductor adopts role |
| `debugger` | `debugger` | `general-purpose` + `debugger.md` template | Conductor adopts role (sequential only) |
| `error-detective` | `error-detective` | `general-purpose` + `error-detective.md` template | Conductor adopts role |
...
| `security-trpc` | `security` | `general-purpose` + `security-trpc.md` template | Conductor adopts role |
| `security-fastapi` | `security` | `general-purpose` + `security-fastapi.md` template | Conductor adopts role |
| `security-frontend` | `security` | `general-purpose` + `security-frontend.md` template | Conductor adopts role |
...
5. Update the **Platform Compatibility Matrix** above with the new agent's registered name and fallback behavior for each platform

---

## Native .claude/agents/ Definitions

The 29 agents in this registry can each have a generated native definition in `.claude/agents/` that enables Claude Code's agent mechanism. These files use YAML frontmatter to configure model and tool access. The source of truth remains `skills/sub-agents/agents/*.md`; regenerate native files after editing the portable source.
...
- **Registered agent names are load-bearing:** the value in the registry table is used by orchestration tooling where native agents exist. Typos cause silent failures where orchestration dispatches to the wrong role.

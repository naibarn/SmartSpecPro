---
name: kb-retriever
description: "Retrieve and answer questions from a local knowledge-base directory. Use when the user asks to search docs, query a local knowledge folder, answer from knowledge/, inspect PDFs/Excel in a local corpus, or cite local sources. Uses hierarchical data_structure.md navigation, progressive rg/read windows, and file-type-specific PDF/Excel handling without whole-file loading."
---
...
- Work only inside the user-authorized knowledge directory.
- Default root is `knowledge/` under the current project; if it does not exist, ask for the path.
- Reject path traversal and avoid following symlinks outside the selected root.
- Do not use web search for knowledge-base answers unless the user explicitly asks to combine local and web sources.
- Prefer `rg` for text search, then read only narrow line windows around matches.
- For PDF/Excel, read the relevant `references/` file first, extract to a temporary/project-local artifact, then search the extracted text/table summary.

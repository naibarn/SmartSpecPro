...
- **Reproducible prompt + image archival** under `gpt-image-2-output/prompt/` and `gpt-image-2-output/image/` with task-slug + timestamp naming.
...
| **C — Advisor** | Garden disabled, host has no image tool | Skill degrades into a high-quality prompt writer — saves the rendered prompt to `gpt-image-2-output/prompt/` and instructs the user to paste it into ChatGPT / Midjourney / DALL·E / Sora / Nano Banana / their own gateway |
...
Read in this order: CLI args → `process.env` → `<cwd>/.env`.
...
| `OPENAI_BASE_URL` | optional | Default `https://api.openai.com/v1`; can point to a custom OpenAI-compatible gateway only when explicitly confirmed and SKILLPACK_ALLOW_CUSTOM_OPENAI_BASE_URL=1 |
...
| What | Where | Used in |
|---|---|---|
| Rendered prompts | `gpt-image-2-output/prompt/<task-slug>-<timestamp>.md` | A / B / C |
| Generated images | `gpt-image-2-output/image/<task-slug>-<timestamp>.png` | A only (B = host decides, C = none) |

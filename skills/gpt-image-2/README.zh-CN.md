...
- **可复用的 prompt + 图片归档**：默认落盘到 `gpt-image-2-output/prompt/` 和 `gpt-image-2-output/image/`，按 `<task-slug>-<timestamp>` 命名。
...
| **C · Advisor 纯提示词顾问** | 未启用 Garden，宿主也没有图像工具 | 退化成"高质量 prompt 撰写顾问"——把 prompt 落盘到 `gpt-image-2-output/prompt/`，告诉用户去 ChatGPT / Midjourney / DALL·E / Sora / Nano Banana / 自己的网关里执行 |
...
按以下顺序读取：CLI 参数 → `process.env` → `<cwd>/.env`。

| 变量 | 必需性 | 说明 |
|---|---|---|
| `ENABLE_GARDEN_IMAGEGEN` | Mode A 必需 | 模式开关：`1` / `true` / `yes` / `on` 启用 Mode A |
| `OPENAI_API_KEY` | Mode A 必需 | 真正调图像 API 用 |
| `OPENAI_BASE_URL` | 可选 | 默认 `https://api.openai.com/v1`，仅在明确确认且设置 SKILLPACK_ALLOW_CUSTOM_OPENAI_BASE_URL=1 时可指向自定义 OpenAI 兼容网关 |
...
| 内容 | 落盘位置 | 适用模式 |
|---|---|---|
| 渲染好的 prompt | `gpt-image-2-output/prompt/<task-slug>-<timestamp>.md` | A / B / C |
| 生成的图片 | `gpt-image-2-output/image/<task-slug>-<timestamp>.png` | 仅 A（B 由宿主决定，C 不产出） |

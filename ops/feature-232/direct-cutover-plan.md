...
**Status (2026-09-26):** SearchResultCache is deployed and enabled on the Cloudflare target, but G1 Production verification is incomplete. The deployed Worker `smartspec-cloudflare-runtime` version `f6f0ba85-3d23-476e-b8af-55840609442d` serves `https://runtime.smartaihub.app` with KV namespace binding `SEARCH_RESULT_CACHE` (ID `df6d9bead8644d0f8f334f59b6bdfbeb`). Existing live health/authenticated probes, synthetic tenant isolation, unauthenticated rejection, PostgreSQL provider selection and app-adapter roundtrip passed. A read-only Redis snapshot found zero legacy SearchResultCache keys. Real Beta searches for both accounts, Production traces, authenticated Admin UI inspection and caller-level Redis attribution are still missing; do not mark G1 `PRODUCTION_CUTOVER_VERIFIED`. Local code now propagates sanitized Web trace IDs into Worker KV logs and has an Admin-only, request-scoped KV-get fault-injection gate, but neither change is deployed or Production-tested. G2-A–D PostgreSQL implementations/tests exist locally; the latest JTI dry-run found 275 active revocations and imported zero. Production migrations/import/caller cutover have not occurred. `CLOUDFLARE_ACTIVATION=disabled`; G3–G6 remain open.
...
**Tests:** JTI unit contract and PostgreSQL integration passed locally. Latest read-only Production dry-run found 275 active revocations, 0 persistent keys and imported 0. The Production DB does not yet have `revoked_token_jtis`; Production still uses Redis. No Production caller is closed.
...
**G2 readiness:** G2-A–D implementations/tests exist locally, but the group is **not cut over**. Five Production migrations 0345–0349, both state imports, a verified restorable backup, the separate pairing keyring and a coordinated auth maintenance window remain mandatory. The inspected shell environment does not have the pairing keyring configured; Production secret-manager state was not accessible. Use a 32-byte key generated directly into the secret manager (never chat or logs), set both keyring variables on every instance, and run key-rotation integration tests before deployment. Acceptance requires all auth/pairing flows paused across every Web instance, fresh Redis reconciliation, deploy/restart all instances, multi-instance probes and proof revoked tokens remain denied before resuming traffic. Keep Redis for Voice/G3–G6 and keep `CLOUDFLARE_ACTIVATION=disabled`.

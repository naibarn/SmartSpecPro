 ...
 * candidates `remotionRuntimeAdapter.ts`'s `resolveThaiFontPath()` already
 * probes via `fc-match` (`"Prompt,Noto Sans Thai,Kanit,Sarabun,Loma:lang=th"`)
 * and the ASS burn-in post-pass already hardcodes one of
 * (`postPassArgs.ts:244`, `"Noto Sans Thai"`) — i.e. these are the families
 * this codebase already treats as its Thai-capable set. `"Loma"` is
 * deliberately EXCLUDED from this allowlist: unlike the other four, it has
 * no publicly hosted, checksummable Google Fonts file this server can
 * fetch+verify server-side (see `videoProjectAssetResolver.ts`'s
 ...
 * `packages/remotion-render/src/fontAllowlist.ts` — see
 * `__tests__/fontAllowlistSync.test.ts`, mirroring the existing
 * `layerTemplateSchemasSync.test.ts` guard for the same cross-package drift
 * risk (RK1).
 */

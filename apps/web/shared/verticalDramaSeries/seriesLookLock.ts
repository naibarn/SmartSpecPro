...
const CATALOG: Record<VdLookLockGenre, VerticalDramaPresetVisualIdentity> = {
  drama_romance: {
    ...
    wardrobeGrammar: [
      "grounded contemporary tailoring",
  ...
  sci_fi_cyberpunk: {
    ...
    imagePromptFragments: {
      positive: [
        "grounded near-future production design",
        "functional luminous interfaces",
  ...
  action_epic: {
    ...
    imagePromptFragments: {
      positive: [
        "monumental grounded scale",
        "directional natural light",
        "credible impact wear",
      ],
  ...
  fantasy_fairytale: {
    ...
    environmentMotifs: [
      "handcrafted organic architecture",
      "weathered magical artifacts",
    ],
    wardrobeGrammar: [
      "layered natural textiles",
      "symbolic handcrafted details",
    ],
    ...
    imagePromptFragments: {
      positive: [
        "tactile storybook fantasy",
        "handcrafted organic detail",
        "soft enchanted naturalism",
      ],
      negative: ["plastic costume finish", "generic game interface styling"],
    },
  },
  animation_cartoon: {
    styleName: "Expressive animated story world",
    palette: ["storybook blue", "sunlit yellow", "bold coral"],
    lighting: "clear readable stylized light with expressive color accents",
    environmentMotifs: [
      "imaginative silhouette architecture",
      "playful prop scale",
    ],
    wardrobeGrammar: [
      "distinct graphic shapes",
      "character-readable color blocking",
    ],
    signaturePropsAndCompanions: [
      "expressive companion objects",
      "impossible whimsical details",
    ],
    cameraGrammar: "dynamic readable composition with expressive staging",
    characterArchetypes: [],
    imagePromptFragments: {
      positive: [
        "expressive animated visual language",
        "clear silhouette design",
        "stylized cinematic staging",
      ],
      negative: ["flat generic realism", "unreadable visual clutter"],
    },
  },
};
...
export function readSeriesLookLockControl(
  value: unknown
): VdLookLockControl | undefined {
  ...
  const genreKey = VD_LOOK_LOCK_GENRES.includes(
    value.genreKey as VdLookLockGenre
  )
    ? (value.genreKey as VdLookLockGenre)
    : undefined;
  const inheritedIdentity = parseIdentity(value.inheritedIdentity);
  const inheritedSource = ["preset", "ai_mix", "lineage"].includes(
    String(value.inheritedSource)
  )
  ...
  const inheritedGovernance = ["preset_mix", "look_lock"].includes(
    ...
    ? (value.inheritedGovernance as VdLookLockGovernance)
    : undefined;
...
export function resolveEffectiveSeriesVisualIdentity(params: {
  ...
  const inheritedAuthorized =
    control.inheritedGovernance === "preset_mix"
  ...
  return inheritedAuthorized
    ? (control.inheritedIdentity ?? current)
    : undefined;
}
...
function validateStringArray(
  ...
  return parsed.every((entry): entry is string => entry !== undefined)
    ? parsed
    : undefined;
}
...
export function validateSeriesLookManualPatch(
  ...
  if (!isRecord(raw))
    return { ok: false, error: "Manual look patch must be an object" };
  ...
  if (Object.keys(raw).some(key => !allowed.has(key))) {
    return {
      ok: false,
      error: "Manual look patch contains a non-editable field",
    };
  }
...
export function applySeriesLookLockTransition(params: {
  ...
  const inheritedIdentity =
    previousControl?.inheritedIdentity ?? currentIdentity;
  const inheritedSource =
    previousControl?.inheritedSource ??
    (inheritedIdentity ? (params.inheritedSource ?? "preset") : undefined);
  const inheritedGovernance =
    previousControl?.inheritedGovernance ??
    (inheritedIdentity
      ? (params.inheritedGovernance ?? "preset_mix")
      : undefined);
  ...
  if (params.mode === "genre") {
    ...
    if (!validated.ok) {
      throw new SeriesLookLockTransitionError(
        "invalid_manual_patch",
        currentRevision
      );
    }
    ...
    const candidate = {
      ...
      ...validated.value,
      imagePromptFragments:
        validated.value.imagePromptFragments ?? base.imagePromptFragments,
    };
  ...
  const control: VdLookLockControl = {
    ...
    ...(params.mode === "genre" && params.genreKey
      ? { genreKey: params.genreKey }
      : {}),
    ...(inheritedIdentity ? { inheritedIdentity } : {}),
    ...(inheritedSource ? { inheritedSource } : {}),
    ...(inheritedGovernance ? { inheritedGovernance } : {}),
...
export function applySeriesLookToImagePrompt(params: {
  ...
  if (!params.identity)
    return { prompt: params.prompt, negativePrompt: params.negativePrompt };
  return {
    prompt:
      appendUniqueCsv(
        params.prompt,
        params.identity.imagePromptFragments.positive
      ) ?? "",

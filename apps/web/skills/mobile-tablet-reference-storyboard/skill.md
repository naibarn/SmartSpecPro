---
name: mobile-tablet-reference-storyboard
description: Mobile And Tablet Reference Storyboard prompt skill adapted from the furniture reference storyboard pattern. Optimized for smartphones, tablets, foldables, e-readers when sold as device, phone cases, tablet cases, screen protectors, styluses, chargers bundled with mobile devices, and mobile accessory kits, strict reference-image fidelity, exact product shape and proportion preservation, material and texture accuracy, marking/label preservation, product-source dominance, clean no-text-by-default image generation, infographic text controls, equal-frame storyboard layout, anti-generic-substitution guards, and category-specific QA gates. Use when generating image or storyboard prompts for มือถือและแท็บเล็ต (smartphones, tablets, foldables, e-readers when sold as device, phone cases, tablet cases, screen protectors, styluses, chargers bundled with mobile devices, and mobile accessory kits).
...
auto_trigger: false
triggerPatterns:
  - Mobile And Tablet Reference Storyboard
...
This skill writes high-fidelity image and storyboard prompts for มือถือและแท็บเล็ต (smartphones, tablets, foldables, e-readers when sold as device, phone cases, tablet cases, screen protectors, styluses, chargers bundled with mobile devices, and mobile accessory kits). It is adapted from the furniture reference storyboard skill's product-source-dominance, equal-frame storyboard, no-text-by-default, and QA-loop conventions, but all product fidelity rules are rewritten for this category.

The current product reference images are the highest-priority source of truth. Creative styling may change the environment, camera, lighting, props, and mood, but it MUST NOT redesign, resize, recolor, relabel, simplify, beautify, or replace the referenced mobile phone or tablet product.
...
For มือถือและแท็บเล็ต, the prompt must additionally preserve:
- phones: camera island, lens count, flash, side buttons, speaker grilles, charging port, SIM tray
- tablets: bezel width, screen ratio, camera placement, stylus magnet area, keyboard case connector
- foldables: hinge line, inner/outer screen, crease, folding angle, camera placement
- cases and protectors: cutouts, raised edges, lens ring, stand flap, card slot, transparent back
- screen states: blank glass, lock screen, UI mockup only if requested; avoid fake app text by default

## Category-Specific Physical Fidelity Rules

### Structure And Proportion

Preserve device aspect ratio, corner radius, thickness, camera module shape, lens count, button/port placement, bezel width, case cutouts, hinge geometry, and stylus/case alignment.

### Material And Texture

Preserve glass reflectivity, matte or glossy back, metal frame, silicone case, leather case, clear TPU, screen protector edge, camera glass, and fingerprint/smudge realism when useful.

### Marking, Label, And Physical Text Preservation

Preserve physical brand logos, model marks, case prints, packaging text, and screen text only if physically present or requested; never invent app UI claims.

### Scale And Context

Show realistic hand, pocket, desk, bag, charging, tablet stand, stylus, or case scale; do not turn phones into tablets or tablets into laptops.

### Product Interaction And Use

Use scenes such as hand-held use, camera module close-up, charging, case install, screen protector alignment, stylus use, fold/unfold, desk/tablet productivity, or pocket portability.

### Common Wrong Substitutions To Reject

The prompt must explicitly reject:
- changing camera lens count
- inventing ports/buttons
- changing aspect ratio
- adding fake UI text
- turning phone into tablet/laptop
- removing case cutouts
...
Frame 1: front/back hero.
Frame 2: hand scale.
Frame 3: camera module detail.
Frame 4: side buttons/ports.
Frame 5: case or accessory fit.
Frame 6: charging/stylus use.
Frame 7: screen/glass detail.
Frame 8: pocket/desk context.
Frame 9: final mobile lifestyle scene.
...
The final prompt must make the image model preserve the real referenced mobile phone or tablet product: exact shape, proportions, material, texture, markings, and scale from the image, not a generic category archetype. It must also render the storyboard as cinematic, dimensional, photorealistic product-film imagery with clear, consistent human faces when character references are used.
...
The final prompt must make the image model preserve the real referenced computer or laptop product: exact shape, proportions, material, texture, markings, and scale from the image, not a generic category archetype. It must also render the storyboard as cinematic, dimensional, photorealistic product-film imagery with clear, consistent human faces when character references are used.
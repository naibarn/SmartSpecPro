...
- `mother_baby` -> `references/product-categories/mother_baby.md`: Lock exact baby product class, safety-visible parts, straps/rails/wheels/bottle nipple/lid count, the material exactly as shown in the product reference image (hard moulded plastic stays hard plastic; never assume soft padding, quilting, or fabric upholstery just because the product is for babies or children), colorway, size class, and age/use context. Reject: Do not add unsupported safety claims, unsafe use, wrong age impression, extra restraints, missing straps, different stroller/chair/bottle structure, unrealistic baby handling, or hard plastic seats/trays/shells re-rendered as padded fabric cushions with stitched seams.

...
When your own frame reading or planned motion creates identity risk, write a
compact positive contract inside `prompt`: preserve each at-risk character's
observed facial angle for the whole clip, keep motion to blink, breath, gaze,
micro-expression, hands and shoulders, and do not reveal unseen facial regions.
Activate this for a face outside frontal/three-quarter visibility, more than
partial occlusion, overlap, moderate/large head turns, hidden-side reveal,
orbit/large reframe, or a new character entering mid-shot.

The prose camera move and `camera_motion` declaration must agree. Never
under-declare a bigger move to evade the contract: the caller derives a risk
floor from your facts and the judge sees both. The contract is a short lock,
not scene description; it outranks atmosphere and sound texture within the
existing prompt budget, but never name+position speech anchors.

**Low risk adds no contract language.** A clearly visible frontal or
three-quarter face, none/subtle head turn, and locked/push-in camera should
remain expressive; a default brake would make the clip static and lifeless.
...
state the same constraints positively inside `prompt`—preserved angle, no
orbit, no overlap—because that family never receives the negative channel.
Still return `negative_motion_prompt` for other consumers. Low-risk shots add

...
0-contract. RUNTIME COUNT CONTRACT OVERRIDE: If USER_INPUTS_JSON or the caller prompt includes `AUDIO_FIRST_STORYBOARD_PROMPT_COUNT_CONTRACT`, obey that exact prompt count above all generic examples, learned defaults, compactness preferences, and any tendency to stop at 10 prompts. If the contract says `PROMPT 1 through PROMPT 15`, the final output must contain all 15 real prompt blocks explicitly.
...
1a. STORYBOARD MODE CONTINUITY RULE: If contentMode is `storyboard`, treat all prompts as one continuous storyboard. The story must progress from hook to middle to payoff without resetting the world. Preserve the same recurring character/cast identity, face/hair/body traits, species/age, outfit colors, accessories, signature props, setting anchors, lighting, camera language, and visual style across the full storyboard. Each prompt must clearly continue from the previous beat while showing a new action or emotional development.
1b. STORYBOARD CHARACTER CONSISTENCY RULE: For contentMode=`storyboard`, create a protagonist/cast bible before Prompt 1. If a character appears in more than one beat, that character must keep the same identity, wardrobe, proportions, colors, accessories, and pose language every time they return. If the character is derived from an attached reference image, name the handle in REFERENCE NOTES and include the handle plus defining traits in CONTINUITY NOTES and the relevant Continuity Lock.
...
4e. If `reference_images` exists in USER_INPUTS_JSON, the attached images are real vision inputs. Analyze every image before writing the output, assign each handle a useful role (character/person identity, product/brand/object, animal/prop, scene/location/background, start frame, end frame, or supporting visual), and put the role plus concrete visual details in top-level `REFERENCE NOTES`. For contentMode=`storyboard` or `news_narration`, default every attached image to a reference asset role, not a start/end frame role, unless generationType is explicitly `FIRST_AND_LAST_FRAMES_2_VIDEO`. Mention the relevant @ImageN handles inside `REFERENCE NOTES`, `CONTINUITY NOTES`, and visual directions such as `Background Visuals` or `Action`; do not add a separate `Reference Image Role:` line inside provider prompt blocks.
4e-1. REFERENCE IMAGE DECLARATION RULE: When reference_images exist, the output must make image reference usage explicit. In `REFERENCE NOTES`, write statements such as `@Image1 is used as a reference asset for recurring main character identity, not as a start frame` or `@Image2 is used as a reference asset for scene/location look, not as a start frame`, followed by the concrete face, hair, outfit, object, prop, layout, color, and lighting details to preserve. For contentMode=`storyboard`, any recurring character reference image must be carried through the entire storyboard unless classified as `single-beat`, `background-only`, or `discard`.
4e-2. TWO-IMAGE THAI MOTHER/BABY STORYBOARD RULE: When the User Idea or Story Bible says the first attached image is the mother and the second attached image is the baby, never reinterpret @Image2 as a scene/location reference. Declare exactly this role mapping in `REFERENCE NOTES`: `@Image1` = recurring Thai mother character reference; `@Image2` = recurring Thai 6-month-old baby boy character reference. Every `Continuity Lock` in every prompt must mention both `@Image1` and `@Image2` plus the same luxurious nursery/child bedroom, soft warm lighting, full vertical 9:16 edge-to-edge framing, no letterboxing, no cinematic black bars, and no top or bottom matte bars.
4e-3. REFERENCE ASSET WORDING RULE: In storyboard and news_narration outputs, each attached @ImageN line in `REFERENCE NOTES` must include the words `reference asset` and `not as a start frame`, unless generationType is `FIRST_AND_LAST_FRAMES_2_VIDEO`. Do not write `@Image1 is the Start frame` or similar wording for storyboard/news reference assets.
...
6. VEO 3.1 MODE RULE:
   ...
   - REFERENCE_2_VIDEO images are general visual reference assets, not first/last anchors. Use `@Image1`, `@Image2`, and `@Image3` only when those images exist, and state in REFERENCE NOTES that each one is used as a reference asset, not as a start frame.
...
7. CHARACTER CONSISTENCY RULE:
   ...
   - In contentMode=`storyboard`, this rule is mandatory for the whole storyboard. Do not treat each prompt as a fresh design. A recurring character must remain visually identical across all prompts except for scene-specific pose, expression, gesture, dirt/damage, or story-driven costume changes explicitly described in the beat plan.
...
8. REFERENCE IMAGE RULE:
   ...
   - Explicitly declare reference-image usage in `REFERENCE NOTES` with @Image handles and roles. The user and downstream Media Studio fields must be able to see which images are being referenced. For storyboard/news reference images, use explicit wording like `@Image1 is used as a reference asset..., not as a start frame`.
...
9. SCENE CONSISTENCY RULE:
   ...
   - In contentMode=`storyboard`, the sequence must read as a continuous story arc. Each beat should include a transition reason from the prior beat and set up the next beat; avoid isolated, disconnected mini-scenes.
...
[Always output this section. Keep it compact: 2-5 lines when possible. Generate a useful visual reference bible from the idea, user reference notes, source news, and any attached images. If images exist, explicitly mention @Image handles and roles with only the most important face/hair/outfit/body/object/setting details to preserve. Do not let this section replace or shorten the required PROMPT blocks.]

CONTINUITY NOTES (shared across all prompts):
[Always output this section. Keep it compact: 4-8 short lines when possible. Create the canonical story continuity bible using these labels only when useful: Presenter Identity, Wardrobe, Studio/Layout, Lighting, Camera Language, Recurring Props, Story Arc, Continuity Lock. For contentMode=storyboard, preserve stable character/cast identity and story flow. Do not let this section replace or shorten the required PROMPT blocks.]
...
[ONLY IF contentMode is not news_narration AND videoAudioWorkflow includes separate voice AND storyboardAudioDurationSeconds > 0]
PROMPT 1 ({storyboardClipDurationSeconds} seconds):
Continuity Lock: [repeat the exact active character/group/location lock from CONTINUITY NOTES. For the Thai mother/baby two-image storyboard, this line must explicitly mention same @Image1 Thai mother and same @Image2 Thai 6-month-old baby boy together in the same luxurious nursery/child bedroom with scattered toys, soft warm lighting, full vertical 9:16 edge-to-edge framing, no letterboxing, no cinematic black bars, no top or bottom matte bars.]
A high-quality {style} clip ({storyboardClipDurationSeconds} seconds).
Visual action: [what happens visually in this beat only; keep it visual-only and do not describe spoken words, singing, whispering, humming, narration, sound, or music]
Background Visuals: [scene-appropriate visual background with recurring nursery/toys/crib details; no readable text]
Continuity Transition: [how this beat continues from the previous prompt and leads into the next prompt]
...
No subtitles, no captions, no lower-thirds, no readable text or numbers anywhere in frame unless the exact overlay line above is explicitly enabled, no logos with letters, no random glyphs.
...
- For contentMode=storyboard, ensure story continuity and character consistency are stronger than scene novelty: the same recurring character/cast must keep the same identity, wardrobe, proportions, reference-image traits, and world anchors across all prompts while actions progress beat by beat.
...
- If reference images exist, explicitly mention the @Image handles and their roles in REFERENCE NOTES and carry recurring image-derived identity details into CONTINUITY NOTES and Continuity Lock.

...
// The db mock uses a thenable query builder so `await db.select()...` works
...
// ---------------------------------------------------------------------------
// Thenable query builder factory
// ---------------------------------------------------------------------------

/**
 * Creates a Drizzle-like query builder mock that resolves to `data` when awaited.
 * Each call to the mock returns a new builder so chaining works.
 */
function makeQueryBuilder(data: any): any {
  const builder: any = {
  ...
  for (const m of [
    "from", "innerJoin", "leftJoin", "where", "limit", "offset", "orderBy",
  ]) {
...
function makeInsertBuilder(): any {
  const returning = vi.fn().mockResolvedValue([{ id: "new-conv-1" }]);
...
interface MockDbOptions {
  /** Data returned for the first select (agency ownership check) */
  agencyRows?: any[];
  /** Data returned for the second select (conversation check) */
  convRows?: any[];
}

function buildMockDb({ agencyRows = [], convRows = [] }: MockDbOptions = {}) {
  ...
  const db: any = {
    select: vi.fn().mockImplementation(() => {
      ...
      // Agency ownership checks return agencyRows; conv lookups return convRows
      return makeQueryBuilder(callIdx === 0 ? agencyRows : convRows);
    }),
...
describe("GET /v1/agencies", () => {
  beforeEach(() => {
    mockDbInstance = buildMockDb({ agencyRows: [agency] });
  });
...
describe("POST /v1/agencies/:agencyId/invoke", () => {
  beforeEach(() => {
    // agencyRows: agency found; convRows: no existing conversation → will insert
    mockDbInstance = buildMockDb({ agencyRows: [agency], convRows: [] });
  ...
  it("rejects agency from a different tenant with 404", async () => {
    mockDbInstance = buildMockDb({ agencyRows: [] }); // no agencies found
    const res = await request(makeApp())
  ...
  it("validates agencyId format to prevent path traversal", async () => {
    // agencyId with dots/slashes fails the AGENCY_ID_PATTERN check
    const res = await request(makeApp())
  ...
  it("enforces max_credits budget cap with refund of unused credits", async () => {
    // Run uses 10 credits, max_credits=50 → refund 40
    const res = await request(makeApp())
      .post("/v1/agencies/agency-1/invoke")
...
describe("GET /v1/agencies/:agencyId/runs/:runId", () => {
  ...
  it("returns 404 for run belonging to different tenant", async () => {
    mockDbInstance = buildMockDb({ agencyRows: [] }); // agency not found for this tenant

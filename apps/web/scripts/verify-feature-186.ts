...
if (missing.length || missingContractFields.length || missingContractVersion.length || missingTimeoutPolicy.length || forbidden || artifacts.length) {
  console.error(JSON.stringify({ ok: false, missing, missingContractFields, missingContractVersion, missingTimeoutPolicy, forbiddenMigrationOperation: forbidden, missingArtifacts: artifacts }));
  process.exit(1);
}

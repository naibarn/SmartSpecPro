/**
 * OneDriveFolderPicker - lazy-loading folder tree dialog for OneDrive folder selection.
 *
 * Used by the OneDrive dashboard to let users choose which folders
...
function FolderTreeNode({
  ...
  const childrenQuery = trpc.oneDrive.listDriveFolders.useQuery(
...
export function OneDriveFolderPicker({
  ...
  const rootFoldersQuery = trpc.oneDrive.listDriveFolders.useQuery(
    { parentFolderId: null },
    { enabled: open },
  );
  ...
  const handleConfirm = () => {
    const folders = Array.from(selectedFolders.entries()).map(
      ([id, name]) => ({ id, name }),
    );
    onConfirm(folders);
    onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Manage OneDrive Folders</DialogTitle>
        ...
        <div className="flex-1 overflow-y-auto border rounded-lg p-2 min-h-[200px] max-h-[400px]">
          ...
          {rootFoldersQuery.data?.length === 0 &&
            !rootFoldersQuery.isLoading && (
              <div className="flex items-center justify-center py-8 text-sm text-gray-500">
                No folders found in your OneDrive
              </div>
            )}
        </div>

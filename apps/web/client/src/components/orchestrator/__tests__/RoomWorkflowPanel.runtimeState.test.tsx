...
describe("RoomWorkflowPanel runtime visibility", () => {
  it("renders the durable runtime summary for the active run", async () => {
    ...
    expect(screen.getByTitle("Policy gate (Evidence): evidence missing")).toHaveClass(
      "border-amber-200",
      "bg-amber-50",
      "text-amber-700",
    );
    expect(screen.getByText(/Waiting: polling media job/i)).toBeInTheDocument();
    expect(screen.getByText(/Policy gate: evidence missing/i)).toBeInTheDocument();
    ...
    expect(screen.getByTitle("Candidate plan comparison summary")).toBeInTheDocument();
    ...
    expect(screen.getAllByText(/Proceed/i)[0]).toHaveClass("border-emerald-200", "bg-emerald-50", "text-emerald-700");
    expect(screen.getByText(/Loops: 1/i)).toBeInTheDocument();
    expect(screen.getByText(/Looks solid; proceed to execution\./i)).toBeInTheDocument();
    expect(screen.getByText(/Score: 0.92/i)).toHaveClass("border-emerald-200", "bg-emerald-50", "text-emerald-700");
    expect(screen.getByRole("tab", { name: "Summary" })).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: "Evidence" })).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: "Raw" })).toBeInTheDocument();

    await user.click(screen.getByRole("tab", { name: "Evidence" }));
    expect(screen.getByText(/Plan and decompose the objective/i)).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Details" }));
    expect(screen.getByText(/Evidence: durable plan artifact, subtask breakdown, review note/i)).toBeInTheDocument();

    await user.click(screen.getByRole("tab", { name: "Raw" }));
    expect(screen.getByText(/"currentPhase": "waiting_for_poll"/i)).toBeInTheDocument();
    expect(screen.getByText(/"selectedCandidateId": "balanced-hybrid"/i)).toBeInTheDocument();
  });
  ...
  it("shows the final approval window after reviewer sign-off", () => {
    ...
    expect(screen.getByTitle("The automation reviewer scored the final output")).toBeInTheDocument();

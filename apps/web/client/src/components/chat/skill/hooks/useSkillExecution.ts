...
export function useSkillExecution(
  ...
  const externalLocalTextBackend =
    useExternalLocalTextBackendAvailability(platform);
    ...
    async (params: {
        ...
        if (effectivePolicy?.runtimeKind === "gemma4_text") {
          ...
          if (platform === "tauri" && allowOnDeviceLocalEngine) {
            for (const profileId of candidateProfiles) {
              ...
              localTextError =
                localTextResult.error ?? "local_gemma4_text_failed";
            }
          }
          ...
          if (localOnlyRequested) {
            const message =
              localTextError ??
              resolveBlockedLocalExecutionMessage(
                params.skillId,
                effectivePolicy
              );
            const executionResult: SkillExecutionResult = {
          ...
          if (!localSkillPath) {
            ...
            if (localOnlyRequested) {
              setError(
                new Error(localResult.error || "Local skill execution failed.")
              );
      ...
      try {
        const _data = await mutation.mutateAsync({
          ...
          platform,
          origin,
          requestedExecutionRoute: allowCloudMediaRoute
            ? "cloud"
            : localAiEnabled &&
                !forceCloudOnly &&
                localAiExecutionMode === "local_only" &&
        ...
        const data = _data as typeof _data & {
          isAsync?: boolean;
          taskId?: string;
          jobId?: string;
        };
        ...
        if (data.isAsync && data.taskId) {
          ...
          for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
            ...
            if (task.status === "done") {
              ...
              return finishRemoteSkillResult(
                task.result as SkillExecutionResult
              );
            }
        ...
        // Legacy sandbox jobs are no longer dispatched. Keep a hard failure here so
        // stale skill records cannot bypass the worker control plane.

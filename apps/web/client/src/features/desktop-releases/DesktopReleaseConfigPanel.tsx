...
import { AlertCircle, Eye, EyeOff, Globe, KeyRound, Loader2, RefreshCw, Save, ShieldCheck, Sparkles } from "lucide-react";
...
function sourceBadgeLabel(
  ...
  if (source === "db") return t("dashboard:desktopReleases.admin.config.source.ui");
  if (source === "env") return t("dashboard:desktopReleases.admin.config.source.legacy");
...
export function DesktopReleaseConfigPanel(props: { enabled?: boolean }) {
  ...
  const { data: config, isLoading, refetch } = trpc.systemSettings.getDesktopReleaseSettings.useQuery(undefined, {
    enabled,
  });

  const updateMutation = trpc.systemSettings.updateDesktopReleaseSettings.useMutation({
    onSuccess: (result) => {
      toast.success(t("dashboard:desktopReleases.admin.config.saveSuccess"));
      setForm((prev) => ({
        ...prev,
        githubToken: "",
        githubRepository: result.settings.githubRepository || prev.githubRepository,
    ...
    onError: (error) => {
      toast.error(error.message || t("dashboard:desktopReleases.admin.config.saveFailed"));
    },
  });
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="githubRepository">{t("dashboard:desktopReleases.admin.config.repository")}</Label>
                  <Badge variant="outline" className={sourceBadgeClass(config?.githubRepositorySource)}>
                ...
                <Input
                  ...
                  onChange={(event) => setForm((prev) => ({ ...prev, githubRepository: event.target.value }))}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <Label htmlFor="githubWorkflow">{t("dashboard:desktopReleases.admin.config.workflow")}</Label>
                  <Badge variant="outline" className={sourceBadgeClass(config?.githubWorkflowSource)}>
                ...
                <Input
                  ...
                  onChange={(event) => setForm((prev) => ({ ...prev, githubWorkflow: event.target.value }))}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <Label htmlFor="githubRef">{t("dashboard:desktopReleases.admin.config.ref")}</Label>
                  <Badge variant="outline" className={sourceBadgeClass(config?.githubRefSource)}>
                ...
                <Input
                  ...
                  onChange={(event) => setForm((prev) => ({ ...prev, githubRef: event.target.value }))}
                  placeholder="main"
                />
              </div>
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="webUrl">{t("dashboard:desktopReleases.admin.config.webUrl")}</Label>
                  <Badge variant="outline" className={sourceBadgeClass(config?.webUrlSource)}>
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="githubToken">{t("dashboard:desktopReleases.admin.config.githubToken")}</Label>
                  <Badge variant="outline" className={sourceBadgeClass(config?.githubTokenSource)}>
                    {config?.githubTokenConfigured
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder={config?.githubTokenConfigured
                      ? t("dashboard:desktopReleases.admin.config.githubTokenKeep")
                      : t("dashboard:desktopReleases.admin.config.githubTokenPlaceholder")}
                  />
                  <Button
                    type="button"
                    variant="ghost"
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-sky-100 bg-white/95 p-4 shadow-sm">
              <div className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                ...
                {t("dashboard:desktopReleases.admin.config.guide.title")}
              </div>
              <p className="mt-1 text-sm leading-6 text-slate-600">
                {t("dashboard:desktopReleases.admin.config.guide.description")}
              </p>
              <ol className="mt-4 space-y-3 text-sm text-slate-700">
                {[
                  ...
                  <li key={step} className="rounded-2xl border border-slate-200 bg-slate-50/80 p-3">
                    <div className="font-medium text-slate-900">
                      {t(`dashboard:desktopReleases.admin.config.guide.${step}.title`)}
                    </div>
                    <div className="mt-1 leading-6 text-slate-600">
                      {t(`dashboard:desktopReleases.admin.config.guide.${step}.body`)}
                    </div>
                  </li>
                ))}
              </ol>
            ...
            <div className="rounded-2xl border border-amber-100 bg-amber-50/70 p-4 shadow-sm">
              ...
              <div className="mt-4 grid gap-2">
                {sourceInfoRows.map((row) => (
                  <div key={row.label} className="flex items-center justify-between gap-3 rounded-xl border border-amber-100 bg-white/80 px-3 py-2 text-sm">
                    <span className="font-medium text-slate-700">{row.label}</span>
                    <Badge variant="outline" className={sourceBadgeClass(row.source)}>
            ...
            <div className="rounded-2xl border border-cyan-100 bg-cyan-50/70 p-4 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-cyan-900/90">
                {t("dashboard:desktopReleases.admin.config.githubTokenGuide.description")}
              </p>
              <ol className="mt-4 space-y-2 text-sm text-cyan-900/90">
                {[
                  ...
                  <li key={step} className="rounded-xl border border-cyan-100 bg-white/85 px-3 py-2">
                    <div className="font-medium text-cyan-950">
                      {t(`dashboard:desktopReleases.admin.config.githubTokenGuide.${step}.title`)}
                    </div>
                    <div className="mt-1 leading-6 text-cyan-900/80">
                      {t(`dashboard:desktopReleases.admin.config.githubTokenGuide.${step}.body`)}
                    </div>
                  </li>
                ))}
              </ol>

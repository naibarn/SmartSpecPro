...
export default function AdminSTTProviders() {
  ...
  const { data: providers, refetch } = trpc.sttProviders.list.useQuery(undefined, {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-pink-50/20">
      ...
      <header className="bg-white/70 backdrop-blur-xl border-b border-gray-200/50 sticky top-0 z-10">
        <div className="px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="sm" onClick={() => setLocation("/dashboard")} className="text-gray-600">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Speech-to-Text Providers</h1>
                <p className="text-sm text-gray-500">Configure STT providers for voice transcription</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {(templates || []).map((tpl) => {
          ...
          return (
            <div key={tpl.providerName} className="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/50 shadow-lg shadow-purple-500/5 p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                    configured?.hasApiKey ? "bg-green-100" : "bg-gray-100"
                  }`}>
                    <Mic className={`w-6 h-6 ${configured?.hasApiKey ? "text-green-600" : "text-gray-400"}`} />
                  </div>
                  <div>
                    <div className="font-bold text-gray-900 text-lg">{tpl.displayName}</div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {configured && (
                    <button
                      onClick={() => toggleMutation.mutate({ id: configured.id })}
              ...
              {isEditing ? (
                <div className="space-y-3 border-t border-gray-200 pt-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="outline" onClick={() => { setEditId(null); setApiKey(""); }}>
                      Cancel
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex gap-2 border-t border-gray-200 pt-4">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => { setEditId(configured?.id ?? -1); setApiKey(""); }}
                  >
                  ...
                  {configured?.hasApiKey && (
                    <>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => testMutation.mutate({ id: configured.id })}
                        disabled={testMutation.isPending}
                      >
                        {testMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <TestTube2 className="w-4 h-4 mr-1" />}
                        Test Connection
                      </Button>
                      <Button
                        size="sm"
                        ...
                        onClick={() => deleteMutation.mutate({ id: configured.id })}

...
const BEAM_PAYMENT_LINK_GUIDE_URL =
  "https://docs.beamcheckout.com/payment-links/payment-links";
  ...
  return {
    tenant_id:
      typeof data.tenant_id === "string"
        ? data.tenant_id
        : DEFAULT_VECTOR_DB_HEALTH.tenant_id,
    timestamp:
      typeof data.timestamp === "string"
        ? data.timestamp
        : DEFAULT_VECTOR_DB_HEALTH.timestamp,
    provider_status: {
...
function McpProviderConfigPanel() {
  ...
  const save = () => {
    saveConfig.mutate({
      ...
      redirectAllowlist: form.redirectAllowlist
        .split("\n")
        .map(item => item.trim())
        .filter(Boolean),
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <Switch
            ...
            onCheckedChange={checked =>
              setForm(prev => ({ ...prev, [`${prefix}Enabled`]: checked }))
            }
          />
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            ...
            <Input
              value={form[`${prefix}ClientId` as const] as string}
              onChange={event =>
                setForm(prev => ({
                  ...prev,
                  [`${prefix}ClientId`]: event.target.value,
                }))
              }
            />
          </div>
          <div>
            ...
            <Input
              ...
              value={form[`${prefix}ClientSecret` as const] as string}
              onChange={event =>
                setForm(prev => ({
                  ...prev,
                  [`${prefix}ClientSecret`]: event.target.value,
                }))
              }
            />
          </div>
          <div>
            ...
            <Input
              value={form[`${prefix}AuthorizationUrl` as const] as string}
              onChange={event =>
                setForm(prev => ({
                  ...prev,
                  [`${prefix}AuthorizationUrl`]: event.target.value,
                }))
              }
            />
          </div>
          <div>
            ...
            <Input
              value={form[`${prefix}TokenUrl` as const] as string}
              onChange={event =>
                setForm(prev => ({
                  ...prev,
                  [`${prefix}TokenUrl`]: event.target.value,
                }))
              }
            />
          </div>
    ...
    >
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div>
          ...
          <Input
            type="number"
            value={form.schemaCacheTtlSeconds}
            onChange={event =>
              setForm(prev => ({
                ...prev,
                schemaCacheTtlSeconds: Number(event.target.value),
              }))
            }
          />
        </div>
      ...
      <div>
        ...
        <Textarea
          ...
          onChange={event =>
            setForm(prev => ({
              ...prev,
              redirectAllowlist: event.target.value,
            }))
          }
          placeholder="One allowed origin per line"
        />
      </div>
...
export default function AdminSettings() {
  ...
  const copy = {
    ...
    platformSubtitle: isThai
      ? "จัดการ integrations ความปลอดภัย และการเชื่อมต่อระบบ"
      : "Configure integrations and security",
    languageLabel: isThai ? "ภาษา" : "Language",
    languageHelp: isThai
      ? "สลับภาษาได้ทันทีในหน้านี้"
      : "Switch this page instantly",
    billingConsole: isThai ? "คอนโซลบิลลิ่ง" : "Billing Console",
    providerSwitch: {
      title: isThai ? "เกตเวย์การชำระเงิน" : "Payment Gateways",
      description: isThai
        ? "เลือกผู้ให้บริการชำระเงินหลัก และจัดการ Stripe, Beam และ PromptPay Direct ในที่เดียว"
        : "Choose the active payment provider and manage Stripe, Beam, and PromptPay Direct in one place.",
      activePaymentProvider: isThai
        ? "ผู้ให้บริการชำระเงินหลัก"
        : "Active payment provider",
      save: isThai ? "บันทึกการสลับผู้ให้บริการ" : "Save Provider Switch",
      summaryTitle: isThai
        ? "สรุปการตั้งค่าใช้งานปัจจุบัน"
      ...
      stripeDesc: isThai
        ? "ให้ Stripe ยังเป็นตัวเลือกการชำระเงินได้"
        : "Keep Stripe available as a provider.",
      beamEnabled: isThai ? "เปิดใช้งาน Beam" : "Beam enabled",
      beamDesc: isThai
        ? "เปิดใช้ Beam checkout, PromptPay QR และระบบบิลลิ่งของ Beam"
        : "Enable Beam checkout, QR PromptPay, and Beam billing runtime.",
      beamFallback: isThai
        ? "ใช้ Beam payment link เป็น fallback"
        : "Beam payment link fallback",
      beamFallbackDesc: isThai
        ? "ให้ใช้ Beam payment link ได้เมื่อ charge flow ปกติใช้งานไม่ได้"
        : "Allow Beam payment links as fallback when charge flow is unavailable.",
      runtimeNote: isThai
        ? "การตั้งค่าแต่ละช่องทางอยู่ในแท็บของตัวเองด้านล่าง"
        : "Each payment method has its own settings tab below.",
    ...
    stripe: {
      ...
      descPrefix: isThai
        ? "ตั้งค่า Stripe API keys สำหรับการชำระเงิน รับคีย์ได้จาก"
      ...
      publishableHint: isThai
        ? "ใช้ในฝั่ง frontend สำหรับ Stripe.js integration"
    ...
    sms: {
      ...
      secretPlaceholderKeep: isThai
        ...
        : "•••••••• (leave blank to keep)",
      secretPlaceholderNew: isThai
        ? "กรอก token หรือ secret"
        : "Enter token/secret",
      fromLabel: isThai ? "From Number / Sender ID" : "From Number / Sender ID",
      fromPlaceholder: isThai
        ? "SMARTSPEC หรือ +1234567890"
        : "SMARTSPEC or +1234567890",
      fromHint: isThai
      ...
      recommendedTitle: isThai
        ? "Twilio เป็นผู้ให้บริการที่แนะนำ"
      ...
      twilioGuideTitle: isThai
        ? "Twilio Setup และ Sender ID"
        : "Twilio Setup and Sender ID",
      ...
      docsTitle: isThai
        ? "ฟอร์ม/เอกสารหลัก (ตัวอย่างประเทศไทย)"
        : "Core forms/documents (Thailand example)",
      docsItems: isThai
        ? [
    ...
    beam: {
      ...
      description: isThai
        ? "ตั้งค่า Beam checkout, PromptPay QR, payment links, webhook verification และ card setup จากหน้า Payments นี้"
        : "Configure Beam checkout, PromptPay QR, payment links, webhook verification, and card setup from the main Payments page.",
      step1Title: isThai
        ? "1. เปิด Beam Lighthouse"
        : "1. Open Beam Lighthouse",
      step1Body: isThai
        ? "ใช้ merchant console ของ Beam เพื่อคัดลอก API credentials, ตั้งค่า webhook endpoint และจัดการ hosted checkout"
        : "Use your Beam merchant console to copy API credentials, configure webhook endpoints, and manage hosted checkout settings.",
      step1Button: isThai ? "เปิด Beam Lighthouse" : "Open Beam Lighthouse",
      step2Title: isThai
        ? "2. คู่มือตั้งค่า Webhook"
        : "2. Webhook setup guide",
      step2Body: isThai
        ? "ลงทะเบียน webhook endpoint ใน Beam แล้วคัดลอก signing secret มากรอกด้านล่าง"
        : "Register your webhook endpoint in Beam and copy the signing secret into the fields below.",
      step2Button: isThai ? "เปิดคู่มือ Webhook" : "Open Webhook Guide",
      step3Title: isThai ? "3. คู่มือ Payment Links" : "3. Payment Links guide",
      step3Body: isThai
        ? "ใช้ตอนเปิด Beam-hosted checkout fallback หรือใช้ตรวจสอบ path templates ของ payment links"
        : "Use this when enabling Beam-hosted checkout fallback or validating path templates for payment links.",
      ...
      configuredSecrets: isThai
        ? "Secrets ที่ตั้งค่าไว้"
        : "Configured secrets",
      yes: isThai ? "ใช่" : "Yes",
      no: isThai ? "ไม่" : "No",
      notConfigured: isThai ? "ยังไม่ตั้งค่า" : "not configured",
      missing: isThai ? "ยังขาด" : "Missing",
      gatewayNotice: isThai
        ? "การตั้งค่า Beam gateway อยู่ในหน้า Payments แล้ว ส่วน Billing Console ใช้เฉพาะ invoice, recovery และงานปฏิบัติการ"
        : "Beam gateway setup now lives in Payments. Billing Console remains only for invoice, recovery, and operational workflows.",
      ...
      runtimeDesc: isThai
        ? "จัดการนโยบายบิลลิ่งของ Beam, recovery toggles, QR/card setup flow, reminders และ rollout controls จากหน้า Payments"
        : "Manage Beam-specific billing policies, recovery toggles, QR/card setup flow, reminders, and rollout controls from the Payments page.",
      runtimeSummary: isThai
        ? "สรุปการตั้งค่า Beam runtime"
        : "Beam runtime summary",
      openBillingConsole: isThai ? "เปิดคอนโซลบิลลิ่ง" : "Open Billing Console",
      field: {
        apiBaseUrl: isThai ? "API base URL" : "API base URL",
        apiBaseUrlHint: isThai
          ? "ปลายทางหลักของ Beam API ตัวอย่าง production คือ https://api.beamcheckout.com และ sandbox คือ https://playground.api.beamcheckout.com"
          : "Beam API base endpoint. Example: `https://api.beamcheckout.com` for production or `https://playground.api.beamcheckout.com` for sandbox.",
        apiKey: isThai ? "API key" : "API key",
        apiKeyKeep: isThai
          ? "เว้นว่างเพื่อคง API key เดิมไว้"
          : "Leave blank to keep existing key",
        apiKeyEnter: isThai ? "กรอก API key" : "Enter API key",
        apiKeyHint: isThai
          ? "คัดลอกมาจาก developer credentials ใน Beam Lighthouse ถ้าไม่ต้องการเปลี่ยนคีย์เดิมให้เว้นว่างไว้"
          : "Copy this from Beam Lighthouse developer credentials. Leave blank if you only want to keep the existing saved key.",
        chargesPath: isThai ? "Charges path" : "Charges path",
        chargesPathHint: isThai
          ? "relative path สำหรับสร้าง direct charge เช่น /v1/charges"
          : "Relative path used to create direct charges. Example: `/v1/charges`.",
        paymentLinksPath: isThai ? "Payment Links path" : "Payment Links path",
        paymentLinksPathHint: isThai
          ? "relative path สำหรับสร้าง Beam-hosted payment links เช่น /api/v1/payment-links"
          : "Relative path used to create Beam-hosted payment links. Example: `/api/v1/payment-links` or your Beam environment’s equivalent path.",
        chargeStatusPathTemplate: isThai
          ? "Charge status path template"
          : "Charge status path template",
        chargeStatusPathTemplateHint: isThai
          ? "template สำหรับดึง charge ตาม id โดยใช้ placeholder {id}"
          : "Template for retrieving a charge by id. Use `{id}` placeholder. Example: `/v1/charges/{id}`.",
        paymentLinkStatusPathTemplate: isThai
          ? "Payment Link status path template"
          : "Payment Link status path template",
        paymentLinkStatusPathTemplateHint: isThai
          ? "template สำหรับดึง payment link ตาม id โดยใช้ placeholder {id}"
          : "Template for retrieving a payment link by id. Use `{id}` placeholder. Example: `/api/v1/payment-links/{id}`.",
        cancelPathSuffix: isThai ? "Cancel path suffix" : "Cancel path suffix",
        cancelPathSuffixHint: isThai
          ? "suffix ที่ต่อท้ายเวลายกเลิกหรือปิด Beam object เช่น /cancel"
          : "Suffix appended when canceling or disabling a Beam object. Example: `/cancel`.",
        setupApiPath: isThai ? "Setup API path" : "Setup API path",
        setupApiPathHint: isThai
          ? "path สำหรับสร้าง session setup/authorization ของบัตรที่บันทึกไว้ ใช้เมื่อบัญชี Beam รองรับ card setup"
          : "API path used to create card setup or authorization sessions for saved-card enrollment. Fill this only if your Beam account supports card setup.",
        hostedSetupUrlTemplate: isThai
          ? "Hosted setup URL template"
          : "Hosted setup URL template",
        hostedSetupUrlTemplateHint: isThai
          ? "URL template ของ Beam hosted card setup ต้องมี {sessionId} และมักมี {returnUrl}"
          : "Beam-hosted card setup page URL template. Must include placeholders like `{sessionId}` and usually `{returnUrl}`.",
        setupReturnUrl: isThai ? "Setup return URL" : "Setup return URL",
        setupReturnUrlHint: isThai
          ? "URL ที่ Beam จะพาผู้ใช้กลับมาหลังทำ hosted card setup เสร็จ เช่นหน้าบิลลิ่งหรือหน้ากลับจาก checkout"
          : "Where Beam should redirect the user after completing hosted card setup. Example: your billing settings or checkout return page.",
        webhookSecretCurrent: isThai
          ? "Webhook secret ปัจจุบัน"
          : "Webhook secret current",
        webhookSecretCurrentKeep: isThai
          ? "เว้นว่างเพื่อคง secret เดิมไว้"
          : "Leave blank to keep existing secret",
        webhookSecretCurrentEnter: isThai
          ? "กรอก webhook secret"
          : "Enter webhook secret",
        webhookSecretCurrentHint: isThai
          ? "HMAC signing secret ปัจจุบันจากการตั้งค่า Beam webhook ให้วาง secret ล่าสุดที่ใช้งานอยู่"
          : "Current HMAC signing secret from Beam webhook configuration. Paste the newest active secret here.",
        webhookSecretPrevious: isThai
          ? "Webhook secret ก่อนหน้า"
          : "Webhook secret previous",
        webhookSecretPreviousKeep: isThai
          ? "เว้นว่างเพื่อคง secret เดิมไว้"
          : "Leave blank to keep previous secret",
        optional: isThai ? "ไม่บังคับ" : "Optional",
        webhookSecretPreviousHint: isThai
          ? "ใช้ชั่วคราวตอน rotate secret เพื่อให้ webhook retry เก่าตรวจสอบผ่านได้"
          : "Optional previous secret used during secret rotation so older retries can still validate.",
        setupCallbackSecretCurrent: isThai
          ? "Setup callback secret ปัจจุบัน"
          : "Setup callback secret current",
        setupCallbackSecretCurrentKeep: isThai
          ? "เว้นว่างเพื่อคง secret เดิมไว้"
          : "Leave blank to keep existing secret",
        setupCallbackSecretCurrentEnter: isThai
          ? "กรอก callback secret"
          : "Enter callback secret",
        setupCallbackSecretCurrentHint: isThai
          ? "secret สำหรับตรวจสอบ Beam hosted setup callbacks ควรแยกจาก webhook secret ปกติหาก Beam ออก secret เฉพาะมาให้"
          : "Secret used to verify Beam hosted setup callbacks. Keep it separate from the normal webhook secret if Beam provides a dedicated callback secret.",
        setupCallbackSecretPrevious: isThai
          ? "Setup callback secret ก่อนหน้า"
          : "Setup callback secret previous",
        setupCallbackSecretPreviousKeep: isThai
          ? "เว้นว่างเพื่อคง secret เดิมไว้"
          : "Leave blank to keep previous secret",
        setupCallbackSecretPreviousHint: isThai
          ? "ใช้ชั่วคราวเฉพาะช่วงที่กำลัง rotate setup callback verification"
          : "Optional previous callback secret used only while rotating setup callback verification.",
      },
      runtimeLabels: {
        PAYMENT_RECONCILIATION_ENABLED: isThai
          ? "กระทบยอดการชำระเงิน"
          : "Payment reconciliation",
        FINAL_RECONCILIATION_BEFORE_DOWNGRADE: isThai
          ? "กระทบยอดรอบสุดท้ายก่อน downgrade"
          : "Final reconciliation before downgrade",
        AUTO_DOWNGRADE_AFTER_7_DAYS: isThai
          ? "downgrade อัตโนมัติหลัง 7 วัน"
          : "Auto downgrade after 7 days",
        INVOICE_HEADER_SYNC_ENABLED: isThai
          ? "ซิงก์หัวใบแจ้งหนี้"
          : "Invoice header sync",
        PAID_INVOICE_REISSUE_ENABLED: isThai
          ? "ออก paid invoice ใหม่ได้"
          : "Paid invoice reissue",
        BILLING_PHASE2_SAVED_CARDS_ENABLED: isThai
          ? "บันทึกบัตร"
          : "Saved cards",
        BILLING_PHASE2_AUTO_RENEW_ENABLED: isThai
          ? "ต่ออายุอัตโนมัติ"
          : "Auto renew",
        BILLING_PHASE2_DUNNING_ENABLED: isThai ? "Dunning" : "Dunning",
        BILLING_PHASE2_CARD_SETUP_ENABLED: isThai
          ? "ตั้งค่าบัตร"
          : "Card setup",
        BILLING_PHASE2_FORCE_MANUAL_FALLBACK_ENABLED: isThai
          ? "สั่ง fallback เป็น manual"
          : "Manual fallback actions",
        BILLING_EMAIL_NOTIFICATIONS_ENABLED: isThai
          ? "อีเมลแจ้งเตือนบิลลิ่ง"
          : "Billing email notifications",
        BILLING_PHASE2_REQUIRE_STEP_UP: isThai
          ? "บังคับ step-up auth"
          : "Require step-up auth",
        SUPPORT_RECOVERY_CASES_ENABLED: isThai
          ? "เคส recovery ของ support"
          : "Support recovery cases",
        DOCUMENT_RECOVERY_ENABLED: isThai
          ? "กู้คืนเอกสาร"
          : "Document recovery",
        BILLING_SUBSCRIPTION_CUTOVER_READY: isThai
          ? "พร้อม cutover subscription"
          : "Subscription cutover ready",
      },
      runtimeFields: {
        allowedCohorts: isThai
          ? "cohort ที่อนุญาตให้ rollout"
          : "Allowed rollout cohorts",
        defaultCohort: isThai ? "cohort เริ่มต้น" : "Default rollout cohort",
        billingPublicUrl: isThai ? "Billing public URL" : "Billing public URL",
        stepUpSharedSecret: isThai
          ? "Step-up shared secret"
          : "Step-up shared secret",
        stepUpSharedSecretKeep: isThai
          ? "เว้นว่างเพื่อคง secret เดิมไว้"
          : "Leave blank to keep existing secret",
        stepUpSharedSecretEnter: isThai
          ? "กรอก step-up secret"
          : "Enter step-up secret",
        setupCallbackToleranceSeconds: isThai
          ? "วินาทีที่ยอมรับ setup callback ได้"
          : "Setup callback tolerance seconds",
        webhookTimestampToleranceSeconds: isThai
          ? "วินาทีที่ยอมรับ webhook timestamp ได้"
          : "Webhook timestamp tolerance seconds",
        paymentMethodSetupExpiryMinutes: isThai
          ? "อายุ setup payment method (นาที)"
          : "Payment method setup expiry minutes",
        stepUpWindowMinutes: isThai
          ? "หน้าต่างเวลา step-up (นาที)"
          : "Step-up window minutes",
        evidenceRetentionDays: isThai
          ? "อายุเก็บ evidence (วัน)"
          : "Evidence retention days",
        overdueDays: isThai ? "จำนวนวันค้างชำระ" : "Overdue days",
        subscriptionRenewalDueDays: isThai
          ? "วันครบกำหนด renewal subscription"
          : "Subscription renewal due days",
        topupDueDays: isThai ? "วันครบกำหนด top-up" : "Top-up due days",
        reminderFirstThresholdDays: isThai
          ? "วันเตือนครั้งแรก"
          : "Reminder first threshold days",
        reminderFinalThresholdDays: isThai
          ? "วันเตือนครั้งสุดท้าย"
          : "Reminder final threshold days",
        reminderCooldownHours: isThai
          ? "คูลดาวน์การเตือน (ชั่วโมง)"
          : "Reminder cooldown hours",
        successCooldownHours: isThai
          ? "คูลดาวน์หลังจ่ายสำเร็จ (ชั่วโมง)"
          : "Success cooldown hours",
        defaultCooldownHours: isThai
          ? "คูลดาวน์เริ่มต้น (ชั่วโมง)"
    ...
    nav: {
      payments: {
        label: isThai ? "การชำระเงิน" : "Payments",
        sublabel: isThai
          ? "Stripe / Beam / PromptPay Direct"
          : "Stripe / Beam / PromptPay Direct",
      },
      oauth: {
        label: "OAuth",
        sublabel: isThai ? "เข้าสู่ระบบโซเชียล" : "Social Login",
      },
      registration: {
        label: isThai ? "การสมัครสมาชิก" : "Registration",
        sublabel: isThai ? "สมัครใช้งาน / เครดิต" : "Signup & Credits",
      },
      smtp: {
        label: isThai ? "อีเมล" : "Email",
        sublabel: isThai ? "ตั้งค่า SMTP" : "SMTP Settings",
      },
      sms: {
        label: "SMS",
        sublabel: isThai ? "ตั้งค่าผู้ให้บริการ" : "Provider Config",
      },
      telegram: {
        label: isThai ? "บอท Telegram" : "Telegram Bot",
        sublabel: isThai ? "แจ้งเตือน" : "Alert Notifications",
      },
      twoFA: {
        label: "2FA",
        sublabel: isThai ? "ตัวพิสูจน์ตัวตน" : "Authenticator",
      },
      stt: {
        label: "STT",
        sublabel: isThai ? "แปลงเสียงเป็นข้อความ" : "Speech-to-Text",
      },
      ai: {
        label: isThai ? "AI / หน่วยความจำ" : "AI / Memory",
        sublabel: isThai ? "โมเดลสรุปผล" : "Summary Model",
      },
      documentOcr: {
        label: isThai ? "OCR" : "OCR",
        sublabel: isThai ? "เส้นทาง / keys" : "Routing / keys",
      },
      financeRules: {
        label: isThai ? "Rules" : "Rules",
        sublabel: isThai ? "merchant pins / slips" : "Merchant pins / slips",
      },
      vectordb: {
        label: isThai ? "ฐานข้อมูลเวกเตอร์" : "Vector Database",
        sublabel: isThai ? "RAG / Embeddings" : "RAG & Embeddings",
      },
      storage: {
        label: isThai ? "พื้นที่จัดเก็บ" : "Storage",
        sublabel: isThai ? "Local / R2 / S3" : "Local / R2 / S3",
      },
      infrastructure: {
        label: isThai ? "โครงสร้างพื้นฐาน" : "Infrastructure",
        sublabel: isThai ? "GCP / Redis / Tasks" : "GCP / Redis / Tasks",
      },
      workerRuntime: {
      ...
      menu: {
        label: isThai ? "เมนูหลัก" : "Main Menu",
        sublabel: isThai ? "การมองเห็นเมนู" : "Visibility Control",
      },
    },
  } as const;
  ...
  const [paymentsSubTab, setPaymentsSubTab] = useState<
    "stripe" | "beam" | "promptpay_direct"
  >("beam");
  const [beamProviderForm, setBeamProviderForm] = useState<BeamProviderForm>(
    EMPTY_BEAM_PROVIDER_FORM
  );
  const [paymentProviderForm, setPaymentProviderForm] =
    useState<PaymentProviderForm>({
      BILLING_ACTIVE_PROVIDER: "beam",
      BILLING_STRIPE_ENABLED: false,
      BILLING_BEAM_ENABLED: true,
      BEAM_PAYMENT_LINK_FALLBACK: true,
    });
  const [beamRuntimeForm, setBeamRuntimeForm] = useState<BeamRuntimeForm>(
    EMPTY_BEAM_RUNTIME_FORM
  );
  const [promptPayDirectRuntimeForm, setPromptPayDirectRuntimeForm] =
    useState<PromptPayDirectRuntimeForm>(EMPTY_PROMPTPAY_DIRECT_RUNTIME_FORM);
  ...
  const [regForm, setRegForm] = useState({
    ...
    allowedAuthMethods: ["email", "google", "github"] as Array<
      "email" | "google" | "github"
    >,
  ...
  // Mutations
  const updateStripeMutation =
    trpc.systemSettings.updateStripeSettings.useMutation({
      onSuccess: () => {
  ...
  const testStripeMutation =
    trpc.systemSettings.testStripeConnection.useMutation({
      onSuccess: data => {
        if (data.success) {
  ...
  const beamProviderSettingsQuery =
    trpc.adminBilling.getBeamProviderSettings.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });
  const beamProviderHealthQuery =
    trpc.adminBilling.testBeamProviderSettings.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });
  const billingRuntimeSettingsQuery =
    trpc.adminBilling.getBillingRuntimeSettings.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });

  const updateBeamProviderSettingsMutation =
    trpc.adminBilling.updateBeamProviderSettings.useMutation({
      onSuccess: () => {
  ...
  const updateBillingProviderMutation =
    trpc.adminBilling.updateBillingRuntimeSettings.useMutation({
      onSuccess: () => {
      ...
      onError: err => {
        const message = err.message.includes("PROMPTPAY_DIRECT_ENABLED")
          ? isThai
            ? "กรุณาเปิดใช้งาน PromptPay Direct ในแท็บ PromptPay Direct ก่อนตั้งเป็นผู้ให้บริการหลัก"
            : "Enable PromptPay Direct in its tab before selecting it as the active provider."
          : err.message.includes("PROMPTPAY_DIRECT_RECIPIENT_ID_NOT_CONFIGURED")
            ? isThai
              ? "กรุณาตั้งค่าบัญชีรับเงิน PromptPay Direct ก่อนเปิดใช้งาน"
              : "Configure the PromptPay Direct receiving account before enabling it."
  ...
  const updateOAuthMutation =
    trpc.systemSettings.updateOAuthSettings.useMutation({
      onSuccess: () => {
        toast.success(
          isThai
            ? "บันทึกการตั้งค่า OAuth สำเร็จ"
  ...
  const testGoogleOAuthMutation =
    trpc.systemSettings.testGoogleOAuthConnection.useMutation({
      onSuccess: data => {
        if (data.success) {
          toast.success(data.message);
        } else {
  ...
  const testMetaOAuthMutation =
    trpc.systemSettings.testMetaOAuthConnection.useMutation({
      onSuccess: data => {
        if (data.success) {
          toast.success(
            isThai
  ...
  const updateRegMutation =
    trpc.systemSettings.updateRegistrationSettings.useMutation({
      onSuccess: () => {
  ...
  useEffect(() => {
    if (regSettings) {
      setRegForm({
        ...
        allowedAuthMethods: (regSettings.allowedAuthMethods ?? [
          "email",
          "google",
          "github",
  ...
  const { data: smsSettings, refetch: refetchSms } =
    trpc.systemSettings.getSmsSettings.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });
  ...
  const updateTelegramMutation =
    trpc.telegram.updateTelegramSettings.useMutation({
      onSuccess: () => {
  ...
  const testTelegramMutation = trpc.telegram.testTelegramConnection.useMutation(
    {
      onSuccess: data => {
        if (data.success) {
          toast.success(
            `Connected to bot: @${data.botInfo?.username || "unknown"}`
          );
        } else {
  ...
  const { data: twoFaSettings, refetch: refetchTwoFa } =
    trpc.systemSettings.getTwoFaSettings.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });
  const updateTwoFaMutation =
    trpc.systemSettings.updateTwoFaSettings.useMutation({
      onSuccess: () => {
  ...
  const { data: sttProviders, refetch: refetchStt } =
    trpc.sttProviders.list.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });
  const { data: sttTemplates } = trpc.sttProviders.templates.useQuery(
    undefined,
    {
      enabled: !!user && user.role === "admin",
    }
  ...
  const [
    marketplaceImageIndexAllowedHosts,
    setMarketplaceImageIndexAllowedHosts,
  ] = useState("");
  const { data: aiSettings, refetch: refetchAi } =
    trpc.systemSettings.getSettingsByCategory.useQuery(
      { category: "ai" as any },
      { enabled: !!user && user.role === "admin" }
    );
  const {
    data: marketplaceCaptureSettings,
    refetch: refetchMarketplaceCaptureSettings,
  } = trpc.systemSettings.getSettingsByCategory.useQuery(
  ...
  const updateMarketplaceCaptureSettingMutation =
    trpc.systemSettings.updateSetting.useMutation({
      onSuccess: () => {
  ...
  const enabledAiSummaryModelIds = (modelsData?.models ?? []).map(
    (model: any) => model.id
  );
  ...
  useEffect(() => {
    ...
    const summaryModelSetting = aiSettings.find(
      (s: any) => s.key === "summaryModel"
    );
    const userOwnKeysSetting = aiSettings.find(
      (s: any) => s.key === "allowUserOwnLlmApiKeys"
    );
    setAiSummaryModel(
      pickEnabledModelId({
    ...
    aiSettings,
    defaultAiSummaryModelId,
    enabledAiSummaryModelIds,
    modelsData?.models,
  ]);

  useEffect(() => {
    const allowedHostsSetting = marketplaceCaptureSettings?.find(
      (s: any) => s.key === "image_index_allowed_hosts"
    );
  ...
  const [pgvectorPasswordConfigured, setPgvectorPasswordConfigured] =
    useState(false);
  const [openaiApiKeyConfigured, setOpenaiApiKeyConfigured] = useState(false);
  const [vectorizeApiTokenConfigured, setVectorizeApiTokenConfigured] =
    useState(false);
  const [showProviderSwitchWarning, setShowProviderSwitchWarning] =
    useState(false);
  ...
  const { data: vectorDbSettings, refetch: refetchVectorDb } =
    trpc.systemSettings.getVectorDbSettings.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });
  const { data: vectorDbStats, refetch: refetchVectorDbStats } =
    trpc.systemSettings.getVectorDbStats.useQuery(undefined, {
      enabled: !!user && user.role === "admin",
    });
  ...
  const reindexProgressValue =
    reindexExpectedJobs > 0
      ? Math.min(
          100,
          Math.max(0, (reindexCompletedJobs / reindexExpectedJobs) * 100)
        )
      : 0;
  ...
  useEffect(() => {
    ...
    if (reindexStatus.status === "completed_with_errors") {
      if (isReindexing) {
        toast.warning(
          "Reindex completed with some errors — review recent failures in Vector DB Health"
        );
      }
      refetchVectorDbStats();
      refetchVectorDbHealth();
  ...
  useEffect(() => {
    if (vectorDbSettings) {
      ...
      setVectorizeApiTokenConfigured(
        !!vectorDbSettings.vectorizeApiTokenConfigured
      );
    }
  }, [vectorDbSettings]);
  ...
  useEffect(() => {
    ...
    setBeamProviderForm(prev => ({
      ...
      chargesPath:
        beamProviderSettingsQuery.data.chargesPath ?? prev.chargesPath,
      paymentLinksPath:
        beamProviderSettingsQuery.data.paymentLinksPath ??
        prev.paymentLinksPath,
      chargeStatusPathTemplate:
        beamProviderSettingsQuery.data.chargeStatusPathTemplate ??
        prev.chargeStatusPathTemplate,
      paymentLinkStatusPathTemplate:
        beamProviderSettingsQuery.data.paymentLinkStatusPathTemplate ??
        prev.paymentLinkStatusPathTemplate,
      cancelPathSuffix:
        beamProviderSettingsQuery.data.cancelPathSuffix ??
        prev.cancelPathSuffix,
      apiKey: "",
      webhookSecretCurrent: "",
      webhookSecretPrevious: "",
      paymentMethodSetupPath:
        beamProviderSettingsQuery.data.paymentMethodSetupPath ?? "",
      paymentMethodSetupHostedUrlTemplate:
        beamProviderSettingsQuery.data.paymentMethodSetupHostedUrlTemplate ??
        "",
      paymentMethodSetupReturnUrl:
        beamProviderSettingsQuery.data.paymentMethodSetupReturnUrl ?? "",
      paymentMethodSetupCallbackSecretCurrent: "",
      paymentMethodSetupCallbackSecretPrevious: "",
    }));
  ...
  useEffect(() => {
    ...
    const activeProvider =
      billingRuntimeSettingsQuery.data.BILLING_ACTIVE_PROVIDER === "stripe"
        ? "stripe"
        : billingRuntimeSettingsQuery.data.BILLING_ACTIVE_PROVIDER ===
            "promptpay_direct"
          ? "promptpay_direct"
          : "beam";
    setPaymentProviderForm({
      BILLING_ACTIVE_PROVIDER: activeProvider,
      BILLING_STRIPE_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_STRIPE_ENABLED
      ),
      BILLING_BEAM_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_BEAM_ENABLED
      ),
      BEAM_PAYMENT_LINK_FALLBACK: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BEAM_PAYMENT_LINK_FALLBACK
      ),
    });
    setBeamRuntimeForm({
      PAYMENT_RECONCILIATION_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.PAYMENT_RECONCILIATION_ENABLED
      ),
      FINAL_RECONCILIATION_BEFORE_DOWNGRADE: readBooleanSetting(
        billingRuntimeSettingsQuery.data.FINAL_RECONCILIATION_BEFORE_DOWNGRADE
      ),
      ADMIN_MANUAL_MARK_PAID_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.ADMIN_MANUAL_MARK_PAID_ENABLED
      ),
      ADMIN_DOWNGRADE_REVERSAL_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.ADMIN_DOWNGRADE_REVERSAL_ENABLED
      ),
      SUPPORT_RECOVERY_CASES_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.SUPPORT_RECOVERY_CASES_ENABLED
      ),
      DOCUMENT_RECOVERY_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.DOCUMENT_RECOVERY_ENABLED
      ),
      INVOICE_HEADER_SYNC_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.INVOICE_HEADER_SYNC_ENABLED
      ),
      PAID_INVOICE_REISSUE_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.PAID_INVOICE_REISSUE_ENABLED
      ),
      AUTO_DOWNGRADE_AFTER_7_DAYS: readBooleanSetting(
        billingRuntimeSettingsQuery.data.AUTO_DOWNGRADE_AFTER_7_DAYS
      ),
      BILLING_PHASE2_SAVED_CARDS_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_PHASE2_SAVED_CARDS_ENABLED
      ),
      BILLING_PHASE2_AUTO_RENEW_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_PHASE2_AUTO_RENEW_ENABLED
      ),
      BILLING_PHASE2_DUNNING_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_PHASE2_DUNNING_ENABLED
      ),
      BILLING_PHASE2_CARD_SETUP_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_PHASE2_CARD_SETUP_ENABLED
      ),
      BILLING_PHASE2_FORCE_MANUAL_FALLBACK_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data
          .BILLING_PHASE2_FORCE_MANUAL_FALLBACK_ENABLED
      ),
      BILLING_EMAIL_NOTIFICATIONS_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_EMAIL_NOTIFICATIONS_ENABLED
      ),
      BILLING_PHASE2_REQUIRE_STEP_UP: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_PHASE2_REQUIRE_STEP_UP
      ),
      BILLING_PHASE2_ALLOWED_COHORTS:
        billingRuntimeSettingsQuery.data.BILLING_PHASE2_ALLOWED_COHORTS ?? "",
      BILLING_PHASE2_DEFAULT_COHORT:
        billingRuntimeSettingsQuery.data.BILLING_PHASE2_DEFAULT_COHORT ?? "",
      BILLING_PHASE2_SETUP_CALLBACK_TOLERANCE_SECONDS:
        billingRuntimeSettingsQuery.data
          .BILLING_PHASE2_SETUP_CALLBACK_TOLERANCE_SECONDS ?? "300",
      BILLING_WEBHOOK_TIMESTAMP_TOLERANCE_SECONDS:
        billingRuntimeSettingsQuery.data
          .BILLING_WEBHOOK_TIMESTAMP_TOLERANCE_SECONDS ?? "300",
      BILLING_PAYMENT_METHOD_SETUP_EXPIRY_MINUTES:
        billingRuntimeSettingsQuery.data
          .BILLING_PAYMENT_METHOD_SETUP_EXPIRY_MINUTES ?? "60",
      BILLING_PHASE2_STEP_UP_WINDOW_MINUTES:
        billingRuntimeSettingsQuery.data
          .BILLING_PHASE2_STEP_UP_WINDOW_MINUTES ?? "15",
      BILLING_EVIDENCE_RETENTION_DAYS:
        billingRuntimeSettingsQuery.data.BILLING_EVIDENCE_RETENTION_DAYS ??
        "180",
      BILLING_OVERDUE_DAYS:
        billingRuntimeSettingsQuery.data.BILLING_OVERDUE_DAYS ?? "7",
      BILLING_SUBSCRIPTION_RENEWAL_DUE_DAYS:
        billingRuntimeSettingsQuery.data
          .BILLING_SUBSCRIPTION_RENEWAL_DUE_DAYS ?? "7",
      BILLING_TOPUP_DUE_DAYS:
        billingRuntimeSettingsQuery.data.BILLING_TOPUP_DUE_DAYS ?? "1",
      BILLING_NOTIFICATION_REMINDER_FIRST_THRESHOLD_DAYS:
        billingRuntimeSettingsQuery.data
          .BILLING_NOTIFICATION_REMINDER_FIRST_THRESHOLD_DAYS ?? "4",
      BILLING_NOTIFICATION_REMINDER_FINAL_THRESHOLD_DAYS:
        billingRuntimeSettingsQuery.data
          .BILLING_NOTIFICATION_REMINDER_FINAL_THRESHOLD_DAYS ?? "1",
      BILLING_NOTIFICATION_COOLDOWN_REMINDER_HOURS:
        billingRuntimeSettingsQuery.data
          .BILLING_NOTIFICATION_COOLDOWN_REMINDER_HOURS ?? "12",
      BILLING_NOTIFICATION_COOLDOWN_SUCCESS_HOURS:
        billingRuntimeSettingsQuery.data
          .BILLING_NOTIFICATION_COOLDOWN_SUCCESS_HOURS ?? "24",
      BILLING_NOTIFICATION_COOLDOWN_DEFAULT_HOURS:
        billingRuntimeSettingsQuery.data
          .BILLING_NOTIFICATION_COOLDOWN_DEFAULT_HOURS ?? "1",
      BILLING_SUBSCRIPTION_CUTOVER_READY: readBooleanSetting(
        billingRuntimeSettingsQuery.data.BILLING_SUBSCRIPTION_CUTOVER_READY
      ),
      BILLING_PUBLIC_URL:
        billingRuntimeSettingsQuery.data.BILLING_PUBLIC_URL ?? "",
      BILLING_PHASE2_STEP_UP_SECRET: "",
    });
    setPaymentsSubTab(activeProvider);
    setPromptPayDirectRuntimeForm({
      PROMPTPAY_DIRECT_ENABLED: readBooleanSetting(
        billingRuntimeSettingsQuery.data.PROMPTPAY_DIRECT_ENABLED
      ),
      PROMPTPAY_DIRECT_RECIPIENT_ID: "",
      PROMPTPAY_DIRECT_RECIPIENT_TYPE: (billingRuntimeSettingsQuery.data
        .PROMPTPAY_DIRECT_RECIPIENT_TYPE ??
        "phone") as PromptPayDirectRuntimeForm["PROMPTPAY_DIRECT_RECIPIENT_TYPE"],
      PROMPTPAY_DIRECT_ACCOUNT_DISPLAY_NAME:
        billingRuntimeSettingsQuery.data
          .PROMPTPAY_DIRECT_ACCOUNT_DISPLAY_NAME ?? "",
      PROMPTPAY_DIRECT_ORDER_EXPIRY_MINUTES:
        billingRuntimeSettingsQuery.data
          .PROMPTPAY_DIRECT_ORDER_EXPIRY_MINUTES ?? "60",
      PROMPTPAY_DIRECT_FX_MAX_RATE_AGE_HOURS:
        billingRuntimeSettingsQuery.data
          .PROMPTPAY_DIRECT_FX_MAX_RATE_AGE_HOURS ?? "72",
      PROMPTPAY_DIRECT_FX_SELL_SPREAD_BPS:
        billingRuntimeSettingsQuery.data.PROMPTPAY_DIRECT_FX_SELL_SPREAD_BPS ??
        "200",
      PROMPTPAY_DIRECT_FX_RISK_BUFFER_BPS:
        billingRuntimeSettingsQuery.data.PROMPTPAY_DIRECT_FX_RISK_BUFFER_BPS ??
        "300",
      PROMPTPAY_DIRECT_FX_SANITY_MIN_RATE:
        billingRuntimeSettingsQuery.data.PROMPTPAY_DIRECT_FX_SANITY_MIN_RATE ??
        "20",
      PROMPTPAY_DIRECT_FX_SANITY_MAX_RATE:
        billingRuntimeSettingsQuery.data.PROMPTPAY_DIRECT_FX_SANITY_MAX_RATE ??
        "60",
      PROMPTPAY_DIRECT_SLIP_MAX_BYTES:
        billingRuntimeSettingsQuery.data.PROMPTPAY_DIRECT_SLIP_MAX_BYTES ??
        "10485760",
      PROMPTPAY_DIRECT_SLIP_ALLOWED_TYPES:
        billingRuntimeSettingsQuery.data.PROMPTPAY_DIRECT_SLIP_ALLOWED_TYPES ??
        "application/pdf,image/png,image/jpeg,image/webp",
    });
  }, [billingRuntimeSettingsQuery.data]);
    ...
    if (oauthSettings) {
      setOauthForm({
        ...
        googleDriveRedirectUri:
          (oauthSettings.googleDriveRedirectUri as string) || "",
        ...
        microsoftOneDriveRedirectUri:
          (oauthSettings.microsoftOneDriveRedirectUri as string) || "",
        metaAppId: (oauthSettings.metaAppId as string) || "",
        metaRedirectUri: (oauthSettings.metaRedirectUri as string) || "",
        metaGraphApiVersion:
          (oauthSettings.metaGraphApiVersion as string) || "v25.0",
      });
  ...
  const googleSecretReady =
    googleSecretConfigured || hasConfiguredValue(oauthForm.googleClientSecret);
  const googleLoginReady =
    hasConfiguredValue(oauthForm.googleClientId) &&
    googleSecretReady &&
    hasConfiguredValue(oauthForm.googleRedirectUri);
  const googleDriveReady =
    hasConfiguredValue(oauthForm.googleClientId) &&
    googleSecretReady &&
    hasConfiguredValue(oauthForm.googleDriveRedirectUri);
  ...
  const navItems = [
    {
      key: "stripe",
      label: copy.nav.payments.label,
      sublabel: copy.nav.payments.sublabel,
      icon: CreditCard,
    },
    {
      key: "oauth",
      label: copy.nav.oauth.label,
      sublabel: copy.nav.oauth.sublabel,
      icon: Globe,
    },
    {
      key: "registration",
      label: copy.nav.registration.label,
      sublabel: copy.nav.registration.sublabel,
      icon: UserPlus,
    },
    {
      key: "smtp",
      label: copy.nav.smtp.label,
      sublabel: copy.nav.smtp.sublabel,
      icon: Mail,
    },
    {
      key: "sms",
      label: copy.nav.sms.label,
      sublabel: copy.nav.sms.sublabel,
      icon: MessageSquare,
    },
    {
      key: "telegram",
      label: copy.nav.telegram.label,
      sublabel: copy.nav.telegram.sublabel,
      icon: Send,
    },
    {
      key: "2FA",
      label: copy.nav.twoFA.label,
      sublabel: copy.nav.twoFA.sublabel,
      icon: Shield,
    },
    {
      key: "stt",
      label: copy.nav.stt.label,
      sublabel: copy.nav.stt.sublabel,
      icon: Mic,
    },
    {
      key: "ai",
      label: copy.nav.ai.label,
      sublabel: copy.nav.ai.sublabel,
      icon: Brain,
    },
    {
      key: "document_ocr",
      label: copy.nav.documentOcr.label,
      sublabel: copy.nav.documentOcr.sublabel,
      icon: FileText,
    },
    {
      key: "marketplace_capture",
      label: "Marketplace Capture",
      sublabel: "Visual search",
      icon: Store,
    },
    {
      key: "mcp_connect",
      label: "MCP Connect",
      sublabel: "Provider config",
      icon: Cable,
    },
    {
      key: "finance_rules",
      label: copy.nav.financeRules.label,
      sublabel: copy.nav.financeRules.sublabel,
      icon: CheckSquare,
    },
    {
      key: "vectordb",
    ...
    {
      key: "worker_runtime",
      label: copy.nav.workerRuntime.label,
      sublabel: copy.nav.workerRuntime.sublabel,
      icon: Key,
    },
    {
      key: "public_contact",
    ...
    {
      key: "automation",
      label: copy.nav.automation.label,
      sublabel: copy.nav.automation.sublabel,
      icon: Bot,
    },
    {
      key: "menu",
      label: copy.nav.menu.label,
      sublabel: copy.nav.menu.sublabel,
      icon: Menu,
    },
  ];
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="stripe">
                <div className="space-y-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <div className="text-sm font-medium text-slate-900">
                          {copy.providerSwitch.activePaymentProvider}
                        </div>
                        <div className="mt-2">
                          <Select
                            ...
                            onValueChange={value => {
                              const provider =
                                value as PaymentProviderForm["BILLING_ACTIVE_PROVIDER"];
                              setPaymentProviderForm(prev => ({
                                ...prev,
                                BILLING_ACTIVE_PROVIDER: provider,
                              }));
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="promptpay_direct">
                                PromptPay Direct
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        ...
                        <div className="mt-3 space-y-3 text-sm">
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <div className="font-medium text-slate-900">
                                {copy.providerSwitch.stripeEnabled}
                              </div>
                              <div className="text-xs text-slate-500">
                                {copy.providerSwitch.stripeDesc}
                              </div>
                            </div>
                            <Switch
                              checked={
                                paymentProviderForm.BILLING_STRIPE_ENABLED
                              }
                              onCheckedChange={checked =>
                                setPaymentProviderForm(prev => ({
                                  ...prev,
                                  ...
                                  BILLING_ACTIVE_PROVIDER:
                                    !checked &&
                                    prev.BILLING_ACTIVE_PROVIDER === "stripe"
                                      ? "beam"
                                      : prev.BILLING_ACTIVE_PROVIDER,
                                }))
                              }
                            />
                          </div>
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <div className="font-medium text-slate-900">
                                {copy.providerSwitch.beamEnabled}
                              </div>
                              <div className="text-xs text-slate-500">
                                {copy.providerSwitch.beamDesc}
                              </div>
                            </div>
                            <Switch
                              ...
                              onCheckedChange={checked =>
                                setPaymentProviderForm(prev => ({
                                  ...
                                  BILLING_ACTIVE_PROVIDER:
                                    !checked &&
                                    prev.BILLING_ACTIVE_PROVIDER === "beam"
                                      ? "stripe"
                                      : prev.BILLING_ACTIVE_PROVIDER,
                                }))
                              }
                          ...
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <div className="font-medium text-slate-900">
                                {copy.providerSwitch.beamFallback}
                              </div>
                              <div className="text-xs text-slate-500">
                                {copy.providerSwitch.beamFallbackDesc}
                              </div>
                            </div>
                            <Switch
                              checked={
                                paymentProviderForm.BEAM_PAYMENT_LINK_FALLBACK
                              }
                              onCheckedChange={checked =>
                                setPaymentProviderForm(prev => ({
                                  ...prev,
                                  BEAM_PAYMENT_LINK_FALLBACK: checked,
                                }))
                              }
                            />
                          </div>
                        ...
                        <div className="mt-4 flex justify-end">
                          <Button
                            ...
                            disabled={updateBillingProviderMutation.isPending}
                          >
                            {updateBillingProviderMutation.isPending ? (
                      ...
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                        <div className="font-medium text-slate-900">
                          {copy.providerSwitch.summaryTitle}
                        </div>
                        <div className="mt-2">
                          {copy.providerSwitch.activeProvider}:{" "}
                          <span className="font-medium">
                            {paymentProviderForm.BILLING_ACTIVE_PROVIDER ===
                            "promptpay_direct"
                              ? "PromptPay Direct"
                              : paymentProviderForm.BILLING_ACTIVE_PROVIDER.toUpperCase()}
                          </span>
                        </div>
                        <div>
                          Stripe:{" "}
                          {paymentProviderForm.BILLING_STRIPE_ENABLED
                            ? copy.providerSwitch.enabled
                            : copy.providerSwitch.disabled}
                        </div>
                        <div>
                          Beam:{" "}
                          {paymentProviderForm.BILLING_BEAM_ENABLED
                            ? copy.providerSwitch.enabled
                            : copy.providerSwitch.disabled}
                        </div>
                        <div>
                          PromptPay Direct:{" "}
                          {promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_ENABLED
                            ? copy.providerSwitch.enabled
                            : copy.providerSwitch.disabled}
                        </div>
                        <div>
                  ...
                  <Tabs
                    ...
                    onValueChange={value =>
                      setPaymentsSubTab(
                        value as "stripe" | "beam" | "promptpay_direct"
                      )
                    }
                  >
                    <TabsList className="grid w-full grid-cols-3">
                      <TabsTrigger value="stripe">
                        {copy.providerSwitch.stripeLabel}
                      </TabsTrigger>
                      <TabsTrigger value="beam">
                        {copy.providerSwitch.beamLabel}
                      </TabsTrigger>
                      <TabsTrigger value="promptpay_direct">
                        PromptPay Direct
                      </TabsTrigger>
                    </TabsList>
                          ...
                          <div className="flex gap-2 mt-1">
                            <div className="relative flex-1">
                              <Input
                                ...
                                placeholder={
                                  stripeForm.secretKeyConfigured
                                    ? copy.stripe.secretPlaceholderKeep
                                    : copy.stripe.secretPlaceholderNew
                                }
                          ...
                          <div className="flex gap-2 mt-1">
                            <div className="relative flex-1">
                              <Input
                                ...
                                placeholder={
                                  stripeForm.webhookSecretConfigured
                                    ? copy.stripe.webhookPlaceholderKeep
                                    : copy.stripe.webhookPlaceholderNew
                                }
                        ...
                        <div>
                          ...
                          >
                            <SelectTrigger className="mt-1">
                              <SelectValue
                                placeholder={copy.stripe.selectCurrency}
                              />
                            </SelectTrigger>
                            <SelectContent>
                      ...
                      <div className="space-y-6">
                        ...
                        >
                            ...
                            <div className="rounded-xl border border-cyan-200 bg-cyan-50/70 p-4 text-sm text-slate-700">
                              ...
                              <div className="mt-3 flex flex-wrap gap-2">
                                <Button asChild variant="outline" size="sm">
                                  <a
                                    href={BEAM_PAYMENT_LINK_GUIDE_URL}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                  >
                                    <ExternalLink className="mr-2 h-4 w-4" />
                                ...
                                <Button asChild variant="outline" size="sm">
                                  <a
                                    href={BEAM_PAYMENT_LINK_API_GUIDE_URL}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                  >
                                    <ExternalLink className="mr-2 h-4 w-4" />
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                              ...
                              <div>
                                API configured:{" "}
                                {beamProviderHealthQuery.data?.configured
                                  ? copy.beam.yes
                                  : copy.beam.no}
                              </div>
                              <div>
                                Webhook configured:{" "}
                                {beamProviderHealthQuery.data?.webhookConfigured
                                  ? copy.beam.yes
                                  : copy.beam.no}
                              </div>
                              <div>
                                Hosted setup configured:{" "}
                                {beamProviderHealthQuery.data
                                  ?.setupHostedConfigured
                                  ? copy.beam.yes
                                  : copy.beam.no}
                              </div>
                              <div>
                              ...
                              {(beamProviderHealthQuery.data?.missing?.length ??
                                0) > 0 ? (
                                <div className="mt-2 text-rose-700">
                                  {copy.beam.missing}:{" "}
                                  {(
                                    beamProviderHealthQuery.data?.missing ?? []
                                  ).join(", ")}
                                </div>
                              ) : null}
                            </div>
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                              <div className="font-medium text-slate-900">
                                {copy.beam.configuredSecrets}
                              </div>
                              <div className="mt-2">
                                API key:{" "}
                                {beamProviderSettingsQuery.data
                                  ?.apiKeyConfigured
                                  ? beamProviderSettingsQuery.data?.apiKeyMasked
                                  : copy.beam.notConfigured}
                              </div>
                              <div>
                                Webhook current:{" "}
                                {beamProviderSettingsQuery.data
                                  ?.webhookSecretCurrentConfigured
                                  ? beamProviderSettingsQuery.data
                              ...
                              <div>
                                Webhook previous:{" "}
                                {beamProviderSettingsQuery.data
                                  ?.webhookSecretPreviousConfigured
                                  ? beamProviderSettingsQuery.data
                                      ?.webhookSecretPreviousMasked
                                  : copy.beam.notConfigured}
                              </div>
                              <div>
                                Setup callback current:{" "}
                                {beamProviderSettingsQuery.data
                                  ?.paymentMethodSetupCallbackSecretCurrentConfigured
                                  ? beamProviderSettingsQuery.data
                                      ?.paymentMethodSetupCallbackSecretCurrentMasked
                                  : copy.beam.notConfigured}
                              </div>
                              <div>
                                Setup callback previous:{" "}
                                {beamProviderSettingsQuery.data
                                  ?.paymentMethodSetupCallbackSecretPreviousConfigured
                                  ? beamProviderSettingsQuery.data
                                      ?.paymentMethodSetupCallbackSecretPreviousMasked
                                  : copy.beam.notConfigured}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            <div>
                              ...
                              <Input
                                value={beamProviderForm.apiBaseUrl}
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                ...
                                placeholder="https://api.beam.example"
                              />
                            ...
                            <div>
                              ...
                              <div className="relative">
                                <Input
                                  type={showBeamSecrets ? "text" : "password"}
                                  value={beamProviderForm.apiKey}
                                  onChange={e =>
                                    setBeamProviderForm(prev => ({
                                      ...prev,
                                  ...
                                  placeholder={
                                    beamProviderSettingsQuery.data
                                      ?.apiKeyConfigured
                                      ? copy.beam.field.apiKeyKeep
                                      : copy.beam.field.apiKeyEnter
                                  }
                                />
                              </div>
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              ...
                              <Input
                                value={beamProviderForm.chargesPath}
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    chargesPath: e.target.value,
                                  }))
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              ...
                              <Input
                                value={beamProviderForm.paymentLinksPath}
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    paymentLinksPath: e.target.value,
                                  }))
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              <Label>
                                {copy.beam.field.chargeStatusPathTemplate}
                              </Label>
                              <Input
                                value={
                                  beamProviderForm.chargeStatusPathTemplate
                                }
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    chargeStatusPathTemplate: e.target.value,
                                  }))
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              <Label>
                                {copy.beam.field.paymentLinkStatusPathTemplate}
                              </Label>
                              <Input
                                value={
                                  beamProviderForm.paymentLinkStatusPathTemplate
                                }
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                              ...
                              <p className="mt-1 text-xs text-slate-500">
                                {
                                  copy.beam.field
                                    .paymentLinkStatusPathTemplateHint
                                }
                              </p>
                            </div>
                            <div>
                              ...
                              <Input
                                value={beamProviderForm.cancelPathSuffix}
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    cancelPathSuffix: e.target.value,
                                  }))
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              ...
                              <Input
                                value={beamProviderForm.paymentMethodSetupPath}
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    paymentMethodSetupPath: e.target.value,
                                  }))
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div className="md:col-span-2">
                              <Label>
                                {copy.beam.field.hostedSetupUrlTemplate}
                              </Label>
                              <Input
                                value={
                                  beamProviderForm.paymentMethodSetupHostedUrlTemplate
                                }
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                ...
                                placeholder="https://beam.example/setup?session={sessionId}&return={returnUrl}"
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div className="md:col-span-2">
                              ...
                              <Input
                                value={
                                  beamProviderForm.paymentMethodSetupReturnUrl
                                }
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    paymentMethodSetupReturnUrl: e.target.value,
                                  }))
                                }
                                placeholder="https://app.example/billing"
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              <Label>
                                {copy.beam.field.webhookSecretCurrent}
                              </Label>
                              <Input
                                type={showBeamSecrets ? "text" : "password"}
                                value={beamProviderForm.webhookSecretCurrent}
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    webhookSecretCurrent: e.target.value,
                                  }))
                                }
                                placeholder={
                                  beamProviderSettingsQuery.data
                                    ?.webhookSecretCurrentConfigured
                                    ? copy.beam.field.webhookSecretCurrentKeep
                                    : copy.beam.field.webhookSecretCurrentEnter
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              <Label>
                                {copy.beam.field.webhookSecretPrevious}
                              </Label>
                              <Input
                                type={showBeamSecrets ? "text" : "password"}
                                value={beamProviderForm.webhookSecretPrevious}
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                    webhookSecretPrevious: e.target.value,
                                  }))
                                }
                                placeholder={
                                  beamProviderSettingsQuery.data
                                    ?.webhookSecretPreviousConfigured
                                    ? copy.beam.field.webhookSecretPreviousKeep
                            ...
                            <div>
                              <Label>
                                {copy.beam.field.setupCallbackSecretCurrent}
                              </Label>
                              <Input
                                type={showBeamSecrets ? "text" : "password"}
                                value={
                                  beamProviderForm.paymentMethodSetupCallbackSecretCurrent
                                }
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                ...
                                placeholder={
                                  beamProviderSettingsQuery.data
                                    ?.paymentMethodSetupCallbackSecretCurrentConfigured
                                    ? copy.beam.field
                                        .setupCallbackSecretCurrentKeep
                                    : copy.beam.field
                                        .setupCallbackSecretCurrentEnter
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                            ...
                            <div>
                              <Label>
                                {copy.beam.field.setupCallbackSecretPrevious}
                              </Label>
                              <Input
                                type={showBeamSecrets ? "text" : "password"}
                                value={
                                  beamProviderForm.paymentMethodSetupCallbackSecretPrevious
                                }
                                onChange={e =>
                                  setBeamProviderForm(prev => ({
                                    ...prev,
                                ...
                                placeholder={
                                  beamProviderSettingsQuery.data
                                    ?.paymentMethodSetupCallbackSecretPreviousConfigured
                                    ? copy.beam.field
                                        .setupCallbackSecretPreviousKeep
                                    : copy.beam.field.optional
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500">
                                {
                                  copy.beam.field
                                    .setupCallbackSecretPreviousHint
                                }
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-3">
                            ...
                            <Button
                              type="button"
                              variant="outline"
                              onClick={() => setShowBeamSecrets(v => !v)}
                            >
                              {showBeamSecrets ? (
                              ...
                              {showBeamSecrets
                                ? copy.beam.hideSecrets
                                : copy.beam.showSecrets}
                            </Button>
                          </div>
                          ...
                          <div className="flex gap-3 pt-4 border-t">
                            <Button
                              ...
                              disabled={
                                updateBeamProviderSettingsMutation.isPending
                              }
                            >
                              {updateBeamProviderSettingsMutation.isPending ? (
                                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                            ...
                            <Button
                              variant="outline"
                              onClick={() => beamProviderHealthQuery.refetch()}
                              disabled={beamProviderHealthQuery.isFetching}
                            >
                              {beamProviderHealthQuery.isFetching ? (
                                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                              ) : (
                                <TestTube className="w-4 h-4 mr-2" />
                              )}
                        ...
                        >
                            ...
                            {[
                              ...
                              >
                                <div className="pr-3">
                                  <div className="text-sm font-medium text-slate-900">
                                    {
                                      copy.beam.runtimeLabels[
                                        key as keyof typeof copy.beam.runtimeLabels
                                      ]
                                    }
                                  </div>
                                  <div className="text-xs text-slate-500">
                                ...
                                <Switch
                                  checked={Boolean(
                                    beamRuntimeForm[
                                      key as keyof BeamRuntimeForm
                                    ]
                                  )}
                                  onCheckedChange={checked =>
                                    setBeamRuntimeForm(prev => ({
                                      ...prev,
                                      [key]: checked,
                                    }))
                                  }
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.allowedCohorts}
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_PHASE2_ALLOWED_COHORTS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                                ...
                                placeholder="pilot-a,pilot-b"
                              />
                            </div>
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.defaultCohort}
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_PHASE2_DEFAULT_COHORT
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.billingPublicUrl}
                              </Label>
                              <Input
                                value={beamRuntimeForm.BILLING_PUBLIC_URL}
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                                    BILLING_PUBLIC_URL: e.target.value,
                                  }))
                                }
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.stepUpSharedSecret}
                              </Label>
                              <Input
                                type={showBeamSecrets ? "text" : "password"}
                                value={
                                  beamRuntimeForm.BILLING_PHASE2_STEP_UP_SECRET
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                                ...
                                placeholder={
                                  billingRuntimeSettingsQuery.data
                                    ?.BILLING_PHASE2_STEP_UP_SECRETConfigured
                                    ? copy.beam.runtimeFields
                                        .stepUpSharedSecretKeep
                                    : copy.beam.runtimeFields
                                        .stepUpSharedSecretEnter
                                }
                              />
                            </div>
                            <div>
                              <Label>
                                {
                                  copy.beam.runtimeFields
                                    .setupCallbackToleranceSeconds
                                }
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_PHASE2_SETUP_CALLBACK_TOLERANCE_SECONDS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              ...
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_WEBHOOK_TIMESTAMP_TOLERANCE_SECONDS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {
                                  copy.beam.runtimeFields
                                    .paymentMethodSetupExpiryMinutes
                                }
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_PAYMENT_METHOD_SETUP_EXPIRY_MINUTES
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.stepUpWindowMinutes}
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_PHASE2_STEP_UP_WINDOW_MINUTES
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.evidenceRetentionDays}
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_EVIDENCE_RETENTION_DAYS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.overdueDays}
                              </Label>
                              <Input
                                value={beamRuntimeForm.BILLING_OVERDUE_DAYS}
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                                    BILLING_OVERDUE_DAYS: e.target.value,
                                  }))
                                }
                              />
                            </div>
                            <div>
                              <Label>
                                {
                                  copy.beam.runtimeFields
                                    .subscriptionRenewalDueDays
                                }
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_SUBSCRIPTION_RENEWAL_DUE_DAYS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.topupDueDays}
                              </Label>
                              <Input
                                value={beamRuntimeForm.BILLING_TOPUP_DUE_DAYS}
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                                    BILLING_TOPUP_DUE_DAYS: e.target.value,
                                  }))
                                }
                              />
                            </div>
                            <div>
                              <Label>
                                {
                                  copy.beam.runtimeFields
                                    .reminderFirstThresholdDays
                                }
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_NOTIFICATION_REMINDER_FIRST_THRESHOLD_DAYS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {
                                  copy.beam.runtimeFields
                                    .reminderFinalThresholdDays
                                }
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_NOTIFICATION_REMINDER_FINAL_THRESHOLD_DAYS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.reminderCooldownHours}
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_NOTIFICATION_COOLDOWN_REMINDER_HOURS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.successCooldownHours}
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_NOTIFICATION_COOLDOWN_SUCCESS_HOURS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {copy.beam.runtimeFields.defaultCooldownHours}
                              </Label>
                              <Input
                                value={
                                  beamRuntimeForm.BILLING_NOTIFICATION_COOLDOWN_DEFAULT_HOURS
                                }
                                onChange={e =>
                                  setBeamRuntimeForm(prev => ({
                                    ...prev,
                          ...
                          <div className="rounded-xl border border-cyan-100 bg-cyan-50/70 p-4 text-sm text-slate-700">
                            <div className="font-medium text-slate-900">
                              {copy.beam.runtimeSummary}
                            </div>
                            <div className="mt-2">
                              {copy.beam.runtimeValues.savedCards}:{" "}
                              {beamRuntimeForm.BILLING_PHASE2_SAVED_CARDS_ENABLED
                                ? copy.providerSwitch.enabled
                                : copy.providerSwitch.disabled}
                            </div>
                            <div>
                              {copy.beam.runtimeValues.autoRenew}:{" "}
                              {beamRuntimeForm.BILLING_PHASE2_AUTO_RENEW_ENABLED
                                ? copy.providerSwitch.enabled
                                : copy.providerSwitch.disabled}
                            </div>
                            <div>
                              {copy.beam.runtimeValues.dunning}:{" "}
                              {beamRuntimeForm.BILLING_PHASE2_DUNNING_ENABLED
                                ? copy.providerSwitch.enabled
                                : copy.providerSwitch.disabled}
                            </div>
                            <div>
                              {copy.beam.runtimeValues.cardSetup}:{" "}
                              {beamRuntimeForm.BILLING_PHASE2_CARD_SETUP_ENABLED
                                ? copy.providerSwitch.enabled
                                : copy.providerSwitch.disabled}
                            </div>
                            <div>
                              {copy.beam.runtimeValues.supportRecovery}:{" "}
                              {beamRuntimeForm.SUPPORT_RECOVERY_CASES_ENABLED
                                ? copy.providerSwitch.enabled
                                : copy.providerSwitch.disabled}
                            </div>
                          </div>
                    ...
                    <TabsContent value="promptpay_direct">
                      <DashboardCard
                        ...
                        title={
                          isThai
                            ? "ตั้งค่า PromptPay Direct"
                            : "PromptPay Direct Settings"
                        }
                        description={
                          isThai
                            ? "ตั้งค่าบัญชีรับเงิน อัตราแลกเปลี่ยน และการตรวจสอบสลิปแยกจาก Beam"
                            : "Configure the receiving account, FX pricing, and manual slip approval separately from Beam."
                        }
                        bodyClassName="space-y-6"
                      >
                        <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4">
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <div className="font-medium text-slate-900">
                                {isThai
                                  ? "เปิดใช้งาน PromptPay Direct + อนุมัติสลิปด้วยแอดมิน"
                                  : "Enable PromptPay Direct + manual slip approval"}
                              </div>
                              <div className="mt-1 text-xs text-slate-600">
                                {isThai
                                  ? "เป็นช่องทางแยกจาก Stripe และ Beam เครดิตจะเข้าเมื่อแอดมินอนุมัติสลิปเท่านั้น"
                                  : "A separate payment method from Stripe and Beam. Credits are added only after admin approval."}
                              </div>
                            </div>
                            <Switch
                              checked={
                              ...
                              onCheckedChange={checked =>
                                setPromptPayDirectRuntimeForm(prev => ({
                                  ...prev,
                                  PROMPTPAY_DIRECT_ENABLED: checked,
                                }))
                              }
                            />
                          </div>
                          <div className="mt-4 grid gap-4 md:grid-cols-2">
                            <div>
                              <Label>
                                {isThai
                                  ? "บัญชี/PromptPay ID ผู้รับเงิน"
                                  : "PromptPay recipient ID"}
                              </Label>
                              <Input
                                type="password"
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_RECIPIENT_ID
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                                ...
                                placeholder={
                                  billingRuntimeSettingsQuery.data
                                    ?.PROMPTPAY_DIRECT_RECIPIENT_IDConfigured
                                    ? isThai
                                      ? "เว้นว่างเพื่อใช้ค่าที่บันทึกไว้"
                                      : "Leave blank to keep existing"
                                    : isThai
                                      ? "เบอร์ 10 หลัก หรือ ID 13 หลัก"
                              ...
                              {billingRuntimeSettingsQuery.data
                                ...
                                <p className="mt-1 text-xs text-slate-500">
                                  {isThai
                                    ? "มีการบันทึกค่าไว้แล้ว ระบบจะไม่แสดงค่าเต็ม"
                                    : "A value is already configured and is never revealed."}
                                </p>
                              ) : null}
                            </div>
                            <div>
                              <Label>
                                {isThai ? "ประเภทผู้รับเงิน" : "Recipient type"}
                              </Label>
                              <Select
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_RECIPIENT_TYPE
                                }
                                onValueChange={value =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                                    PROMPTPAY_DIRECT_RECIPIENT_TYPE:
                                      value as PromptPayDirectRuntimeForm["PROMPTPAY_DIRECT_RECIPIENT_TYPE"],
                                  }))
                                }
                              >
                                <SelectTrigger>
                                ...
                                <SelectContent>
                                  <SelectItem value="phone">
                                    {isThai ? "เบอร์โทรศัพท์ไทย" : "Thai phone"}
                                  </SelectItem>
                                  ...
                                  <SelectItem value="ewallet">
                                    {isThai ? "E-Wallet ID" : "E-Wallet ID"}
                                  </SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                            <div>
                              <Label>
                                {isThai
                                  ? "ชื่อบัญชีที่แสดงให้ลูกค้า"
                                  : "Account display name"}
                              </Label>
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_ACCOUNT_DISPLAY_NAME
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              ...
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_ORDER_EXPIRY_MINUTES
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {isThai
                                  ? "อายุอัตรา FX สูงสุด (ชั่วโมง)"
                                  : "Max FX rate age (hours)"}
                              </Label>
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_FX_MAX_RATE_AGE_HOURS
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              ...
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_FX_SELL_SPREAD_BPS
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              ...
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_FX_RISK_BUFFER_BPS
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              ...
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_FX_SANITY_MIN_RATE
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              ...
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_FX_SANITY_MAX_RATE
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div>
                              <Label>
                                {isThai
                                  ? "ขนาดสลิปสูงสุด (bytes)"
                                  : "Max slip size (bytes)"}
                              </Label>
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_SLIP_MAX_BYTES
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                            ...
                            <div className="md:col-span-2">
                              <Label>
                                {isThai
                                  ? "ชนิดไฟล์สลิปที่อนุญาต"
                                  : "Allowed slip MIME types"}
                              </Label>
                              <Input
                                value={
                                  promptPayDirectRuntimeForm.PROMPTPAY_DIRECT_SLIP_ALLOWED_TYPES
                                }
                                onChange={e =>
                                  setPromptPayDirectRuntimeForm(prev => ({
                                    ...prev,
                          ...
                          <div className="mt-3 text-xs text-slate-600">
                            {isThai
                              ? "FX source: Frankfurter daily USD/THB · ปัดเป็น 1 THB แล้วสุ่มสตางค์ 00–99 ไม่ซ้ำ"
                              : "FX source: Frankfurter daily USD/THB · round to 1 THB then assign a unique random satang 00–99."}
                          </div>
                        </div>
                        ...
                        <div className="rounded-xl border border-cyan-100 bg-cyan-50/70 p-4 text-sm text-slate-700">
                          <div className="font-medium text-slate-900">
                            {isThai
                              ? "สถานะ PromptPay Direct"
                              : "PromptPay Direct status"}
                          </div>
                          <div className="mt-2">
                          ...
                          <div>
                            {isThai ? "บัญชีรับเงิน" : "Receiving account"}:{" "}
                            {billingRuntimeSettingsQuery.data
                              ?.PROMPTPAY_DIRECT_RECIPIENT_IDConfigured
                              ? isThai
                                ? "ตั้งค่าแล้ว"
                          ...
                          <div>
                            {isThai ? "อัตราแลกเปลี่ยน" : "FX provider"}:
                            Frankfurter USD/THB
                          </div>
                        </div>
                        ...
                        <div className="flex gap-3 pt-4 border-t">
                          <Button
                            onClick={handleSavePromptPayDirectRuntime}
                            disabled={updateBillingProviderMutation.isPending}
                          >
                            {updateBillingProviderMutation.isPending ? (
                              <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                            ...
                            {isThai
                              ? "บันทึก PromptPay Direct"
                              : "Save PromptPay Direct"}
                          </Button>
                          <Button
                            variant="outline"
                            onClick={() => setLocation("/admin/billing")}
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          Used for Sign Up / Sign In with Google. Recommended:{" "}
                          <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded text-xs font-mono select-all">
                            https://smartaihub.app/auth/callback/google
                          </code>
                        </p>
                      </div>
                      <div>
                        <Label htmlFor="googleDriveRedirectUri">
                        ...
                        <Input
                          ...
                          onChange={e =>
                            setOauthForm(prev => ({
                              ...prev,
                              googleDriveRedirectUri: e.target.value,
                            }))
                          }
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          Used when users connect their Google Drive.
                          Recommended:{" "}
                          <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded text-xs font-mono select-all">
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      {[
                        ...
                        },
                        {
                          ...
                          ready: false,
                          description:
                            "Not supported for Google social-login accounts. Keep this disabled and instruct users to use browser sign-in.",
                        },
                      ].map(item => (
                        <div
                          key={item.title}
                    ...
                    <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                      ...
                      <ul className="mt-2 list-disc space-y-1 pl-4 text-xs text-amber-800">
                        <li>
                          Web sign-in and desktop browser sign-in share the same
                          Google Client ID and Client Secret.
                        </li>
                        <li>
                          Desktop direct login must stay disabled for Google
                          accounts. Tell users to choose{" "}
                          <strong>Sign in via browser</strong>.
                        </li>
                        <li>
                          {googleDriveReady
                            ? "Google Drive redirect is configured for Drive/Docs integration."
                            : "Google Drive features stay disabled until the Google Drive Redirect URI is also configured."}
                        </li>
                        <li>
                          If your OAuth consent screen is still in Testing, add
                          every admin/test user email before validating the
                          flow.
                        </li>
                      </ul>
                    </div>
                        ...
                        <div className="text-sm text-blue-900 dark:text-blue-100 space-y-4">
                          <div className="p-3 rounded-md bg-blue-100/50 dark:bg-blue-900/30">
                            ...
                            <ul className="mt-1 list-disc space-y-1 pl-4 text-blue-700 dark:text-blue-300">
                              <li>
                                <strong>Web sign-in</strong> for Google login on
                                the standard login page.
                              </li>
                              <li>
                                <strong>Desktop browser sign-in</strong> because
                                desktop authorization reuses the same web Google
                                flow.
                              </li>
                              <li>
                                <strong>
                                ...
                                when the Drive redirect URI is also configured.
                              </li>
                            </ul>
                          </div>
                          ...
                          <div>
                            ...
                            <p className="text-blue-700 dark:text-blue-300 mt-1">
                              ...
                              <strong>APIs & Services &gt; Library</strong> and
                              enable the APIs you plan to use:
                            </p>
                            <ul className="list-disc list-inside mt-1 space-y-0.5 text-blue-700 dark:text-blue-300 ml-2">
                              <li>
                                <strong>Google Drive API</strong> for Drive
                                integration
                              </li>
                              <li>
                                <strong>Google Docs API</strong> for document
                                editing
                              </li>
                              <li>
                                <strong>Google Sheets API</strong> for
                                spreadsheet editing
                              </li>
                              <li>
                                <strong>Google Slides API</strong> for
                                presentation editing
                              </li>
                            </ul>
                            <p className="text-blue-600 dark:text-blue-400 text-xs mt-1">
                              For login-only setups, the
                              Drive/Docs/Sheets/Slides APIs can be skipped.
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <ul className="list-disc list-inside mt-1 space-y-0.5 text-blue-700 dark:text-blue-300 ml-2">
                              <li>
                                User Type: <strong>External</strong> (or
                                Internal for Google Workspace)
                              </li>
                              <li>
                                Fill in App name, User support email, and
                                Developer contact
                              </li>
                              <li>
                                Add scopes:{" "}
                              ...
                              <li>
                                Add test users if in Testing status. Desktop
                                browser sign-in uses the same consent screen as
                                the web flow.
                              </li>
                            </ul>
                          </div>
                          ...
                          <div>
                            ...
                            <ul className="list-disc list-inside mt-1 space-y-0.5 text-blue-700 dark:text-blue-300 ml-2">
                              ...
                              <li>
                                Authorized JavaScript origins: your production
                                web origin, for example{" "}
                                <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded text-xs font-mono">
                                  https://smartaihub.app
                                </code>
                              </li>
                              <li>
                                Authorized redirect URIs &mdash; add these exact
                                URLs:
                                <div className="ml-4 mt-1 space-y-1">
                                  <div className="flex items-start gap-2">
                                    <code className="block bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded text-xs font-mono">
                                      {oauthForm.googleRedirectUri ||
                                        "https://smartaihub.app/auth/callback/google"}
                                    </code>
                                    <span className="text-xs text-blue-600 dark:text-blue-400 mt-0.5 whitespace-nowrap">
                                      &larr; web sign-in + desktop browser
                                      sign-in
                                    </span>
                                  </div>
                                  <div className="flex items-start gap-2">
                                    <code className="block bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded text-xs font-mono">
                                      {oauthForm.googleDriveRedirectUri ||
                                        "https://smartaihub.app/auth/callback/google-drive"}
                                    </code>
                                    <span className="text-xs text-blue-600 dark:text-blue-400 mt-0.5 whitespace-nowrap">
                                      &larr; Google Drive integration
                                    </span>
                                  </div>
                                </div>
                              </li>
                          ...
                          <div>
                            <p className="font-semibold">
                              Step 5: Copy credentials here
                            </p>
                            <p className="text-blue-700 dark:text-blue-300 mt-1">
                              Copy the <strong>Client ID</strong> and{" "}
                              <strong>Client Secret</strong> from Google Cloud
                              Console and paste them in the fields above. Then
                              click <strong>Save OAuth Settings</strong>.
                            </p>
                          </div>

                          <div>
                            <p className="font-semibold">
                              Step 6: Validate both web and desktop browser
                              flows
                            </p>
                            <ul className="list-disc list-inside mt-1 space-y-0.5 text-blue-700 dark:text-blue-300 ml-2">
                              <li>
                                Click <strong>Test Google Connection</strong>{" "}
                                below after saving.
                              </li>
                              <li>
                                Open the web login page and confirm the Google
                                button is enabled.
                              </li>
                              <li>
                                If you support desktop users, confirm the
                                desktop app uses{" "}
                                <strong>Sign in via browser</strong> for Google
                                accounts.
                              </li>
                              <li>
                                If Google returns{" "}
                                ...
                                , compare the URI in Google Cloud Console with
                                the exact URI shown in this page.
                              </li>
                            </ul>
                          </div>
                          ...
                          <div className="pt-2 border-t border-blue-200 dark:border-blue-700 space-y-1">
                            <p className="text-blue-600 dark:text-blue-400 text-xs flex items-center gap-1">
                              ...
                              Client Secret is encrypted before storage. It will
                              not be shown after saving.
                            </p>
                            <p className="text-blue-600 dark:text-blue-400 text-xs flex items-center gap-1">
                              <AlertCircle className="w-3 h-3" />
                              Desktop direct login should remain disabled for
                              Google social accounts; only the
                              browser/device-code path is supported.
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setOauthForm(prev => ({
                            ...prev,
                            githubRedirectUri: e.target.value,
                          }))
                        }
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Must match the callback URL in GitHub OAuth App
                  ...
                  <div className="space-y-4 pt-4 border-t">
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        UUID format from Azure App Registration
                      </p>
                    </div>
                    <div>
                      <Label htmlFor="microsoftClientSecret">
                        ...
                        {microsoftSecretConfigured && (
                          <span className="ml-2 text-xs text-green-600 bg-green-50 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded">
                            Configured
                          </span>
                        )}
                      </Label>
                    ...
                    <div>
                      <Label htmlFor="microsoftOneDriveRedirectUri">
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setOauthForm(prev => ({
                            ...prev,
                            microsoftOneDriveRedirectUri: e.target.value,
                          }))
                        }
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Must match the Redirect URI in Azure. Recommended:{" "}
                        <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded text-xs font-mono select-all">
                    ...
                    <details className="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3">
                      ...
                      <ol className="mt-2 space-y-1.5 pl-5 list-decimal">
                        <li>
                          Go to <strong>Azure Portal</strong> &gt;{" "}
                        ...
                        <li>
                          Redirect URI: select <strong>Web</strong>, enter:{" "}
                        ...
                        <li>
                          ...
                          <strong>New client secret</strong> &gt; copy the Value
                          (not the ID)
                        </li>
                        <li>
                        ...
                        <li>
                          ...
                          the Overview page and paste above
                        </li>
                      </ol>
                      <p className="mt-2 text-xs text-gray-500">
                        Azure App Registration and Microsoft Graph API are free
                        — no additional cost.
                      </p>
                    </details>
                  </div>
                  ...
                  <div className="flex gap-3 pt-4 border-t">
                    ...
                    >
                      ...
                      {isThai
                        ? "บันทึกการตั้งค่า OAuth ทั้งหมด"
                        : "Save all OAuth settings"}
                    </Button>
                  </div>
                </DashboardCard>
              </TabsContent>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setSmtpForm(p => ({ ...p, fromName: e.target.value }))
                        }
                        className="mt-1"
                      />
                    </div>
                  ...
                  <div className="rounded-lg border-2 border-amber-400 bg-amber-50 p-4 text-sm space-y-2">
                    ...
                    <p className="text-amber-700">
                      Gmail does not support regular passwords. You must create
                      an <strong>App Password</strong> instead.
                    </p>
                    <a
                  ...
                  <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4 text-sm space-y-3">
                    <p className="font-semibold text-blue-800">
                      Gmail SMTP Settings (smtp.gmail.com)
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="rounded-md border border-blue-200 bg-white p-3">
                        ...
                        <ul className="text-blue-700 space-y-0.5 text-xs">
                          ...
                          <li>
                            SSL/TLS: <strong>Off</strong> (unchecked)
                          </li>
                        </ul>
                      </div>
                      <div className="rounded-md border border-blue-200 bg-white p-3">
                        ...
                        <ul className="text-blue-700 space-y-0.5 text-xs">
                          ...
                          <li>
                            SSL/TLS: <strong>On</strong> (checked)
                          </li>
                        </ul>
                      </div>
                    </div>
                    <p className="text-blue-600 text-xs">
                      Limits: Free Gmail ~500 emails/day • Google Workspace
                      ~2,000/day
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={e =>
                          setSmsForm(p => ({
                            ...p,
                            provider: e.target.value as "twilio" | "vonage",
                          }))
                        }
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={
                          smsSettings?.configured
                            ? copy.sms.secretPlaceholderKeep
                            : copy.sms.secretPlaceholderNew
                        }
                        value={smsForm.authToken}
                        onChange={e =>
                          setSmsForm(p => ({ ...p, authToken: e.target.value }))
                        }
                        className="mt-1"
                      />
                    </div>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          testSmsMutation.mutate({
                            testNumber: smsForm.testNumber,
                          })
                        }
                        disabled={
                          testSmsMutation.isPending || !smsForm.testNumber
                        }
                      >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        <p className="text-sm font-semibold text-amber-900">
                          {copy.sms.recommendedTitle}
                        </p>
                        <p className="text-sm text-amber-800">
                          {copy.sms.recommendedBody}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4 text-sm space-y-4">
                    <div className="space-y-2">
                      <p className="font-semibold text-blue-800">
                        {smsForm.provider === "twilio"
                          ? copy.sms.twilioGuideTitle
                          : copy.sms.vonageGuideTitle}
                      </p>
                      {smsForm.provider === "twilio" && (
                        <div className="space-y-3">
                          <p className="text-sm text-blue-800">
                            {copy.sms.twilioGuideIntro}
                          </p>
                          <a
              ...
              <TabsContent value="telegram">
                ...
                >
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        Master switch for all Telegram alert delivery
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      The bot's @username (without @) — used to generate deep
                      links for account linking
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setTelegramForm(p => ({ ...p, appUrl: e.target.value }))
                      }
                      className="mt-1"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Base URL for "View in SmartAIHub" inline buttons in
                      notifications
                    </p>
                  </div>
                  ...
                  <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4 text-sm space-y-3">
                    ...
                    <ol className="text-blue-700 space-y-1.5 text-xs list-decimal pl-4">
                      ...
                      <li>
                        Click "Test Connection" to verify the bot is reachable
                      </li>
                      <li>
                        Click "Register Webhook" to enable the bot to receive
                        verification requests
                      </li>
                      <li>
                        Users can then link their Telegram accounts from
                  ...
                  {webhookSecretConfigured && (
                    <div className="rounded-lg border border-green-200 bg-green-50/50 p-3 text-sm flex items-center gap-2">
                      ...
                      <span className="text-green-700">
                        Webhook secret is configured and secured
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex gap-3 justify-end border-t pt-4">
                    <Button
                      ...
                      disabled={
                        testTelegramMutation.isPending || !botTokenConfigured
                      }
                    >
                    ...
                    <Button
                      ...
                      disabled={
                        registerWebhookMutation.isPending || !botTokenConfigured
                      }
                    >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="flex gap-4">
                        <label
                          className={`flex-1 p-4 rounded-xl border-2 cursor-pointer transition-all ${regForm.registrationMode === "open" ? "border-blue-500 bg-blue-50/50" : "border-gray-200 hover:border-gray-300"}`}
                        >
                          <input
                            type="radio"
                            name="regMode"
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            Anyone can register (invite code optional)
                          </p>
                        </label>
                        <label
                          className={`flex-1 p-4 rounded-xl border-2 cursor-pointer transition-all ${regForm.registrationMode === "invite_only" ? "border-blue-500 bg-blue-50/50" : "border-gray-200 hover:border-gray-300"}`}
                        >
                          <input
                            type="radio"
                            name="regMode"
                    ...
                    <div>
                      ...
                      <div className="flex gap-4">
                        {(["email", "google", "github"] as const).map(
                          method => (
                            <label
                              key={method}
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={regForm.allowedAuthMethods.includes(
                                  method
                                )}
                                onChange={e => {
                                  const newMethods = e.target.checked
                                    ? [...regForm.allowedAuthMethods, method]
                                    : regForm.allowedAuthMethods.filter(
                                        m => m !== method
                                      );
                              ...
                              <span className="text-sm capitalize">
                                {method === "email" ? "Email/Password" : method}
                              </span>
                            </label>
                          )
                        )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <Label htmlFor="signupBonus">
                            Signup Bonus Credits (New Users)
                          </Label>
                          <Input
                            id="signupBonus"
                            type="number"
                        ...
                        <div>
                          <Label htmlFor="firstUserBonus">
                            First User (Admin) Bonus Credits
                          </Label>
                          <Input
                            id="firstUserBonus"
                            type="number"
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        <input
                          ...
                          onChange={e =>
                            setRegForm(prev => ({
                              ...prev,
                              userInviteEnabled: e.target.checked,
                            }))
                          }
                          className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        />
                        <div>
                          <Label htmlFor="userInviteEnabled">
                            Allow users to share invite codes
                          </Label>
                      ...
                      {regForm.userInviteEnabled && (
                        <div className="ml-7">
                          <Label htmlFor="referralBonus">
                            Referral Bonus Credits (for inviter)
                          </Label>
                          <Input
                            id="referralBonus"
                            type="number"
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Credits given to the inviter when someone registers
                            with their code
                          </p>
                        </div>
                      )}
                    ...
                    <div>
                      <h4 className="font-semibold text-sm text-gray-900 mb-3">
                        Free-credit inactivity policy
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <Label htmlFor="inactiveDays">
                            Reset free credits and disable after days of no
                            usage
                          </Label>
                          <Input
                            id="inactiveDays"
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Default is 15 days. Applies to users who received
                            signup or invite free credits; purchasing credits
                            cancels the policy.
                          </p>
                        </div>
                        <div>
                          <Label htmlFor="maxDeviceReg">
                            Max registrations per device
                          </Label>
                          <Input
                            id="maxDeviceReg"
                            type="number"
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            0 = disabled. Block registration when same device
                            exceeds limit.
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        onChange={e =>
                          setRegForm(prev => ({
                            ...prev,
                            autoAssignTenant: e.target.checked,
                          }))
                        }
                        className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <div>
                        <Label htmlFor="autoTenant">
                          Auto-assign tenant by domain
                        </Label>
                        <p className="text-xs text-gray-500">
                          Automatically assign users to the tenant matching
                          their registration domain
                        </p>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        Users can enable TOTP authenticator from their Security
                        settings
                      </div>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        Require all users to set up 2FA before accessing the
                        platform
                      </div>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setTwoFaForm(p => ({ ...p, issuer: e.target.value }))
                      }
                      placeholder="SmartAIHub"
                      className="mt-1"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      This name appears in Google Authenticator, Authy, etc. as
                      the account label.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setTwoFaForm(p => ({
                          ...p,
                          backupCodesCount: parseInt(e.target.value) || 10,
                        }))
                      }
                      className="mt-1 w-32"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      One-time recovery codes generated when user enables 2FA
                      (5-50)
                    </p>
                  </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-3">
                    {[
                      ...
                      },
                      {
                        ...
                        ready: false,
                        description:
                          "Not supported for 2FA accounts. Users must choose Sign in via browser in the desktop app.",
                      },
                    ].map(item => (
                      <div
                        key={item.title}
                  ...
                  <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4 text-sm space-y-2">
                    ...
                    <ul className="text-blue-700 space-y-1 text-xs list-disc pl-4">
                      ...
                      <li>
                        They scan a QR code with Google Authenticator, Authy, or
                        any TOTP app
                      </li>
                      <li>
                        On login, after password, they must enter a 6-digit TOTP
                        code
                      </li>
                      <li>
                        Recovery codes (one-time use) let users sign in if they
                        lose their authenticator
                      </li>
                      <li>
                        Users with verified backup email or phone can also reset
                        2FA via those channels
                      </li>
                      <li>
                        TOTP uses HMAC-SHA1 with 30-second time window (RFC 6238
                        standard)
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                    ...
                    <ol className="mt-2 list-decimal space-y-1 pl-4 text-xs text-amber-800">
                      <li>
                        Turn on <strong>Allow 2FA</strong> first and test with
                        one internal account on the web login page.
                      </li>
                      <li>
                        Confirm recovery email or SMS is configured before you
                        enforce 2FA broadly.
                      </li>
                      <li>
                        If you use Google sign-in, test an account end-to-end to
                        confirm the OAuth flow redirects into the 2FA prompt
                        correctly.
                      </li>
                      <li>
                        Tell desktop users that{" "}
                        <strong>Sign in via browser</strong> is the required
                        path for any account protected by 2FA.
                      </li>
                      <li>
                        Only then enable{" "}
              ...
              <TabsContent value="stt">
                ...
                >
                  {(sttTemplates || []).map(tpl => {
                    const configured = sttProviders?.find(
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              <Mic
                                className={`w-5 h-5 ${configured?.hasApiKey ? "text-green-600" : "text-gray-400"}`}
                              />
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <div className="relative mt-1">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    setShowSttApiKey(!showSttApiKey)
                                  }
                                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                                >
                                  {showSttApiKey ? (
                            ...
                            {configured?.hasApiKey && (
                              <>
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                    sttTestMutation.mutate({
                                      id: configured.id,
                                    })
                                  }
                                  disabled={sttTestMutation.isPending}
                                >
                                  {sttTestMutation.isPending ? (
                                    <Loader2 className="w-4 h-4 animate-spin" />
                                  ) : (
                  ...
                  <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4 text-sm space-y-2">
                    ...
                    <ul className="text-blue-700 space-y-1 text-xs list-disc pl-4">
                      ...
                      <li>
                        Audio is sent to the first enabled provider for
                        transcription
                      </li>
                      <li>
                        Credits are deducted based on the provider's cost per
                        minute
                      </li>
                      <li>
                        Groq offers free Whisper transcription — recommended to
                        start
                      </li>
                    </ul>
                  </div>
              ...
              <TabsContent value="ai">
                ...
                >
                  <div className="flex items-start justify-between gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <div className="space-y-1">
                      <label className="text-sm font-medium">
                        Allow users to manage their own LLM API keys
                      </label>
                      <p className="text-xs text-muted-foreground">
                        Default is disabled. When off, users cannot add or
                        update personal provider keys in Settings.
                      </p>
                    </div>
                    <Switch
                      ...
                      onCheckedChange={checked => {
                        ...
                        updateAiPolicyMutation.mutate(
                          ...
                          {
                            onError: () =>
                              setAllowUserOwnLlmApiKeys(previousValue),
                          }
                        );
                      }}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">
                      Summary / Consolidation Model
                    </label>
                    <p className="text-xs text-muted-foreground">
                      Model used for auto-summarizing conversation history and
                      consolidating memory. Use a cheaper model to save credits.
                    </p>
              ...
              <TabsContent value="marketplace_capture">
                ...
                >
                  <div className="space-y-2">
                    <Label htmlFor="marketplace-image-index-hosts">
                      Image index allowed hosts
                    </Label>
                    <Textarea
                      id="marketplace-image-index-hosts"
                      value={marketplaceImageIndexAllowedHosts}
                      onChange={event =>
                        setMarketplaceImageIndexAllowedHosts(event.target.value)
                      }
                      placeholder="cdn.example.com&#10;storage.example.com"
                      className="min-h-28 font-mono text-sm"
                    />
                    <p className="text-xs leading-5 text-slate-500">
                      One host per line or comma-separated. Production visual
                      image indexing is fail-closed until at least one host is
                      saved here.
                    </p>
                  </div>
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                    Only image URLs from these HTTPS hosts, including their
                    subdomains, can be fetched for Marketplace Capture visual
                    search indexing.
                  </div>
                  <div className="flex justify-end">
                    <Button
                      onClick={() => {
                        ...
                        updateMarketplaceCaptureSettingMutation.mutate({
                          ...
                          description:
                            "Allowed HTTPS hosts for Marketplace Capture visual image indexing",
                        });
                      }}
                      disabled={
              ...
              <TabsContent value="finance_rules">
                ...
                >
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-600">
                        Pin important merchants from the system merchant list,
                        then let Finance suggest them first.
                      </p>
                    </div>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div className="text-sm font-semibold text-slate-900">
                        Slip mapping presets
                      </div>
                      <p className="mt-1 text-xs leading-5 text-slate-600">
                        Reuse rules for common income, expense, and transfer
                        slips after OCR or LLM parsing.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      className="gap-2"
                      onClick={() => setLocation("/admin/finance-rules")}
                    >
                  ...
                  <div className="rounded-xl border border-dashed border-slate-300 bg-white/80 p-4 text-xs text-slate-600">
                    OCR routing stays on the Document OCR tab. This page is just
                    for merchant pin search and slip mapping rules.
                  </div>
                </DashboardCard>
              </TabsContent>
                ...
                >
                  ...
                  >
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-blue-700 dark:text-blue-300 mt-1">
                        Vector databases store document embeddings for semantic
                        search and RAG. Choose <strong>ChromaDB</strong> for
                        development (zero-config, local storage),{" "}
                        <strong>pgvector</strong> for production (scalable,
                        hybrid search with PostgreSQL), or{" "}
                        <strong>Cloudflare Vectorize</strong> for edge-native
                        global deployment.
                      </p>
                    </div>
                  </DashboardCard>
                  ...
                  {vectorDbStats && !vectorDbStats.error && (
                    <div className="grid gap-4 md:grid-cols-3">
                      ...
                      {vectorDbStats.provider === "chromadb" &&
                        vectorDbStats.totalCollections !== undefined && (
                          <DashboardKpiCard
                            icon={HardDrive}
                            label="Collections"
                      ...
                      {vectorDbStats.provider === "pgvector" &&
                        vectorDbStats.totalDocuments !== undefined && (
                          <DashboardKpiCard
                            icon={HardDrive}
                            label="Indexed Items"
                            ...
                            subLabel={
                              <span className="text-xs text-slate-500">
                                {vectorDbStats.totalVectors?.toLocaleString?.() ??
                                  "—"}{" "}
                                vectors
                                {vectorDbStats.activeItems !== undefined
                                  ? ` • ${vectorDbStats.activeItems.toLocaleString()} active items`
                                  : ""}
                              </span>
                            }
                          />
                      ...
                      <DashboardKpiCard
                        ...
                        value={
                          <span className="text-xs font-mono truncate">
                            {vectorDbStats.storageLocation ||
                              vectorDbStats.storageType ||
                              "N/A"}
                          </span>
                        }
                        subLabel={
                    ...
                    <div className="space-y-4">
                      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <DashboardKpiCard
                          ...
                          subLabel={
                            <span className="text-xs text-slate-500">
                              Current read provider stays authoritative until
                              cutover finishes.
                            </span>
                          }
                        />
                        ...
                        <DashboardKpiCard
                          ...
                          value={
                            <Badge
                              variant={
                                normalizedVectorDbHealth.connection_health
                                  .healthy
                                  ? "default"
                                  : "destructive"
                              }
                        ...
                        <DashboardKpiCard
                          ...
                          value={
                            <span>
                              Queue lag{" "}
                              {normalizedVectorDbHealth.queue_status.lag_minutes.toFixed(
                                1
                              )}{" "}
                              min
                            </span>
                          ...
                          subLabel={
                            <span className="text-xs text-slate-500">
                              Search p95{" "}
                              {Math.round(
                                normalizedVectorDbHealth.latency_status
                                  .current_p95_ms
                              )}{" "}
                              ms
                            </span>
                      ...
                      {normalizedVectorDbHealth.recent_failures.length > 0 && (
                        ...
                        >
                          {normalizedVectorDbHealth.recent_failures
                            .slice(0, 3)
                            .map(
                              (
                                failure: (typeof DEFAULT_VECTOR_DB_HEALTH.recent_failures)[number]
                              ) => (
                                <div
                                  key={failure.job_id}
                                ...
                                >
                                  <div className="font-medium">
                                    Item {failure.library_item_id} • Job{" "}
                                    {failure.job_id}
                                  </div>
                        ...
                        <SelectItem value="pgvector">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">
                                PostgreSQL extension (Production)
                              </div>
                            </div>
                          </div>
                        </SelectItem>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-md text-xs">
                      ...
                      <div>
                        <strong>
                          Dimension: {vectorDbForm.embeddingDimension}
                        </strong>{" "}
                        •{" "}
                  ...
                  {vectorDbForm.provider === "chromadb" && (
                    <div className="space-y-3 border-t pt-4">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="chromaPersistDir">
                        ...
                        <Input
                          ...
                          onChange={e =>
                            setVectorDbForm({
                              ...vectorDbForm,
                              chromaPersistDir: e.target.value,
                            })
                          }
                        ...
                        />
                        <p className="text-xs text-muted-foreground">
                          Path where ChromaDB stores collections. Use absolute
                          path or ~/ for home directory.
                        </p>
                      </div>
                  ...
                  {vectorDbForm.provider === "pgvector" && (
                    <div className="space-y-4 border-t pt-4">
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          <Label htmlFor="pgvectorDatabase">
                            Database Name
                          </Label>
                          <Input
                            id="pgvectorDatabase"
                            ...
                            onChange={e =>
                              setVectorDbForm({
                                ...vectorDbForm,
                                pgvectorDatabase: e.target.value,
                              })
                            }
                            placeholder="smartaihub"
                            className="font-mono text-sm"
                        ...
                        <div className="relative max-w-md">
                          <Input
                            ...
                            onChange={e =>
                              setVectorDbForm({
                                ...vectorDbForm,
                                pgvectorPassword: e.target.value,
                              })
                            }
                            placeholder={
                              pgvectorPasswordConfigured
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setShowPgvectorPassword(!showPgvectorPassword)
                            }
                            className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700"
                          >
                            {showPgvectorPassword ? (
                    ...
                    <div className="space-y-4 border-t pt-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={e =>
                              setVectorDbForm({
                                ...vectorDbForm,
                                vectorizeAccountId: e.target.value,
                              })
                            }
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={e =>
                              setVectorDbForm({
                                ...vectorDbForm,
                                vectorizeIndexName: e.target.value,
                              })
                            }
                        ...
                        <div className="relative max-w-md">
                          <Input
                            ...
                            onChange={e =>
                              setVectorDbForm({
                                ...vectorDbForm,
                                vectorizeApiToken: e.target.value,
                              })
                            }
                            placeholder={
                              vectorizeApiTokenConfigured
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setShowVectorizeApiToken(!showVectorizeApiToken)
                            }
                            className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700"
                          >
                            {showVectorizeApiToken ? (
                              <EyeOff className="h-4 w-4" />
                            ) : (
                              <Eye className="h-4 w-4" />
                            )}
                      ...
                      >
                        ...
                        <ol className="list-decimal ml-4 space-y-1 text-green-700 dark:text-green-300">
                          ...
                          <li>
                            Click "Create Index" — set name, dimension (must
                            match your embedding model), and metric (cosine)
                          </li>
                          <li>
                            Copy your Account ID from the dashboard URL or
                            Overview page
                          </li>
                          <li>
                            Go to My Profile &gt; API Tokens &gt; Create Token
                            with "Vectorize: Edit" permission
                          </li>
                          <li>
                            Paste the Account ID, Index Name, and API Token
                            above
                          </li>
                        </ol>
                      </DashboardCard>
                  ...
                  <div className="space-y-3 border-t pt-4">
                            ...
                            <div className="text-sm">
                              ...
                              {reindexExpectedJobs > 0 && (
                                <span className="text-blue-600 ml-2">
                                  ({reindexCompletedJobs}/{reindexExpectedJobs}{" "}
                                  completed)
                                </span>
                              )}
                            </div>
                    ...
                    <Button
                      ...
                      disabled={
                        isReindexing || triggerReindexMutation.isPending
                      }
                    >
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="space-y-2">
                        ...
                        <p>
                          The process runs in the background and may take
                          several minutes depending on the number of documents.
                          You can safely navigate away — the reindex will
                          continue.
                        </p>
                      </AlertDialogDescription>
                    </AlertDialogHeader>
...
function buildDefaultOverrides(): MenuOverrideEntry[] {
  return defaultMenuItems.map(item => {
    const entry: MenuOverrideEntry = {
      ...
      web_domain_admin:
        item.platforms.includes("web") &&
        (!item.roles || item.roles.includes("domain_admin")),
      web_user:
        item.platforms.includes("web") &&
        (!item.roles || item.roles.includes("user")),
      desktop_admin:
        item.platforms.includes("desktop") &&
        (!item.roles || item.roles.includes("admin")),
      desktop_domain_admin:
        item.platforms.includes("desktop") &&
        (!item.roles || item.roles.includes("domain_admin")),
      desktop_user:
        item.platforms.includes("desktop") &&
...
function MenuOverridesPanel() {
  const { data: savedOverrides, isLoading } =
    trpc.systemSettings.getMenuOverrides.useQuery();
  ...
  useEffect(() => {
    ...
    if (savedOverrides && savedOverrides.length > 0) {
      const map = new Map(
        savedOverrides.map((o: MenuOverrideEntry) => [o.menuItemId, o])
      );
  ...
  if (isLoading) {
    return (
      <DashboardCard
        ...
        bodyClassName="py-12 flex justify-center"
      >
            ...
            <tr className="border-b bg-gray-50/50">
              ...
              {PLATFORMS.map(p =>
                ROLES.map(r => (
                  <th
                    key={`${p}_${r.key}`}
                    className="text-center px-2 py-3 font-medium text-gray-500 text-xs"
                  >
              ...
              return (
                <Fragment key={group.key}>
                  <tr>
                    <td
                      colSpan={7}
                      className="px-4 py-2 bg-gray-100/60 text-xs font-semibold text-gray-600 uppercase tracking-wider"
                    >
                      {group.label}
                    </td>
                  </tr>
                    ...
                    return (
                      ...
                      >
                        ...
                        {PLATFORMS.map(p =>
                          ROLES.map(r => {
                            const col =
                              `${p}_${r.key}` as keyof MenuOverrideEntry;

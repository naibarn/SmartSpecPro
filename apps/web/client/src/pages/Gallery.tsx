...
import type { MediaAspectRatio as AspectRatio } from "@/lib/mediaAspectRatio";
...
/** Keeps broken remote media from rendering the browser's default broken-image UI. */
function MediaFallback({
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`${type} preview unavailable`}
    >
...
function GalleryImage({
  ...
  if (hasFailed) {
    return <MediaFallback type="image" className={className} />;
  }
...
function VideoThumbnailCard({
  ...
  if (hasFailed) {
    return <MediaFallback type="video" className={className} />;
  }
...
function GalleryThumbnail({
  ...
  const storedAspectRatio =
    parseMediaAspectRatio(item.aspectRatio) ??
    (item.type === "video" ? "16:9" : "1:1");
  const [aspectRatio, setAspectRatio] =
    useState<AspectRatio>(storedAspectRatio);

  useEffect(() => {
    setAspectRatio(storedAspectRatio);
  }, [item.id, storedAspectRatio]);

  const handleDimensions = useCallback((width: number, height: number) => {
    const detectedAspectRatio = inferMediaAspectRatio(width, height);
  ...
  const hasImageThumbnail =
    isGalleryImageSource(item.thumbnailKey) ||
    isGalleryImageSource(item.thumbnailUrl);

  return (
    <div
      className={`relative ${aspectRatioStyles[aspectRatio]} bg-muted overflow-hidden`}
    >
      {item.type === "video" ? (
        hasImageThumbnail ? (
          <GalleryImage
            src={getGalleryMediaUrl(item, "thumbnail")}
            alt={item.title}
        ...
        <GalleryImage
          src={getGalleryMediaUrl(item, "thumbnail")}
          fallbackSrc={getGalleryMediaUrl(item, "file")}
          alt={item.title}
          className={mediaClassName}
          onDimensions={handleDimensions}
        />
      )}
      {item.type === "video" && hasImageThumbnail && (
        <video
          src={getGalleryMediaUrl(item, "file")}
          preload="metadata"
          muted
          playsInline
          aria-hidden="true"
...
function LightboxMedia({ item }: { item: GalleryItem }) {
  ...
  const [hasFailed, setHasFailed] = useState(!mediaUrl);

  useEffect(() => {
    setHasFailed(!mediaUrl);
  }, [mediaUrl]);
...
export default function Gallery() {
  ...
  const structuredData = {
    ...
    description:
      "Explore AI-generated images, videos, and website demos created with SmartAIHub",
    ...
    mainEntity: {
      ...
      itemListElement: sortedItems.slice(0, 10).map((item, index) => ({
        ...
        item: {
          ...
          thumbnailUrl: getGalleryMediaUrl(item, "thumbnail"),
          ...
          <div className="container relative mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <p className="text-lg text-muted-foreground">
                Discover stunning AI-generated images, videos, and website demos
                created with SmartAIHub.
              </p>
            </motion.div>
          </div>
        </section>
                ...
                <AnimatePresence mode="popLayout">
                  {sortedItems.map(item => (
                    ...
                    >
                      ...
                      >
                        {/* Thumbnail: use stored ratio first, then correct it from actual media dimensions. */}
                        <GalleryThumbnail item={item}>
                          ...
                          {isAdmin && (
                            <Button
                              size="icon"
                              variant="secondary"
                              className="absolute right-3 top-3 z-10 h-8 w-8 bg-red-500/90 text-white hover:bg-red-600"
            ...
            {selectedItem && (
              <>
                ...
                <div className="relative flex-1 min-h-0 bg-black flex items-center justify-center overflow-hidden">
                  ...
                  <LightboxMedia item={selectedItem} />
                </div>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      ...
                      {isAdmin && (
                        <Button
                          ...
                          disabled={deleteMutation.isPending}
                          title="Delete gallery item"
                          aria-label={`Delete gallery item: ${selectedItem.title}`}
                        >
                          {deleteMutation.isPending ? (

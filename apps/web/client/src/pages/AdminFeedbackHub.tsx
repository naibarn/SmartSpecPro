...
export default function AdminFeedbackHub() {
  ...
  const addCommentMutation = trpc.feedback.addComment.useMutation({
    onSuccess: () => {
      setCommentText("");
      ticketDetailQuery.refetch();
      toast.success(isInternal ? "Internal note added" : "Reply sent — user will be notified");
    },
    onError: (err) => {
  ...
  const statusColor: Record<string, string> = {
    ...
    triaged: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400",
    ...
    duplicate: "bg-gray-100 text-gray-500 dark:bg-gray-800/50 dark:text-gray-500",
    deferred: "bg-gray-100 text-gray-600 dark:bg-gray-800/50 dark:text-gray-500",
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Manage user feedback & reports</p>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex gap-1.5 flex-wrap justify-end">
                    {(["triaged", "in_progress", "resolved", "closed"] as const).map((s) => (
                      <Button
                        ...
                        onClick={() => updateStatusMutation.mutate({ ticketId: selectedTicketId, status: s })}
                      >
                        {statusLabel[s]}
                      </Button>
                    ))}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {detail.expectedBehavior && (
                        <div className="bg-green-50 rounded-lg p-4 border border-green-100">
                          <h3 className="text-sm font-medium mb-2 text-green-800">Expected</h3>
                      ...
                      {detail.actualBehavior && (
                        <div className="bg-red-50 rounded-lg p-4 border border-red-100">
                          <h3 className="text-sm font-medium mb-2 text-red-800">Actual</h3>
                    ...
                    <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-100">
                      ...
                      {(detail.autoCategory || detail.autoPriority) && (
                        <div className="flex gap-2 mt-2">
                          {detail.autoCategory && (
                            <Badge variant="secondary" className="text-xs">Category: {detail.autoCategory}</Badge>
                          )}
                          {detail.autoPriority && (
                            <Badge variant="secondary" className="text-xs">Priority: {detail.autoPriority}</Badge>
                          )}
                        </div>
                      )}
                    </div>
                              ...
                              <button
                                onClick={() => {
                                  if (confirm("Delete this attachment?")) {
                                    deleteAttachmentMutation.mutate({ attachmentId: att.id });
                                  }
                                }}
                      ...
                      {(detail as any).comments?.map((c: any) => (
                        <div
                          ...
                          className={`rounded-lg p-3 text-sm ${
                            c.isInternal
                              ...
                              : c.authorType === "human" && c.authorId !== detail.submittedBy
                        ...
                        >
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            {c.isInternal && (
                              <Badge variant="outline" className="text-[10px] px-1 py-0 gap-0.5 text-yellow-700 border-yellow-300">
              ...
              <div className="border-t bg-white/50 p-3">
                ...
                <div className="flex gap-2">
                  <Textarea
                    ...
                    placeholder={isInternal ? "Add internal note..." : "Reply to user (they will be notified)..."}

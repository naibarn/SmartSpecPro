...
export default function Dashboard() {
  ...
  const adminOpsMenuItems = adminMenuItems.filter((item) => consolidatedAdminOpsIds.has(item.id));
  const adminDirectMenuItems = adminMenuItems.filter((item) => !consolidatedAdminOpsIds.has(item.id));
  const adminCommandCenterItem = adminMenuItems.find((item) => item.id === 'admin-overview') ?? adminOpsMenuItems[0] ?? null;
  const adminOpsSecondaryCount = Math.max(0, adminOpsMenuItems.length - (adminCommandCenterItem ? 1 : 0));
  ...
  const renderAdminDirectMenuItems = (items: typeof adminDirectMenuItems, isMobile: boolean) => (
    items.map((item) => (
      <button
        key={item.id}
        onClick={() => {
                ...
                <div className="pt-3 mt-3 border-t border-slate-200">
                  ...
                  {adminCommandCenterItem && (
                    <div>
                      ...
                      <button
                        onClick={() => {
                          if (adminCommandCenterItem.external) { window.open(adminCommandCenterItem.path, '_blank', 'noopener,noreferrer'); }
                          else { setLocation(adminCommandCenterItem.path); }
                          setSidebarOpen(false);
                        }}
                      ...
                      >
                        <div className="flex items-center gap-3 text-slate-700">
                          ...
                          <span className="font-medium truncate">{adminCommandCenterItem.label}</span>
                          <ChevronRight className="w-4 h-4 ml-auto flex-shrink-0 text-slate-400" />
                        </div>
                        <p className="mt-2 text-xs leading-5 text-slate-500">
                          Start here for server monitoring, alerts, queues, logs, and incident actions.
                        </p>
                        {adminOpsSecondaryCount > 0 && (
                          <p className="mt-1 text-[11px] text-slate-400">
                            {adminOpsSecondaryCount} monitoring tools are grouped inside the command center.
                          </p>
                        )}
                      </button>
                    </div>
                  ...
                  {adminDirectMenuItems.length > 0 && (
                    <div className={adminCommandCenterItem ? "mt-2 space-y-1" : "space-y-1"}>
              ...
              {adminCommandCenterItem && (
                <div>
                  ...
                  <button
                    onClick={() => {
                      if (adminCommandCenterItem.external) {
                        window.open(adminCommandCenterItem.path, '_blank', 'noopener,noreferrer');
                      } else {
                  ...
                  >
                    ...
                    <p className="mt-2 text-xs leading-5 text-slate-500">
                      Open this for server monitoring, alerts, queues, logs, and incident follow-up.
                    </p>
                    {adminOpsSecondaryCount > 0 && (
                      <p className="mt-1 text-[11px] text-slate-400">
                        {adminOpsSecondaryCount} monitoring tools are grouped inside.
                      </p>
                    )}
                  </button>
                </div>

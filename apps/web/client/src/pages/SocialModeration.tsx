...
import { EyeOff, MessageCircleReply, RefreshCcw, ShieldAlert, Trash2, Users } from "lucide-react";
...
function CommentStatusBadge({ status }: { status: SocialModerationCommentSummary["status"] }) {
  return (
    <Badge className={`rounded-full border px-2.5 py-1 text-xs font-medium ${getCommentStatusTone(status)}`}>
...
export default function SocialModeration() {
  ...
  const [replyTarget, setReplyTarget] = useState<SocialModerationCommentSummary | null>(null);
  ...
  const replyMutation = trpc.socialModeration.replyToComment.useMutation({
    ...
    onError: (error) => {
      toast.error(error.message || "Failed to reply to comment");
    },
  });
  ...
  const pages = useMemo<SocialModerationPageOption[]>(() => pagesQuery.data ?? [], [pagesQuery.data]);
  useEffect(() => {
  ...
  const hiddenComments = comments.filter((comment) => comment.status === "hidden").length;
  const deletedComments = comments.filter((comment) => comment.status === "deleted").length;
  const actionedComments = hiddenComments + deletedComments;
  const moderationStats = [
    { label: "Visible", value: visibleComments, color: "bg-emerald-500" },
    { label: "Hidden", value: hiddenComments, color: "bg-amber-500" },
    { label: "Deleted", value: deletedComments, color: "bg-rose-500" },
  ];
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4 xl:col-span-2">
        ...
        <p className="mt-2 text-2xl font-semibold text-slate-900">
          {comments.length} comment{comments.length === 1 ? "" : "s"} reviewed here
        </p>
        <p className="mt-2 max-w-2xl text-sm text-slate-600">
          Keep responses visible, hide spam when needed, and maintain a clear record of moderation actions for the team.
        </p>
            ...
            <div key={stat.label} className="space-y-1">
              ...
              <div className="h-2 rounded-full bg-white/90">
                <div
                  ...
                  style={{ width: `${Math.max((stat.value / moderationMax) * 100, stat.value > 0 ? 22 : 8)}%` }}
                />
              </div>
            </div>
          ))}
      ...
      <div className="rounded-2xl border border-white/80 bg-white/90 p-4 shadow-sm shadow-slate-200/60">
        ...
        <p className="mt-2 text-3xl font-semibold text-slate-900">{visibleComments}</p>
        <p className="mt-2 text-sm text-slate-500">Comments still public on the thread.</p>
      ...
      <div className="rounded-2xl border border-white/80 bg-white/90 p-4 shadow-sm shadow-slate-200/60">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Hidden</p>
        <p className="mt-2 text-3xl font-semibold text-slate-900">{hiddenComments}</p>
        <p className="mt-2 text-sm text-slate-500">Comments temporarily removed from view.</p>
      ...
      <div className="rounded-2xl border border-white/80 bg-white/90 p-4 shadow-sm shadow-slate-200/60">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Deleted</p>
        <p className="mt-2 text-3xl font-semibold text-slate-900">{deletedComments}</p>
        <p className="mt-2 text-sm text-slate-500">Comments permanently removed from the public thread.</p>
  ...
  const handleHide = async (commentId: number) => {
    if (typeof window !== "undefined" && !window.confirm("Hide this comment from the public thread?")) return;
    await hideMutation.mutateAsync({ commentId });
  };

  const handleDelete = async (commentId: number) => {
    if (typeof window !== "undefined" && !window.confirm("Delete this comment permanently?")) return;
  ...
  return (
    <SocialPageShell
      ...
      description="Review public comments, reply to questions, and keep moderation actions visible for the whole team."
      tone="moderation"
      badge={
        <Badge className="rounded-full bg-slate-100 text-slate-700 hover:bg-slate-100">
          {selectedPage ? selectedPage.label : "No page selected"}
        </Badge>
      }
      actions={
        <Button
              ...
              <label className="space-y-2">
                ...
                >
                  <option value="" disabled>
                    {pages.length === 0 ? "No moderation pages available" : "Choose a page"}
                  </option>
                  {pages.map((page) => (
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-200">
                <table className="min-w-full divide-y divide-slate-200 bg-white">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Author</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Comment</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Post</th>
                    ...
                    {comments.map((comment) => (
                      <tr key={comment.id} className="hover:bg-slate-50/70">
                        <td className="px-4 py-4 align-top">
                          ...
                          <div className="text-xs text-slate-500">#{comment.id}</div>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-sm font-medium uppercase tracking-[0.18em] text-slate-500">Reply to comment</div>
                <h2 className="mt-1 text-xl font-semibold text-slate-900">
                  {replyTarget.authorDisplayName || "Unknown"}
                </h2>
            ...
            <div className="mt-5 space-y-2">
              <label className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Reply message</label>
              <Textarea
                value={replyBody}
                onChange={(event) => setReplyBody(event.target.value.slice(0, 2000))}

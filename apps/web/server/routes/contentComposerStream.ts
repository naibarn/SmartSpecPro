...
const streamInputSchema = z.object({
  ...
  executionSource: z.literal("skill").default("skill"),
...
function normalizeArticleHtml(rawText: string, topic: string): string {
  ...
  if (isHtmlLike(stripped)) {
    ...
    return sanitized.includes("<article")
      ? sanitized
...
contentComposerStreamRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!enabled) {
      return res
        .status(403)
        .json({ error: "Content Composer is disabled for this tenant" });
    }
    ...
    const prompt = buildComposerPrompt({
      ...
      sourceLabel: parsed.data.skillId?.trim() || "Skill",
    ...
    try {
        ...
        messages: [
          {
            ...
            content: [
              skill.systemPrompt?.trim() ||
                "You are a senior content editor for the Media Studio composer.",
      ...
      if (!upstream.ok || !upstream.body) {
        const errorText = await upstream
          .text()
          .catch(() => "LLM stream failed");
        sendEvent(res, "error", {
          message:
            errorText || `LLM stream failed with status ${upstream.status}`,
        });
        sendEvent(res, "done", { success: false });
        return res.end();
      }
      ...
      while (true) {
        ...
        while (true) {
          ...
          try {
            const eventPayload = JSON.parse(parsedBlock.data) as Record<
              string,
              unknown
            >;
            const delta =
              eventPayload?.choices && Array.isArray(eventPayload.choices)
                ? (
                    eventPayload.choices[0] as

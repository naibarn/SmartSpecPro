...
const HERMES_RUNTIME_PACK_IDS = new Set([
  "hermes-windows-x64",
  "hermes-macos-arm64",
]);
...
function normalizeConnectDeviceCode(input: unknown): string {
  ...
  if (!code) {
    throw new WorkerAuthError(
      "worker_connect_missing_code",
      400,
      "Missing worker device code"
    );
  }
  return code;
}
...
async function resolveApprovedTenantId(
  ...
  if (cleanRequestedTenantId) {
    if (
      cleanRequestTenantId &&
      cleanRequestTenantId !== cleanRequestedTenantId
    ) {
      return "";
    }
    ...
    const [tenantRow] = await db
      ...
      .from(tenants)
      .where(eq(tenants.id, cleanRequestedTenantId))
      .limit(1);
  ...
  const userId =
    Number.isInteger(auth.userId) && auth.userId && auth.userId > 0
      ? auth.userId
...
function browserSessionPayload(session: WorkerConnectSession) {
  ...
  return {
    status:
      expired && session.status === "pending" ? "expired" : session.status,
...
function handleWorkerRouteError(error: unknown, res: Response): void {
  if (error instanceof RedisEphemeralKeyRegistryError) {
    sendApiError(
      ...
      503,
      error.code,
      "Worker connection state is temporarily unavailable; try again shortly",
      "transient_error"
    );
    return;
  }
...
function findLatestRuntimePack(
  releaseDirs: string[],
  ...
  return (
    packs.sort((left, right) => {
      const versionCompare = right.version.localeCompare(
        left.version,
        undefined,
...
function readRuntimePackManifest(
  packFilePath: string
...
function requestedRuntimeChannel(value: unknown): WorkerRuntimeChannel {
  const channel = String(value || "stable").trim();
  return (workerRuntimeChannelValues as readonly string[]).includes(channel)
    ? (channel as WorkerRuntimeChannel)
    : "stable";
}
...
function archiveEntriesContainFiles(
  entries: Set<string>,
  ...
  return requiredFiles.every(file => {
    if (file.endsWith("*")) {
      ...
      return Array.from(entries).some(entryName =>
...
function archiveContainsFiles(
  filePath: string,
  requiredFiles: string[]
): boolean {
  try {
    return archiveEntriesContainFiles(
      new Set(new AdmZip(filePath).getEntries().map(entry => entry.entryName)),
      requiredFiles
    );
  } catch {
    return false;
...
function runtimeArchiveContainsRequiredFilesAndSignature(
  filePath: string,
  requiredFiles: string[]
): boolean {
  try {
    ...
    if (!archiveEntriesContainFiles(entries, requiredFiles)) return false;
    const signatureEntry = archive.getEntry("runtime-pack/SHA256SUMS.sig");
    if (!signatureEntry) return false;
    const signature = signatureEntry.getData().toString("utf8").trim();
    return (
      signature.length > 0 &&
...
function findLatestAllowedRuntimePack(
  releaseDirs: string[],
  runtimeId = DEFAULT_WORKER_RUNTIME_PACK_ID
) {
  const candidates: Array<
    NonNullable<ReturnType<typeof findLatestRuntimePack>>
  > = [];
  for (const releaseDir of releaseDirs) {
    if (!fs.existsSync(releaseDir)) continue;
    for (const fileName of fs.readdirSync(releaseDir)) {
      ...
      if (
        !runtimeArchiveContainsRequiredFilesAndSignature(
          filePath,
          requiredRuntimeArchiveFiles(runtimeId as WorkerRuntimeId)
        )
      )
        continue;
      candidates.push({
...
const remotionPackVerificationCache = new Map<
  string,
  RemotionPackVerificationCacheEntry
>();
...
function packagedRuntimePath(
  ...
  return allowDirectory
    ? `runtime-pack/${normalized.replace(/\/$/, "")}*`
    : `runtime-pack/${normalized}`;
}
...
function isSignedRemotionExecutorPack(
  ...
  if (
    ...
    parsed.data.runtimeId !== runtimeId
  )
    return false;
  const stat = fs.statSync(filePath);
  ...
  try {
    ...
    const requiredFiles = [
      ...
      packagedRuntimePath(
        parsed.data.sidecarPath ?? "remotion-sidecar/render.mjs"
      ),
    ...
    valid =
      ...
      (parsed.data.archiveSizeBytes === undefined ||
        parsed.data.archiveSizeBytes === stat.size) &&
      (parsed.data.archiveFileName === undefined ||
        parsed.data.archiveFileName === path.basename(filePath)) &&
      ...
      archiveContainsFiles(
        filePath,
        requiredFiles.filter((entry): entry is string => Boolean(entry))
      );
  } catch {
    valid = false;
  }
  remotionPackVerificationCache.set(filePath, {
...
function findLatestRemotionExecutorPack(
  releaseDirs: string[],
  runtimeId: string,
  publicKey: string
) {
  ...
  for (const releaseDir of releaseDirs) {
    ...
    for (const fileName of fs.readdirSync(releaseDir)) {
      ...
      const manifest = readRuntimePackManifest(filePath);
      if (
        !isSignedRemotionExecutorPack(filePath, manifest, runtimeId, publicKey)
      )
        continue;
      ...
      if (
        ...
        (manifestRuntimeId && manifestRuntimeId !== runtimeId)
      )
        continue;
      if (runtimeKind !== "standalone_remotion_executor") continue;
      if (runtimeId.includes("windows") && !platform.includes("windows"))
        continue;
      ...
      if (runtimeId.includes("arm64") && !architecture.includes("arm64"))
        continue;
      if (
        runtimeId.includes("macos-x64") &&
        !architecture.includes("x86_64") &&
        !architecture.includes("x64")
      )
        continue;
      if (
        !archiveContainsFiles(filePath, [
...
function defaultHermesManifestEntry(
  runtimeId: string
  ...
  return {
    ...
    supportedMacModels:
      runtimeId === "hermes-macos-arm64" ? HERMES_MACOS_SUPPORTED_MODELS : [],
    unsupportedMacArchitectures:
      runtimeId === "hermes-macos-arm64" ? ["x86_64 (Intel)"] : [],
  };
}
...
function getWorkerConnectPermissionPolicy(runtimeType: string) {
  const preset: WorkerAccessPermissionPreset =
...
function flattenWorkerCapabilityHints(value: unknown): Set<string> {
  ...
  const visit = (current: unknown) => {
    if (typeof current === "string" && current.length <= 160)
      result.add(current);
...
function workerJobRequirementsMatch(
  value: unknown,
...
async function getWorkerConnectPermissionPolicyForConnection(input: {
  ...
  if (!input.userId || (!input.workerId && !input.externalReference))
    return fallback;
  ...
  try {
    ...
    const parsedPreset = workerAccessPermissionPresetSchema.safeParse(
      policy.permissionPreset
    );
    if (!parsedPreset.success) return fallback;
    return {
      preset: parsedPreset.data,
      scopes:
        parsedPreset.data === "custom"
...
export async function verifyWorkerRouteAccessToken(
  ...
  if (
    await isConnectedDeviceRevoked({
      tenantId: String(claims.tenantId ?? ""),
      workerConnectionId: claims.workerConnectionId
        ? String(claims.workerConnectionId)
        : null,
    ...
    throw new WorkerAuthError(
      ...
      401,
      "Worker connection is revoked and must be paired again"
    );
  }
...
const HERMES_MEDIA_REFERENCE_URL_ACTIVE_STATUSES: ReadonlySet<string> = new Set(
  ["claimed", "preparing", "running", "uploading"]
);
...
function ensureHermesJobScopedAccess(
  ...
  if (job.tenantId !== auth.tenantId || job.runtimeType !== auth.runtimeType) {
    throw new WorkerRuntimeServiceError(
      ...
      403,
      "Worker token does not match the requested job scope",
      "auth_error"
    );
  }
  if (job.workerId && job.workerId !== auth.workerId) {
    throw new WorkerRuntimeServiceError(
      "worker_scope_mismatch",
      403,
      "Worker token does not own the requested job",
      "auth_error"
    );
  }
}
...
function ensureHermesJobLease(
  ...
  if (
    !leaseOwnerToken ||
    !job.leaseOwnerToken ||
    job.leaseOwnerToken !== leaseOwnerToken
  ) {
    throw new WorkerRuntimeServiceError(
      "stale_worker_lease",
      409,
      "Worker lease token is stale or invalid"
    );
  }
  if (
    job.leaseExpiresAt &&
...
export function registerWorkerRuntimeRoutes(
  ...
  const runtimePackReleaseDirs =
    deps.runtimePacks?.releaseDirs ?? getRuntimePackReleaseDirs();
  const runtimePackPublicKey = normalizeRuntimePackPublicKey(
    deps.runtimePacks?.publicKey?.trim() ||
  ...
  const delegatedSessionLimiter = rateLimit("worker-delegated-session", {
    rpm: 60,
  });
    ...
    async (req, res) => {
      try {
        ...
        const runtimeId = String(
          req.query.runtimeId || DEFAULT_WORKER_RUNTIME_PACK_ID
        ).trim();
        ...
        if (HERMES_RUNTIME_PACK_IDS.has(runtimeId)) {
          const hermesPack = findLatestHermesRuntimePack(
          ...
          const hermesManifestArchiveSha256 = stringField(
            hermesManifest.archiveSha256
          ).toLowerCase();
          const hermesArchiveSha256 = /^[a-f0-9]{64}$/.test(
        ...
        if (REMOTION_EXECUTOR_PACK_IDS.has(runtimeId)) {
          const executorPack = findLatestRemotionExecutorPack(
            runtimePackReleaseDirs,
            runtimeId,
          ...
          if (!executorPack) {
            sendApiError(
              ...
              "The standalone Remotion executor pack has not been published yet.",
              "not_found_error"
            );
            return;
          }
          const executorManifest = readRuntimePackManifest(
            executorPack.filePath
          );
          const archiveSha256 = /^[a-f0-9]{64}$/.test(
          ...
          res.json({
            ...(executorManifest ?? {}),
            runtimeId,
            version: executorManifest?.version ?? executorPack.version,
            archiveFileName: executorPack.fileName,
            archiveSha256,
            archiveSizeBytes: executorPack.sizeBytes,
        ...
        if (!SUPPORTED_WORKER_RUNTIME_PACK_IDS.has(runtimeId)) {
          sendApiError(
            ...
            `Runtime pack is not available for ${runtimeId}`,
            "not_found_error"
          );
          return;
        }
        const durableRelease = await getLatestPublishedWorkerRuntimeRelease(
          runtimeId as
            | "hyperframes-wsl2"
            | "hyperframes-windows-x64"
            | "hyperframes-macos-arm64",
        ...
        if (!pack) {
          sendApiError(
            ...
            404,
            "runtime_pack_not_published",
            "Official HyperFrames runtime pack has not been published yet, or the latest pack is incomplete. It must include whisper.cpp transcription, the large-v3 model, and a non-placeholder signature; Mock, fallback, diagnostic smoke, and FFmpeg test-source packs are not allowed for render jobs.",
            "not_found_error"
          );
          return;
        }
        ...
        const manifestArchiveSha256 = stringField(
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        if (hermesMatch?.[1] && HERMES_RUNTIME_PACK_IDS.has(hermesMatch[1])) {
          ...
          if (!hermesPack || hermesPack.fileName !== fileName) {
            sendApiError(
              ...
              404,
              "runtime_pack_not_found",
              "Hermes runtime pack file was not found",
              "not_found_error"
            );
            return;
          }
          ...
          if (!hermesManifest || hermesManifest.allowed !== true) {
            sendApiError(
              ...
              409,
              "runtime_pack_not_allowed",
              "Hermes runtime pack is not allowed for download",
              "invalid_request_error"
            );
            return;
          }
          ...
          res.setHeader(
            "Content-Disposition",
            `attachment; filename="${hermesPack.fileName.replace(/"/g, "")}"`
          );
          fs.createReadStream(hermesPack.filePath).pipe(res);
          return;
        }

        const executorMatch = fileName.match(
          REMOTION_EXECUTOR_PACK_FILE_PATTERN
        );
        if (
          executorMatch?.[1] &&
          REMOTION_EXECUTOR_PACK_IDS.has(executorMatch[1])
        ) {
          const executorPack = findLatestRemotionExecutorPack(
            runtimePackReleaseDirs,
            executorMatch[1],
          ...
          if (!executorPack || executorPack.fileName !== fileName) {
            sendApiError(
              ...
              404,
              "runtime_pack_not_found",
              "Standalone Remotion executor pack was not found",
              "not_found_error"
            );
            return;
          }
        ...
        if (
          !runtimeMatch?.[1] ||
          !SUPPORTED_WORKER_RUNTIME_PACK_IDS.has(runtimeMatch[1])
        ) {
          sendApiError(
            res,
            400,
            "invalid_runtime_pack_file",
            "Invalid runtime pack file name",
            "invalid_request_error"
          );
          return;
        }
        ...
        if (durableRelease && durableRelease.runtimeId === runtimeMatch[1]) {
          ...
          if (!stored) {
            sendApiError(
              ...
              404,
              "runtime_pack_not_found",
              "Runtime pack file was not found in storage",
              "not_found_error"
            );
            return;
          }
        ...
        if (!pack || pack.fileName !== fileName) {
          sendApiError(
            ...
            404,
            "runtime_pack_not_found",
            "Runtime pack file was not found",
            "not_found_error"
          );
          return;
        }
        ...
        if (!isOfficialRuntimePackManifest(manifest, runtimeMatch[1])) {
          sendApiError(
            ...
            409,
            "runtime_pack_not_allowed",
            "Runtime pack is not allowed for download",
            "invalid_request_error"
          );
          return;
        }
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="${pack.fileName.replace(/"/g, "")}"`
        );
    ...
    async (req, res) => {
      try {
        ...
        const expiresAt = new Date(
          now.getTime() + WORKER_CONNECT_TTL_SECONDS * 1000
        );
        const deviceCode = randomCode(32);
        let userCode = randomUserCode();
        for (
          let attempt = 0;
          attempt < 5 && (await getWorkerConnectSessionByUserCode(userCode));
          attempt += 1
        ) {
  ...
  app.get("/api/workers/connect/status", connectLimiter, async (req, res) => {
    try {
      ...
      if (!session) {
        sendApiError(
          res,
          404,
          "worker_connect_not_found",
          "Worker connection request was not found or has expired",
          "not_found_error"
        );
        return;
      }
      ...
      try {
        ...
        if (isWorkerConnectExpired(session)) {
          ...
          sendApiError(
            res,
            410,
            "worker_connect_expired",
            "Worker connection request has expired. Please start Connect again in the Worker App.",
            "invalid_request_error"
          );
          return;
        }
        ...
        if (session.status !== "pending") {
          sendApiError(
            res,
            409,
            "worker_connect_not_pending",
            "Worker connection request is no longer waiting for approval.",
            "invalid_request_error"
          );
          return;
        }
        ...
        if (!auth.ok || auth.mode !== "session") {
          sendApiError(
            ...
            401,
            "unauthorized",
            "Please log in before approving this Worker App.",
            "auth_error"
          );
          return;
        }
        ...
        const requestTenantId = String(
          requestTenant.tenantId || requestTenant.tenant?.id || ""
        ).trim();
        const tenantId = await resolveApprovedTenantId(
          auth,
          requestedTenantId,
          requestTenantId
        );
        if (!tenantId) {
          sendApiError(
            ...
            400,
            "tenant_required",
            "Please select a workspace before approving this Worker App.",
            "invalid_request_error"
          );
          return;
        }

        const connectPermissionPolicy =
          await getWorkerConnectPermissionPolicyForConnection({
            runtimeType: session.payload.runtimeType,
            tenantId,
            userId: auth.userId,
        ...
        const registrationAuth = await verifyWorkerRegistrationToken(
          registrationToken,
          {
            runtimeType: session.payload.runtimeType,
          }
        ...
        if (auth.userId && session.payload.deviceBinding) {
          await upsertConnectedDevice({
            ...
            platform:
              typeof session.payload.hardwareJson?.platform === "string"
                ? session.payload.hardwareJson.platform
                : null,
            architecture:
              typeof session.payload.hardwareJson?.architecture === "string"
                ? session.payload.hardwareJson.architecture
                : null,
            scopes: connectPermissionPolicy.scopes,
            approvedAt: new Date(),
            metadataJson: {
              source: "worker_connect",
              externalReference: result.worker.externalReference,
            },
          });
        }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (
          session.payload.deviceBinding?.deviceId &&
          (await isConnectedDeviceRevoked({
          ...
          sendApiError(
            res,
            401,
            "worker_connection_revoked",
            "Worker connection was revoked and must be approved again",
            "authorization_error"
          );
          return;
        }
        ...
        if (session.payload.deviceBinding?.deviceId) {
          const executionMetadataUpdated = await verifyBearerToken(
          ...
          const refreshMetadataUpdated = await verifyBearerToken(
            tokens.refreshToken
          ).then(async claims => {
            return updateConnectedDeviceTokenMetadata({
              tenantId: String(
              ...
              deviceId:
                typeof claims.deviceId === "string" ? claims.deviceId : null,
              authKind: "worker_executor",
              refreshTokenExpiresAt: claims.exp
                ? new Date(claims.exp * 1000)
                : null,
            });
          });
    ...
    async (req, res) => {
      try {
        ...
        await updateConnectedDeviceTokenMetadata({
          ...
          deviceId:
            typeof refreshClaims.deviceId === "string"
              ? refreshClaims.deviceId
              : null,
          authKind: "worker_executor",
          accessTokenExpiresAt: executionClaims.exp
            ? new Date(executionClaims.exp * 1000)
            : null,
          refreshTokenExpiresAt: refreshClaims.exp
            ? new Date(refreshClaims.exp * 1000)
            : null,
        }).catch(error => {
          console.warn(
            "[workerRuntime] connected device token metadata unavailable",
    ...
    async (req, res) => {
      try {
        ...
        await updateConnectedDeviceTokenMetadata({
          ...
          console.warn(
            "[workerRuntime] connected device heartbeat metadata unavailable",
            error instanceof Error ? error.message : String(error)
          );
        });
        res.json({
          ...
          warningFlagsJson: Array.isArray(worker.warningFlagsJson)
            ? worker.warningFlagsJson
            : [],
        });
      } catch (error) {
        handleWorkerRouteError(error, res);
  ...
  app.get("/api/worker-runtime/jobs/summary", async (req, res) => {
    try {
      ...
      const [worker] = await db
        ...
        .where(
          and(
            eq(workers.id, auth.workerId),
            eq(workers.tenantId, auth.tenantId)
          )
        )
        .limit(1);
      if (!worker)
      ...
      if (!Number.isInteger(requestedLimit) || requestedLimit < 1)
        throw new WorkerRuntimeServiceError(
          "invalid_job_summary_limit",
          400,
          "Invalid job summary limit"
        );
      ...
      if (
        ...
        throw new WorkerRuntimeServiceError(
          "invalid_job_summary_cursor",
          400,
          "Invalid job summary cursor"
        );
      const cursor =
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            or(
              eq(workerJobs.workerId, auth.workerId),
              eq(workerJobs.status, "queued")
            )
          )
        )
        .orderBy(
          desc(workerJobs.priority),
          asc(workerJobs.createdAt),
          asc(workerJobs.id)
        )
        .limit(501);
      const capabilities = flattenWorkerCapabilityHints(
        worker.capabilitiesJson
      );
      const visible = rows.filter(row => {
        if (
          row.status === "queued" &&
          !workerJobRequirementsMatch(
            row.capabilityRequirementsJson,
            capabilities
          )
        )
          return false;
        ...
        return (
          ...
          (requestedScope === "recent" &&
            ["completed", "failed", "canceled", "expired"].includes(row.status))
        );
      });
      ...
      const queuePositions = new Map(
        waitingRows.map((row, index) => [row.id, index + 1])
      );
      const ordered = [...visible].sort((left, right) => {
        ...
        return terminalStatuses.has(left.status) &&
          terminalStatuses.has(right.status)
      ...
      const seriesRows =
        seriesIds.length > 0
          ? await db
              ...
              .where(
                and(
                  eq(verticalDramaSeries.tenantId, auth.tenantId),
                  inArray(
      ...
      const seriesTitles = new Map(
        seriesRows.map(series => [String(series.id), series.title])
      );
      ...
      const events = visibleIds.length
        ? await db
            ...
            .where(inArray(workerJobEvents.workerJobId, visibleIds))
            .orderBy(desc(workerJobEvents.createdAt))
            .limit(500)
        : [];
      const latest = new Map<string, (typeof events)[number]>();
      for (const event of events)
        if (!latest.has(event.workerJobId))
          latest.set(event.workerJobId, event);
      res.setHeader("Cache-Control", "no-store");
      ...
      const projectedItems = ordered.map(row => {
        ...
        const input =
          ...
          !Array.isArray(row.inputJson)
            ...
            : {};
        const seriesId =
          typeof input.seriesId === "string" ? input.seriesId : null;
        const workflowResolution =
          typeof input.workflowResolution === "object" &&
          input.workflowResolution &&
          !Array.isArray(input.workflowResolution)
            ? (input.workflowResolution as Record<string, unknown>)
            : null;
        const workflowId =
          typeof input.workflowId === "string"
            ? input.workflowId
            : typeof workflowResolution?.selectedWorkflowId === "string"
              ? workflowResolution.selectedWorkflowId
              : typeof workflowResolution?.workflowId === "string"
                ? workflowResolution.workflowId
                : null;
        const connectionResolution =
          typeof input.connectionResolution === "object" &&
          input.connectionResolution &&
          !Array.isArray(input.connectionResolution)
            ? (input.connectionResolution as Record<string, unknown>)
            : null;
        const output =
          row.outputJson &&
          typeof row.outputJson === "object" &&
          !Array.isArray(row.outputJson)
            ? (row.outputJson as Record<string, unknown>)
            : {};
        const outputCount = Array.isArray(output.outputs)
        ...
        const eventSequence =
          ...
          Number.isFinite(payload.sequenceNumber)
            ? Math.max(0, Math.floor(payload.sequenceNumber))
            : 0;
        const statusReason =
          typeof row.statusReason === "string"
            ? row.statusReason.slice(0, 240)
            : null;
        const failureReason =
          typeof row.failureReason === "string"
            ? row.failureReason.slice(0, 500)
            : null;
        ...
        return {
          jobId: row.id,
          jobType: row.jobType,
          jobTypeLabelKey: `worker.jobType.${row.jobType}`,
          status: row.status,
          phase: typeof payload.stage === "string" ? payload.stage : row.status,
          progressPercent:
          ...
          updatedAt: event?.createdAt ?? row.startedAt ?? row.createdAt,
          queuedAt: row.createdAt,
          claimedAt: row.startedAt,
          terminalAt: row.finishedAt,
          workerId: row.workerId,
          workerDisplayName: isAssignedToThisWorker ? worker.displayName : null,
          workerMachineName: isAssignedToThisWorker ? worker.machineName : null,
          seriesId,
          seriesTitle: seriesId ? (seriesTitles.get(seriesId) ?? null) : null,
          episodeId:
            typeof input.episodeId === "string" ? input.episodeId : null,
          shotId: typeof input.shotId === "string" ? input.shotId : null,
          workflowId,
          workflowVersion:
            typeof workflowResolution?.version === "string"
              ? workflowResolution.version
              : null,
          connectionProfileId:
            typeof connectionResolution?.selectedProfileId === "string"
              ? connectionResolution.selectedProfileId
              : null,
          remoteExecutionId:
            typeof payload.executionId === "string"
              ...
              : null,
          capacitySlot:
            row.resourceProfile === "gpu_required" ? "gpu" : "worker",
          resourceProfile: row.resourceProfile,
          queuePosition: queuePositions.get(row.id) ?? null,
          waitReason:
            row.status === "queued"
              ? row.capabilityRequirementsJson
                ? "waiting_for_worker_capacity_or_capability"
          ...
          latestEventMessage:
            typeof payload.message === "string"
              ...
              : null,
          retryable: ["failed", "expired"].includes(row.status),
          cacheHit:
            typeof payload.cacheHit === "boolean" ? payload.cacheHit : null,
          nextAction:
            row.status === "queued"
              ? "wait_for_worker"
              : row.status === "failed"
                ...
                : null,
          recoveryState:
            String(row.status) === "reconciling" ? "reconciling" : null,
          canCancel,
          cancellationState: row.status === "canceled" ? "confirmed" : null,
          outputCount,
          eventSequence,
          lastEventSequence: eventSequence,
          projectionRevision,
          observedAt: serverNow.toISOString(),
          staleAt: new Date(serverNow.getTime() + 30_000).toISOString(),
        };
      });
      const pageItems = projectedItems.slice(cursor, cursor + limit);
      const activeItems = projectedItems.filter(item =>
        [
          "claimed",
          "preparing",
          "preflight",
      ...
      const waitingItems = projectedItems.filter(
        item => item.status === "queued"
      );
      const recentItems = projectedItems.filter(item =>
        ["completed", "failed", "canceled", "expired"].includes(item.status)
      );
      res.json({
        projectionRevision,
        ...
        active: pageItems.filter(item =>
          activeItems.some(candidate => candidate.jobId === item.jobId)
        ),
        waiting: pageItems.filter(item =>
          waitingItems.some(candidate => candidate.jobId === item.jobId)
        ),
        recent: pageItems.filter(item =>
          recentItems.some(candidate => candidate.jobId === item.jobId)
        ),
        counts: {
        ...
        nextCursor:
          cursor + limit < projectedItems.length
            ? String(cursor + limit)
            : undefined,
        items: pageItems,
      });
  ...
  app.get("/api/worker-jobs/:jobId/delegated-manifest", async (req, res) => {
    try {
      const token = requireBearerToken(req);
      const auth = await verifyWorkerRouteAccessToken(req, token, {
        allowedTokenUses: ["worker_execution"],
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const token = requireBearerToken(req);
        const parsed = delegatedWorkerCallbackPayloadSchema.parse(
          req.body ?? {}
        );
        const auth = await verifyWorkerRouteAccessToken(req, token, {
          allowedTokenUses: ["worker_execution"],
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const job = await defaultHermesMediaAdapterRepo.getJobById(
          req.params.jobId
        );
        if (!job) {
          throw new WorkerRuntimeServiceError(
            "not_found",
            404,
            `Worker job ${req.params.jobId} was not found`,
            "not_found_error"
          );
        }
      ...
      try {
        ...
        if (job && HERMES_MEDIA_JOB_TYPES.has(job.jobType)) {
          try {
            ...
            await finalizeHermesMediaArtifact({
              job,
              artifact: result.artifact as unknown as WorkerArtifact,
            });
            ...
            try {
              const completedJob =
                await defaultHermesMediaAdapterRepo.getJobById(
                  req.params.jobId
                );
              if (completedJob) {
              ...
              debugError(
                "workerRuntime",
                `Failed to settle hermes job ${req.params.jobId} after finalize`,
                settleError
              );
            }
          } catch (finalizeError) {
            ...
            debugError(
              "workerRuntime",
              `Hermes media finalize failed for job ${req.params.jobId}`,
              finalizeError
            );
          }
        }

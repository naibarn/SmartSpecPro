...
const createModelSchema = z.object({
  ...
  thinkingModeDefault: z.string().min(1).max(32).default("none"),
  thinkingModes: z.array(z.string().min(1).max(32)).default(["none"]),
...
const updateModelSchema = z.object({
  ...
  thinkingModeDefault: z.string().min(1).max(32).optional(),
  thinkingModes: z.array(z.string().min(1).max(32)).optional(),
    ...
    .mutation(async ({ input }) => {
      ...
      const [model] = await db
        ...
        .values({
          ...
          voices: input.voices,
          thinkingModeDefault: input.thinkingModeDefault,
          thinkingModes: input.thinkingModes,
          configJson,
    ...
    .mutation(async ({ input }) => {
      ...
      const [created] = await db
        ...
        .values({
          ...
          thinkingModeDefault: template.thinkingModeDefault ?? "none",
          thinkingModes: template.thinkingModes ?? ["none"],
          configJson,
          isEnabled: template.isEnabled !== false,
          priority: template.priority ?? 99,
          sortOrder: template.priority ?? 99,

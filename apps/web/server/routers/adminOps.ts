...
function isProviderUsageLogUnavailableError(error: unknown): boolean {
  ...
  return (
    (message.includes("provider_usage_log") &&
...
export const adminOpsRouter = router({
    ...
    .query(async ({ input }) => {
      ...
      const [totals] = await db
        .select({
          ...
          activeToday:
            sql<number>`COUNT(*) FILTER (WHERE "lastSignedIn" >= CURRENT_DATE)`.as(
              "active_today"
            ),
        })
        .from(users);
      ...
      try {
        ...
        const metrics = await db
          .select({
            ...
            errorCount:
              sql<number>`COUNT(*) FILTER (WHERE "statusCode" >= 500)`.as(
                "error_count"
              ),
            avgLatencyMs: sql<number>`COALESCE(AVG("responseTimeMs"), 0)`.as(
              "avg_latency_ms"
            ),
            p95LatencyMs:
              sql<number>`COALESCE(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY "responseTimeMs"), 0)`.as(
                "p95_latency_ms"
              ),
          })
        ...
        const byProvider = await db
          .select({
            ...
            totalCostUsd: sql<number>`COALESCE(SUM("costUsd"::numeric), 0)`.as(
              "total_cost_usd"
            ),
          })
          .from(providerUsageLog)
        ...
        return {
          summary: {
            ...
            errorRate:
              totalReqs > 0
                ? Number(((errorCnt / totalReqs) * 100).toFixed(2))
                : 0,
        ...
        console.warn("[adminOps.apiHealth] falling back to empty metrics", {
          reason: unavailable
            ? "provider_usage_log_unavailable"
            : "query_failed",
   ...
   */
  jobsHealth: domainAdminProcedure.query(async () => {
    const { getDb } = await import("../db");
    const db = await getDb();
    if (!db) {
      return {
    ...
    const { workerJobs, workerJobOutbox } =
    ...
    try {
      ...
      const [unpublished] = await db
        .select({ count: count(workerJobOutbox.id).as("count") })
        .from(workerJobOutbox)
        .where(
          sql`${workerJobOutbox.publishedAt} IS NULL AND ${workerJobOutbox.cancelledAt} IS NULL AND ${workerJobOutbox.quarantinedAt} IS NULL`
        );
  ...
  kieAiHealth: domainAdminProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!db)
        return {
          summary: { total: 0, completed: 0, failed: 0, callbackRate: 0 },
          dlqCount: 0,
        };

      const { mediaCallbackEvents, mediaCallbackDlq } =
        await import("../../drizzle/schema");
      const { sql, gte, count } = await import("drizzle-orm");
  ...
  storageStats: domainAdminProcedure.query(async () => {
    ...
    try {
      ...
      const bucket =
        process.env.R2_BUCKET || process.env.S3_BUCKET || "smartspec-storage";
    ...
    const stats = {
      ...
      totalSizeGb: Number(
        (
          Object.values(results).reduce((sum, d) => sum + d.sizeBytes, 0) /
          (1024 * 1024 * 1024)
        ).toFixed(3)
      ),
    ...
    if (db) {
      try {
        ...
        reason = unavailable
          ...
          : "query_failed";
        console.warn(
          "[adminOps.securityStats] falling back to empty provider error metrics",
          {
            reason,
            error: getErrorMessage(error),
          }
    ...
    .query(async ({ input }) => {
      ...
      if (!db)
        return {
          topModels: [],
          daily: [],
          totals: { requests: 0, cost: 0, inputTokens: 0, outputTokens: 0 },
        };
      ...
      try {
        ...
        const dailyRows = await db
          .select({
            date: sql<string>`date_trunc('day', "createdAt")::date::text`.as(
              "date"
            ),
            model: modelCaseExpr.as("model"),
            requests: sql<number>`count(*)::int`.as("requests"),
            cost: sql<number>`coalesce(sum("costUsd"::numeric), 0)::float`.as(
              "cost"
            ),
            inputTokens: sql<number>`coalesce(sum("inputTokens"), 0)::int`.as(
              "input_tokens"
            ),
            outputTokens: sql<number>`coalesce(sum("outputTokens"), 0)::int`.as(
              "output_tokens"
            ),
          })
          .from(providerUsageLog)
  ...
  dailyMediaUsage: domainAdminProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!db)
        return {
          daily: [],
          totals: { requests: 0, cost: 0, credits: 0, byType: {} },
        };
      ...
      try {
        const dailyRows = await db
          .select({
            date: sql<string>`date_trunc('day', ${apiAuditEvents.createdAt})::date::text`.as(
              "date"
            ),
            mediaType: apiAuditEvents.mediaType,
            requests: sql<number>`count(*)::int`.as("requests"),
            cost: sql<number>`coalesce(sum(${apiAuditEvents.costUsd}::numeric), 0)::float`.as(
              "cost"
            ),
            credits:
              sql<number>`coalesce(sum(${apiAuditEvents.creditsCharged}), 0)::int`.as(
                "credits"
              ),
          })
          .from(apiAuditEvents)
          ...
          .groupBy(
            sql`date_trunc('day', ${apiAuditEvents.createdAt})`,
            apiAuditEvents.mediaType
          )
  ...
  pendingApprovalCounts: domainAdminProcedure.query(async () => {
    try {
      ...
      return { skills: s, agencies: 0, templates: 0, total: s };
    } catch {
  ...
  pendingApprovalList: domainAdminProcedure
    .input(
      z.object({
        ...
        type: z.enum(["skill"]).optional(),
      })
    )
    .query(async ({ input }) => {
      // Retired Agency/workflow approval records are intentionally not exposed.
      // The old branches below remain unreachable until their historical tables
      // are removed in a data-retention migration.
      if (!input.type) input = { ...input, type: "skill" };
      try {
        const { db: getDb } = await import("../db");
        const { skills, agencies, workflowTemplates, users } =
        ...
        if (input.type === "skill") {
          const rows = await getDb
            .select({
              ...
              requestedAt: skills.requestedPublishAt,
            })
            .from(skills)
        ...
        if (input.type === "agency") {
          const rows = await getDb
            .select({
              ...
              ownerName: users.name,
              requestedAt: agencies.requestedPublishAt,
            })
            .from(agencies)
        ...
        if (input.type === "template") {
          const rows = await getDb
            .select({
              ...
              ownerName: users.name,
              requestedAt: workflowTemplates.requestedPublishAt,
            })

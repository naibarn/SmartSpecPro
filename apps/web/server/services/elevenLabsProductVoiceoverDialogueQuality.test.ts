...
} from "./elevenLabsProductVoiceoverDialogueQuality";

describe("evaluateElevenLabsProductVoiceoverDialogueQuality", () => {
  it("flags cleanser treatment and weak audio quality issues by rule category", () => {
    ...
    const report = evaluateElevenLabsProductVoiceoverDialogueQuality(output, ELEVENLABS_PRODUCT_VOICEOVER_DIALOGUE_SKILL_ID);
  ...
  it("does not evaluate other skills", () => {
    const report = evaluateElevenLabsProductVoiceoverDialogueQuality(
  ...
  it("still evaluates the legacy beauty dialogue slug as an alias", () => {
    const report = evaluateElevenLabsProductVoiceoverDialogueQuality(
      "Speaker 1: Tea Tree Oil ช่วยฆ่าเชื้อแบคทีเรีย",
    ...
    expect(report.passed).toBe(false);
    expect(report.issues.map((issue) => issue.code)).toContain("cleanser_claim_risk");
  });
  ...
  it("normalizes accidental two-speaker output into unprefixed single-speaker voiceover", () => {
    ...
    expect(normalizeElevenLabsProductVoiceoverDialogueOutput(
      output,
      ELEVENLABS_PRODUCT_VOICEOVER_DIALOGUE_SKILL_ID,
      { speaker_count: "1" },
    )).toBe([
  ...
  it("adds duration and metadata conversion rules to the repair prompt for longer scripts", () => {
    const prompt = buildElevenLabsProductVoiceoverDialogueRepairPrompt({
    ...
    expect(resolveElevenLabsProductVoiceoverDialogueRepairMaxTokens({ target_duration_seconds: "90" })).toBe(3060);
  });
});

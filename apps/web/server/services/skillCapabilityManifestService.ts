...
> = {
  ...
  "video-project-motion-director": {
    manifestSchemaVersion: 1,
    purpose:
      "Propose deterministic motion-template alternatives for Video Studio scenes without generating media.",
    surfaceSupport: ["skill"],
    supportedOriginSurfaces: ["video_edit"],
    supportedEntryPoints: ["system"],
    taskTypes: ["motion_variant_generation", "video_motion_planning"],
    requiredContext: ["objective_brief", "scene_sequence"],
    preferredContext: ["timeline", "template_catalog", "narration"],
    inputs: {
      brief: "object",
      scenes: "object[]",
    ...
    outputs: {
      scenes: "motion_variant_list",
      summary: "string",
    },
    supportedArtifactTypes: ["motion_variant_list"],
    evidenceRequired: ["objective", "scene_sequence"],
    reviewChecklist: [
      "Every proposed variant references a real template and scene.",
      "The skill never emits a media-generation prompt or provider instruction.",
    ],
    failureModes: ["invalid_template_binding", "motion_variant_schema_failure"],
    doNotUseWhen: ["direct_media_generation", "final_render"],
    requiredConnectors: [],
    writeScope: [],
    sideEffectClass: "read_only",
    dataSensitivity: "internal",
    ...
    humanApprovalRequired: false,
    allowedModelFamilies: [...COMMON_ALLOWED_MODEL_FAMILIES],
    completionSignals: ["motion_variants_ready", "template_bindings_valid"],
    selectionSignals: ["motion", "animation", "timeline", "scene variants"],
    negativeSignals: ["image_prompt", "video_prompt", "direct_generation"],
    requiredEvidenceKinds: ["objective", "timeline"],
    reviewerProfile: "video_producer",
    repairStrategy: "Regenerate invalid scene variants while preserving the supplied template catalog.",
    supportsRepairLoop: true,
    ownerTeam: "video-studio",
    ownerCodeownersPath: "apps/web/skills/video-project-motion-director/skill.md",
    ownerReviewCadence: "monthly",
  },
  "video-project-broll-prompt": {
    manifestSchemaVersion: 1,
    purpose:
      "Draft a detailed, editable B-roll image or image-to-video prompt from one Video Studio scene before paid generation.",
    surfaceSupport: ["skill"],
    supportedOriginSurfaces: ["video_edit"],
    supportedEntryPoints: ["system"],
    taskTypes: ["broll_prompt_generation", "image_prompt_generation", "video_prompt_generation"],
    requiredContext: ["objective_brief", "scene_sequence"],
    preferredContext: ["narration", "timeline", "reference_notes"],
    inputs: {
      kind: "image_or_video",
      brief: "object",
      scene: "object",
    ...
    outputs: {
      ...
      negativePrompt: "text",
      shotSummary: "text",
      motionDirection: "text",
    },
    supportedArtifactTypes: ["image_prompt", "video_prompt", "broll_prompt"],
    evidenceRequired: ["objective", "scene_sequence"],
    reviewChecklist: [
      "The prompt is traceable to the selected scene narration and captions.",
      "The prompt is editable and no media task is submitted by the skill.",
    ],
    failureModes: ["prompt_drift_from_scene", "invented_claims", "generic_visual_direction"],
    doNotUseWhen: ["direct_media_job_submission", "final_render"],
    requiredConnectors: [],
    writeScope: [],
    sideEffectClass: "read_only",
    dataSensitivity: "internal",
    ...
    humanApprovalRequired: false,
    allowedModelFamilies: [...PROMPT_ALLOWED_MODEL_FAMILIES],
    completionSignals: ["broll_prompt_ready", "scene_link_preserved"],
    selectionSignals: ["b-roll", "broll", "image prompt", "image to video"],
    negativeSignals: ["direct_generation", "final_publish"],
    requiredEvidenceKinds: ["objective", "scene_sequence"],
    reviewerProfile: "video_producer",
    repairStrategy: "Rewrite only the prompt details that drift from the selected scene.",
    supportsRepairLoop: true,
    ownerTeam: "video-studio",
    ownerCodeownersPath: "apps/web/skills/video-project-broll-prompt/skill.md",
    ownerReviewCadence: "monthly",
  },

...
export interface ResponsesRuntimeTurnInput {
  tenantId: string;
  userId: number;
  objective: string;
  skillSlug: string;
...
export type ResponsesRuntimeTurnResult =
  SharedSkillRuntimeExecutionResult<SkillLlmResult>;

export interface ResponsesRuntimeReplaySnapshot {
...
export interface ResponsesRuntimeReplayComparison {
  matches: boolean;
  mismatchCodes: Array<
    | "selected_skill_drift"
    | "schema_validity_drift"
    | "status_drift"
    | "model_drift"
    | "provider_drift"
  >;
...
export function selectResponsesRuntimeSelection(
  featureFlags?: Partial<OpenAiAgentsRuntimeFlagSnapshot> | null,
...
export function compareResponsesReplaySnapshots(
  baseline: ResponsesRuntimeReplaySnapshot,
  candidate: ResponsesRuntimeReplaySnapshot,
): ResponsesRuntimeReplayComparison {
  const mismatchCodes: ResponsesRuntimeReplayComparison["mismatchCodes"] = [];
...
export async function executeResponsesRuntimeTurn(
  input: ResponsesRuntimeTurnInput,
): Promise<ResponsesRuntimeTurnResult> {
  ...
  return executeSharedSkillRuntime<SkillLlmResult, SkillLlmResult>({
    ...
    requestLabel: input.requestLabel ?? `responses:${input.skillSlug}`,

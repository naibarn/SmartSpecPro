...
export interface ChatRuntimeReplaySnapshot {
  selectedSkillSlug: string | null;
  selectedModelId: string | null;
  selectedProviderName: string | null;
  traceShape: string | null;
  status: string | null;
}

export interface ChatRuntimeReplayComparison {
  matches: boolean;
  mismatchCodes: Array<
...
export function compareChatReplaySnapshots(
  baseline: ChatRuntimeReplaySnapshot,
  candidate: ChatRuntimeReplaySnapshot,
): ChatRuntimeReplayComparison {
  const mismatchCodes: ChatRuntimeReplayComparison["mismatchCodes"] = [];
  ...
  if ((baseline.traceShape ?? null) !== (candidate.traceShape ?? null)) {
    mismatchCodes.push("trace_shape_drift");
  }
...
export async function executeChatRuntimeTurn(
  input: ChatRuntimeTurnInput,
  ...
  return executeSharedSkillRuntime<SkillLlmResult, SkillLlmResult>({
    ...
    requestLabel: input.requestLabel ?? `chat:${input.skillSlug}`,

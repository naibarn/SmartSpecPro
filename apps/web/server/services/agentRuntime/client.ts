...
import { getAppRuntimeConfig, getPreferredInternalToken, type RuntimeConfig } from "../appRuntimeConfig";
...
const AgentRuntimeCancelRequestSchema = z.object({
...
const AgentRuntimeHealthSchema = z.object({
  ...
  supportedRuntimeContractVersions: z.array(z.number().int()).default([
    CURRENT_RUNTIME_CONTRACT_VERSION,
  ]),
  supportedTraceSchemaVersions: z.array(z.number().int()).default([
    CURRENT_TRACE_SCHEMA_VERSION,
  ]),
  supportedCheckpointSchemaVersions: z.array(z.number().int()).default([
    CURRENT_CHECKPOINT_SCHEMA_VERSION,
  ]),
});
...
function assertHealthCompatibility(health: AgentRuntimeHealth): void {
  if (!health.supportedRuntimeContractVersions.includes(CURRENT_RUNTIME_CONTRACT_VERSION)) {
    throw new AgentRuntimeClientError({
  ...
  if (!health.supportedTraceSchemaVersions.includes(CURRENT_TRACE_SCHEMA_VERSION)) {
    throw new AgentRuntimeClientError({
...
function assertCompatibleResponseVersions(response: AgentRuntimeResponse): void {
...
function parseValidatedAgentRuntimeResponse(
  ...
  if (!parsed.success) {
    ...
    throw new AgentRuntimeClientError({
      ...
      message: "Adapter returned a payload that does not match the runtime response contract.",
      status: 502,
      details: {
        issues: parsed.error.issues,
      },
...
export function verifyAgentRuntimeResponseForRequest(
  request: Pick<
    AgentRuntimeRequest,
    "allowedSkills" | "allowedTools" | "allowedAgents" | "requestId"
  >,
...
export class AgentRuntimeClient {
  ...
  constructor(options: AgentRuntimeClientOptions = {}) {
    ...
    this.runtimeConfigLoader = options.runtimeConfigLoader ?? getAppRuntimeConfig;
  ...
  async runStreamed(payload: AgentRuntimeRequest): Promise<AgentRuntimeResponse> {
    const request = AgentRuntimeRequestSchema.parse(payload);
    return this.callMutation("runStreamed", request);
  }

  async resume(payload: AgentRuntimeResumeRequest): Promise<AgentRuntimeResponse> {
    const request = AgentRuntimeResumeRequestSchema.parse(payload);
    return this.callMutation("resume", request);
  }

  async cancel(payload: AgentRuntimeCancelRequest): Promise<AgentRuntimeResponse> {
  ...
  private async callMutation(
    ...
    payload: AgentRuntimeRequest | AgentRuntimeResumeRequest | AgentRuntimeCancelRequest,
    ...
    if ("allowedSkills" in payload && "allowedTools" in payload && "allowedAgents" in payload) {

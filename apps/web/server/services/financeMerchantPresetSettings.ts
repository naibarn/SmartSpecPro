...
import {
  DEFAULT_FINANCE_PINNED_MERCHANT_PRESETS,
  applyFinancePinnedMerchantPresetToDraft,
  financePinnedMerchantPresetCollectionSchema,
...
const KEY_PINNED_MERCHANT_PRESETS = "pinned_merchant_presets";
...
function parsePresetCollection(raw: SettingsRow | undefined): FinanceSlipMappingPreset[] {
  ...
  for (const candidate of candidates) {
    const parsed = financePinnedMerchantPresetCollectionSchema.safeParse(candidate);
    if (parsed.success) {
...
function loadPresetFromRow(row?: SettingsRow): FinanceSlipMappingPreset[] {
  ...
  return parsed.length > 0 ? parsed : DEFAULT_FINANCE_PINNED_MERCHANT_PRESETS;
}

async function loadPinnedMerchantPresets(): Promise<FinanceSlipMappingPreset[]> {
  try {
    const db = await getDb();
    if (!db) {
      return DEFAULT_FINANCE_PINNED_MERCHANT_PRESETS;
    }
    ...
    const [row] = await db
      ...
      .where(and(
        ...
        eq(systemSettings.key, KEY_PINNED_MERCHANT_PRESETS),
      ))
      .limit(1);
...
export async function getPinnedMerchantPresets(): Promise<FinanceSlipMappingPreset[]> {
  const now = Date.now();
  ...
  if (!refreshPromise) {
    refreshPromise = loadPinnedMerchantPresets()
      .then((presets) => {
...
export function getPinnedMerchantPresetsSnapshot(): FinanceSlipMappingPreset[] {
  return cachedPresets ?? DEFAULT_FINANCE_PINNED_MERCHANT_PRESETS;
}

export function clearPinnedMerchantPresetCache(): void {
  cachedPresets = null;
  cacheExpiresAt = 0;
  refreshPromise = null;
}

export function applyPinnedMerchantPresetsToDraft(
  draft: FinanceStructuredDraft,
  text: string,
  presets: FinanceSlipMappingPreset[] = getPinnedMerchantPresetsSnapshot(),
): FinanceStructuredDraft {
  const matchedPreset = findBestFinancePinnedMerchantPreset({
    text,
  ...
  return applyFinancePinnedMerchantPresetToDraft(draft, matchedPreset);
}

export async function applyPinnedMerchantPresetsToDraftAsync(
  draft: FinanceStructuredDraft,
  text: string,
): Promise<FinanceStructuredDraft> {
  return applyPinnedMerchantPresetsToDraft(draft, text, await getPinnedMerchantPresets());
}

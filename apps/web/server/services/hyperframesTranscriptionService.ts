...
function resolveWhisperExecutable(env?: HyperframesRuntimeAdapterEnv): string {
...
export async function transcribeHyperframesStoryboardShot(input: {
  ...
  try {
    ...
    if (/whisper-cpp not found/i.test(message)) {
      throw new Error(
        "HyperFrames transcribe runtime is not ready: whisper-cpp is missing on this local worker. Install whisper.cpp before using shot subtitle transcription."
      );
    }

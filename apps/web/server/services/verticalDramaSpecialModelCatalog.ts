...
function snapshot(model: ModelDefinition, durations: number[]): SpecialModelSnapshot {
  return { modelId: model.id, label: model.name ?? model.id, provider: model.provider, providerModel: model.id, catalogVersion: "media-models-v1", supportedDurationsSeconds: durations, supportedAspectRatios: model.aspectRatios?.length ? model.aspectRatios : ["9:16"], supportsReferenceConditioning: (model.maxReferenceImages ?? 0) > 0, supportsDialogueAudio: model.nativeAudioDialogue === true };
}

export async function listSpecialTieInModels(input: { durationSeconds: SpecialTieInDurationSeconds; dialogueMode: "none" | "character_dialogue"; referenceImageCount?: number }) {
  ...
  const imageModels = images.filter(model => model.isEnabled !== false && (!model.aspectRatios?.length || model.aspectRatios.includes("9:16")) && (model.maxReferenceImages ?? 1) >= referenceImageCount).map(model => snapshot(model, model.durations ?? []));

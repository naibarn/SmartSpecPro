...
function evidenceFromAsset(
  asset: typeof verticalDramaMediaAssets.$inferSelect,
  index: typeof verticalDramaMediaIndexRecords.$inferSelect | undefined,
  score: number
): VerticalDramaMediaEvidence {
  ...
  const silence = Array.isArray(intelligence.silenceSegments)
    ? intelligence.silenceSegments
    : [];
  return {
    ...
    tags: Array.isArray(index?.tagsJson)
      ? index.tagsJson.filter(
          (value): value is string => typeof value === "string"
        )
      : [],
    sourceTimeRanges: scenes
      .flatMap(value => {
        ...
        return [
          {
            startMs,
            endMs:
              rawEnd === null || rawEnd === undefined ? null : Number(rawEnd),
            label: typeof scene.label === "string" ? scene.label : "scene",
          },
        ];
      })
      .slice(0, 256),
    silenceSegments: silence
      ...
      .slice(0, 256),
    subjectLabels: Array.isArray(intelligence.subjects)
      ? intelligence.subjects
          .filter((value): value is string => typeof value === "string")
          .slice(0, 32)
      : [],
    transform:
      intelligence.transform && typeof intelligence.transform === "object"
        ? (intelligence.transform as Record<string, unknown>)
        : null,
  };
}
 ...
 */
export async function retrieveVerticalDramaMediaEvidence(input: {
  tenantId: string;
  seriesId: string;
  query: string;
  limit?: number;
}): Promise<VerticalDramaMediaEvidence[]> {
  ...
  try {
    ...
    const provider =
      resolveVectorProvider("search", providerConfig).provider ===
    ...
    const namespace = vectorizeNamespaceForTenant(input.tenantId);
    const indexName =
    ...
    ranked = (
      ...
      .map(match => ({
        mediaAssetId: String(match.metadata.mediaAssetId ?? "").trim(),
        score: Number(match.score) || 0,
      }))
      .filter(
        item => item.mediaAssetId.length > 0 && item.mediaAssetId.length <= 160
      )
      .slice(0, limit);
  } catch {
  ...
  if (ranked.length === 0) {
    ...
    const predicates = terms.map(term =>
      ilike(verticalDramaMediaIndexRecords.searchableText, `%${term}%`)
    );
    const rows =
      predicates.length > 0
        ? await db
            ...
            .where(
              and(
                eq(verticalDramaMediaIndexRecords.tenantId, input.tenantId),
                eq(verticalDramaMediaIndexRecords.seriesId, seriesId),
                or(...predicates)
              )
            )
            .limit(limit)
    ...
    ranked = rows.map((row, index) => ({
      ...
      score: Math.max(0.01, 1 - index / Math.max(1, limit)),
    }));
  }
  ...
  const [assets, indexes] = await Promise.all([
    db
      ...
      .where(
        and(
          eq(verticalDramaMediaAssets.tenantId, input.tenantId),
          eq(verticalDramaMediaAssets.seriesId, seriesId),
          inArray(verticalDramaMediaAssets.id, assetIds),
          eq(verticalDramaMediaAssets.pipelineState, "published")
        )
      )
      .limit(limit),
    db
      ...
      .where(
        and(
          eq(verticalDramaMediaIndexRecords.tenantId, input.tenantId),
          eq(verticalDramaMediaIndexRecords.seriesId, seriesId),
          inArray(verticalDramaMediaIndexRecords.mediaAssetId, assetIds)
        )
      )
      .limit(limit),
  ]);
  ...
  return ranked.flatMap(item => {
    ...
    return asset
      ? [
          evidenceFromAsset(
            asset,
            indexByAsset.get(item.mediaAssetId),
            item.score
          ),
        ]
      : [];
...
export function projectVerticalDramaMediaEvidence(
  value: VerticalDramaMediaEvidence
): VerticalDramaMediaEvidence {
  return {
    ...value,
    sourceTimeRanges: value.sourceTimeRanges.slice(0, 256),
    silenceSegments: value.silenceSegments.slice(0, 256),
    tags: value.tags.slice(0, 64),
    subjectLabels: value.subjectLabels.slice(0, 32),
  };
}

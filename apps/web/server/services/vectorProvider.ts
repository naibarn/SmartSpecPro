...
export interface VectorProviderConfig {
  ...
  vectorizeKnowledgeIndexName?: string;
  vectorizeMediaIndexName?: string;
  vectorizeAgentMemoryIndexName?: string;
}
...
const effectiveConfigCache = new Map<
  string,
  { expiresAt: number; value: VectorProviderConfig }
>();
...
function applyDefinedSettings(
  target: VectorProviderConfig,
  patch: Partial<VectorProviderConfig>
...
async function loadStoredVectorProviderConfig(params?: {
  tenantId?: string;
}): Promise<Partial<VectorProviderConfig>> {
  ...
  const fromSettings: Partial<VectorProviderConfig> = {
    ...
    vectorizeKnowledgeIndexName: settingsMap.get("vectorizeKnowledgeIndexName"),
    vectorizeMediaIndexName: settingsMap.get("vectorizeMediaIndexName"),
    vectorizeAgentMemoryIndexName: settingsMap.get(
      "vectorizeAgentMemoryIndexName"
    ),
  };
  const governedDefaults: Partial<VectorProviderConfig> = {
    ...fromSettings,
    // The provider setting is configuration/preparation state. A read
    // provider becomes active only through the persisted switch-state row.
    provider: "pgvector",
  ...
  try {
    ...
    const switchRow = extractRows(switchResult)[0] as
      SwitchStateRow | undefined;
    if (!switchRow) {
      return governedDefaults;
    }
    const switchStatus = String(switchRow.status || "")
      .trim()
      .toLowerCase();
    const candidateReadProvider = isProvider(switchRow.current_read_provider)
      ? String(switchRow.current_read_provider)
      : "pgvector";
    const currentReadProvider =
      candidateReadProvider === "pgvector" ||
      switchStatus === "cutover_complete"
        ? candidateReadProvider
        : "pgvector";
    return {
      ...governedDefaults,
      provider: currentReadProvider,
      currentReadProvider,
      targetProvider: switchRow.target_provider
        ? String(switchRow.target_provider)
        : fromSettings.targetProvider,
      mirrorWrites:
...
function getChromaIndexPath(
  indexName: string,
  config?: VectorProviderConfig
): string {
  const configuredPersistDir =
    config?.chromaPersistDir ||
  ...
  return join(
    persistDir,
    `${sanitizeIndexName(indexName)}${CHROMA_INDEX_FILE_SUFFIX}`
  );
}
...
async function readChromaEntries(pathname: string): Promise<VectorEntry[]> {
  try {
    ...
    return parsed
      .filter(
        (entry): entry is Partial<VectorEntry> =>
...
async function writeChromaEntries(
  pathname: string,
  entries: VectorEntry[]
): Promise<void> {
...
async function withChromaFileLock<T>(
  pathname: string,
  fn: () => Promise<T>
): Promise<T> {
...
function requireCloudflareConfig(config?: VectorProviderConfig): {
  accountId: string;
  apiToken: string;
} {
  const accountId =
...
function normalizeCloudflareMatches(
  ...
  return rawMatches.flatMap(rawMatch => {
    ...
    const score =
      typeof match.score === "number" ? match.score : Number(match.score);
    ...
    if (
      ...
      String(metadata.tenantId) !== String(filter.tenantId)
    ) {
...
function normalizeCloudflareVectors(
  rawVectors: unknown
): VectorProviderVectorRecord[] {
  ...
  return rawVectors.map(rawVector => {
    ...
    if (!id)
      throw new Error("Vectorize get_by_ids returned a vector without an ID");
    return { id, metadata: toVectorMetadata(vector.metadata) };
  });
}

function createCloudflareVectorizeAdapter(
  config?: VectorProviderConfig
  ...
  return {
    ...
    async index(params) {
      ...
      try {
        ...
        if (data.success === false)
          throw new Error("Vectorize upsert rejected");
        if (
          !data.result?.mutationId ||
          typeof data.result.mutationId !== "string"
        ) {
          throw new VectorProviderError({
            provider: "cloudflare_vectorize",
        ...
        return {
          ...
          mutationId: data.result.mutationId,
        };
      } catch (error) {
    ...
    async delete(params) {
      ...
      try {
        ...
        if (data.success === false)
          throw new Error("Vectorize delete rejected");
        if (
          !data.result?.mutationId ||
          typeof data.result.mutationId !== "string"
        ) {
    ...
    async search(params) {
      ...
      try {
        ...
        return {
          matches: normalizeCloudflareMatches(
            data.result?.matches,
            params.filter
          ),
        };
    ...
    async getByIds(params) {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(`Vectorize get_by_ids failed: ${response.status}`);
        const data = (await response.json()) as {
          success?: boolean;
          result?: { vectors?: unknown[] } | unknown[];
        };
        if (data.success === false)
          throw new Error("Vectorize get_by_ids rejected");
...
async function getOrCreatePgPool(
  config?: VectorProviderConfig
): Promise<{ pool: PgPoolLike; key: string }> {
  ...
  if (!pool) {
    ...
    const PoolCtor = pg.Pool as unknown as new (
      cfg: PgPoolConfig
    ) => PgPoolLike;
...
function createPgVectorAdapter(
  config?: VectorProviderConfig
): VectorProviderAdapter {
  return {
    ...
    async search(params) {
      try {
        ...
        let sqlText = `SELECT vector_id, embedding, metadata
        ...
        const matches = result.rows
          ...
          .filter(
            row =>
              row.id.length > 0 &&
              metadataMatchesFilter(row.metadata, params.filter)
          )
...
export function resolveVectorProvider(
  ...
  // Keep pgvector authoritative until an explicit, measured Vectorize
  // cutover updates the persisted switch-state.
  const defaultProvider: VectorProvider = "pgvector";
  const configuredProvider = isProvider(config?.provider)
    ? config?.provider
    : undefined;
  const readProvider = isProvider(config?.currentReadProvider)
    ? config?.currentReadProvider
    : undefined;
  const writeProvider = isProvider(config?.targetProvider)
...
export function validateProviderCapabilityRequest(params: {
  ...
  if (request.topK !== undefined) {
    if (
      request.topK < capabilities.minTopK ||
      request.topK > capabilities.maxTopK
    ) {
      throw new VectorProviderError({
  ...
  if (
    ...
    !capabilities.supportedDimensions.includes(request.dimension)
  ) {
    throw new VectorProviderError({
      provider: capabilities.provider,
  ...
  if (
    ...
    !capabilities.supportsMetadataFilter
  ) {
    throw new VectorProviderError({
      provider: capabilities.provider,
...
export function registerVectorProviderAdapter(
  provider: VectorProvider,
  adapter: VectorProviderAdapter
): void {
...
export function clearVectorProviderConfigCache(): void {
...
export async function verifyVectorizeVectorOwnership(params: {
  ...
  if (resolved.provider !== "cloudflare_vectorize" || params.ids.length === 0)
    return [];
  ...
  const records = await adapter.getByIds({
  ...
  if (
    ...
    throw new VectorProviderError({
      ...
      message:
        "Cloudflare Vectorize returned incomplete or unexpected vector ownership evidence",
      classification: "permanent",
    });
  }
...
export async function dispatchVectorOperation(
  params:
    | {
        operation: "index";
        indexName: string;
  ...
  { count: number; mutationId?: string } | { matches: VectorSearchMatch[] }
> {
  const resolved = resolveVectorProvider(
    params.operation,
...
export function getVectorProviderConfigFromEnv(): VectorProviderConfig {
  return {
    ...
    mirrorWrites: ["1", "true", "yes", "on"].includes(
      (process.env.VECTORDB_MIRROR_WRITES || "").toLowerCase()
    ),
    ...
    vectorizeIndexName:
      process.env.VECTORIZE_INDEX_NAME || process.env.VECTORIZE_LIBRARY_INDEX,
    vectorizeKnowledgeIndexName:
      process.env.VECTORIZE_KNOWLEDGE_INDEX ||
      process.env.VECTORIZE_LIBRARY_INDEX,
    vectorizeMediaIndexName: process.env.VECTORIZE_MEDIA_INDEX,
    vectorizeAgentMemoryIndexName: process.env.VECTORIZE_AGENT_MEMORY_INDEX,
  };
}
...
export async function getEffectiveVectorProviderConfig(params?: {
  ...
  const storedConfig = await loadStoredVectorProviderConfig({

...
export type TxRepo = {
  ...
  countActiveJobs?(input: { tenantId: string; executionClass: string }): Promise<number>;
  ...
  assertNoActiveTenantTransfer?(input: { tenantId: string; userId: number }): Promise<void>;
  findScheduleOccurrence?(input: { tenantId: string; scheduleId: string; occurrenceKey: string }): Promise<{ workerJobId: string; definitionHash: string } | null>;
  findAttempt(jobId: string, attempt: number): Promise<{ id: string; leaseGeneration: number } | null>;
  findAttemptDetails?(jobId: string, attempt: number): Promise<{ id: string; leaseGeneration: number; runnerId: string | null } | null>;
  markDispatchConsumed?(input: { jobId: string; attemptId?: string; adapter: string; consumedAt: Date }): Promise<void>;
  findEventByIdempotency(jobId: string, key: string): Promise<{ eventType: string } | null>;
  findAction(actionId: string): Promise<{ workerJobId: string; command: string; reason: string; outcomeJson: Record<string, unknown> } | null>;
  insertAction(values: Record<string, unknown>): Promise<void>;
  updateAction(actionId: string, values: Record<string, unknown>): Promise<void>;
  findCallback(input: { adapterNamespace: string; providerEventId?: string; replayKey?: string }): Promise<{ disposition: string } | null>;
  ...
  assertProviderPollLease?(input: { operationKey: string; pollerLeaseTokenHash: string }): Promise<boolean>;
};
...
function boundedEventKey(prefix: string, ...parts: Array<string | number | undefined>): string {
...
function validateActionId(actionId: string): void {
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(actionId)) {
    throw new JobControlPlaneError("JOB_ACTION_INVALID", "Action identifier is invalid");
  }
}
...
function assertClaimAdapterCompatible(runtimeType: string | null | undefined, adapter: string): void {
  if (!/^[a-z0-9][a-z0-9._-]{0,79}$/.test(adapter)) {
    throw new JobControlPlaneError("JOB_ADAPTER_INVALID", "Job adapter identity is invalid");
  }
  ...
  if (!KNOWN_JOB_ADAPTERS.has(adapter)) {
    ...
    throw new JobControlPlaneError("JOB_ADAPTER_INVALID", "Job adapter is not registered");
  }
  if (!runtimeType) return;
  const allowed = runtimeType === "python_job_worker"
    ? new Set(["postgres-pull", "cloudflare", "cloudflare-queues", "cloudflare-workflows", "cloudflare-containers", "cloudflare-worker-app"])
    : runtimeType === "node_job_worker" || runtimeType === "external_runtime"
      ? new Set(["postgres-pull", "postgres-direct", "bullmq", "cloudflare", "cloudflare-queues", "cloudflare-workflows", "cloudflare-containers", "cloudflare-worker-app"])
      : null;
  if (allowed && !allowed.has(adapter)) {
    throw new JobControlPlaneError("JOB_ADAPTER_RUNTIME_MISMATCH", "Adapter is not permitted for the job runtime");
  }
}

export function canonicalizeStoredStatus(status: string): string {
  switch (status) {
    case "claimed": return "leased";
    case "preparing":
    case "uploading":
    case "publishing":
    case "indexing": return "running";
...
export function compatibilityStatus(status: string): string {
  switch (canonicalizeStoredStatus(status)) {
    ...
    case "running":
    case "waiting_external": return "running";
    case "retry_scheduled": return "queued";
    case "succeeded": return "completed";
    case "cancelled": return "canceled";
    case "pending": return "queued";
    default: return canonicalizeStoredStatus(status);
  }
}
 ...
 */
export function sanitizeJobErrorMessage(value: string, maxLength = 2000, fallback = "unknown_error"): string {
  const sanitized = value
    .replace(/[\u0000-\u001f\u007f]/g, " ")
    .replace(/(bearer\s+)[^\s,;]+/gi, "$1[REDACTED]")
    .replace(/((?:authorization|api[_-]?key|token|password|secret|credential)\s*[:=]\s*)[^\s,;]+/gi, "$1[REDACTED]")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, maxLength);
 ...
 */
export function normalizeResultReference(value: string | undefined): string | null {
  if (value === undefined) return null;
  const reference = value.trim();
  if (!reference || reference.length > 2000 || /[\u0000-\u001f\u007f\s]/.test(reference)) {
    throw new JobControlPlaneError("JOB_RESULT_INVALID", "Result reference is invalid");
  }
  if (/^(?:data|javascript|file):/i.test(reference) || /(?:authorization|api[_-]?key|token|password|secret|credential)\s*=/i.test(reference)) {
    throw new JobControlPlaneError("JOB_RESULT_INVALID", "Result reference is not a durable managed reference");
  }
  try {
    const url = new URL(reference);
    if (url.username || url.password || url.search || url.hash) {
      throw new JobControlPlaneError("JOB_RESULT_INVALID", "Signed or credential-bearing result URLs are not allowed");
    }
    if (reference.includes("://") && !["http:", "https:"].includes(url.protocol)) {
      throw new JobControlPlaneError("JOB_RESULT_INVALID", "Result reference scheme is invalid");
    }
  } catch (error) {
    ...
    if (/^[A-Za-z][A-Za-z0-9+.-]*:/.test(reference) && !/^[A-Za-z][A-Za-z0-9+.-]*:\S+$/.test(reference)) {
      throw new JobControlPlaneError("JOB_RESULT_INVALID", "Result reference is invalid");
    }
  }
  return reference;
...
function transactionRetryCount(): number {
  ...
  return Number.isInteger(configured) ? Math.max(1, Math.min(configured, 5)) : 3;
}
...
function buildDefaultRepository(): JobControlPlaneRepository {
  return {
    async transaction(work) {
      ...
      for (let transactionAttempt = 0; transactionAttempt < maxAttempts; transactionAttempt += 1) {
        try {
          ...
          async lockAdmission(tenantId, executionClass) {
            ...
            await query.execute(sql`SELECT pg_advisory_xact_lock(hashtextextended(${`feature-186-admission:global:${executionClass}`}, 0))`);
            await query.execute(sql`SELECT pg_advisory_xact_lock(hashtextextended(${`feature-186-admission:tenant:${tenantId}:${executionClass}`}, 0))`);
          },
          ...
          async assertNoActiveTenantTransfer({ tenantId, userId }) {
            ...
            await query.execute(sql`SELECT 1 FROM "users" WHERE "id" = ${userId} FOR UPDATE`);
            const rows = await query.execute(sql`
              SELECT 1
            ...
            `);
            if (rows.length > 0) {
              throw new JobControlPlaneError("TRANSFER_IN_PROGRESS", "User has an active tenant transfer");
            }
            ...
            if (identityFence.length > 0) {
              throw new JobControlPlaneError("TRANSFER_IN_PROGRESS", "User has an active tenant identity move");
            }
          },
          async findScheduleOccurrence({ tenantId, scheduleId, occurrenceKey }) {
            const [row] = await query.select({
              ...
              eq(workerJobScheduleOccurrences.occurrenceKey, occurrenceKey),
            )).limit(1);
            return row ?? null;
          },
          ...
          async markDispatchConsumed({ jobId, attemptId, adapter, consumedAt }) {
            const conditions = [
            ...
            await query.update(workerJobDispatches).set({
              consumedAt,
          ...
          async findAction(actionId) {
            ...
            }).from(workerJobActions).where(eq(workerJobActions.actionId, actionId)).limit(1);
            return row ?? null;
          },
          async insertAction(values) {
            await query.insert(workerJobActions).values(values).onConflictDoNothing();
          },
          async updateAction(actionId, values) {
            await query.update(workerJobActions).set(values).where(eq(workerJobActions.actionId, actionId));
          },
          async findCallback(input) {
            const predicates = [eq(workerJobCallbacks.adapterNamespace, input.adapterNamespace)];
            if (input.providerEventId) predicates.push(eq(workerJobCallbacks.providerEventId, input.providerEventId));
            else if (input.replayKey) predicates.push(eq(workerJobCallbacks.replayKey, input.replayKey));
            else return null;
            const [row] = await query.select({ disposition: workerJobCallbacks.disposition })
              .from(workerJobCallbacks).where(and(...predicates)).limit(1);
            return row ?? null;
          },
          async insertCallback(values) {
            const rows = await query.insert(workerJobCallbacks)
              .values(values)
              .onConflictDoNothing()
          ...
          async insertScheduleOccurrence(values) {
            const rows = await query.insert(workerJobScheduleOccurrences).values(values).onConflictDoNothing().returning({ id: workerJobScheduleOccurrences.id });
            return rows.length > 0;
          },
          ...
          async assertProviderPollLease({ operationKey, pollerLeaseTokenHash }) {
            const rows = await query.execute(sql`
              SELECT 1
          ...
          if (!isRetryableTransactionError(error) || transactionAttempt + 1 >= maxAttempts) throw error;
...
export async function appendJobEvent(
  ...
  if (!input.eventType || input.eventType.length > 100 || !input.eventIdempotencyKey || input.eventIdempotencyKey.length > 200) {
    throw new JobControlPlaneError("JOB_EVENT_INVALID", "Lifecycle event identity is invalid");
  }
  const safePayload = omitUndefinedEventValues(input.payloadJson ?? {}) as Record<string, unknown>;
  ...
  if (existingEventType) {
    if (existingEventType !== input.eventType) {
      throw new JobControlPlaneError("JOB_EVENT_IDEMPOTENCY_CONFLICT", "Event key was already used for another event type");
    }
    return;
  }
...
export async function createCanonicalJobInTransaction(input: {
  ...
  const normalizedDefinition = input.definition.idempotencyKey !== undefined
    ? { ...input.definition, idempotencyKey: normalizeIdempotencyKey(input.definition.idempotencyKey) }
    : input.definition;
  ...
  if (!/^[a-zA-Z0-9_-]{1,36}$/.test(jobId)) {
    throw new JobControlPlaneError("JOB_ID_INVALID", "Canonical job ID is invalid");
  }
  const [collision] = await input.query.select({ id: workerJobs.id }).from(workerJobs).where(eq(workerJobs.id, jobId)).limit(1);
  if (collision) throw new JobControlPlaneError("JOB_ID_CONFLICT", "Canonical job ID is already bound to another job");
  ...
  const now = new Date();
  const [row] = await input.query.insert(workerJobs).values({
    id: jobId,
    tenantId: normalizedDefinition.tenantId,
    requestedByUserId: normalizedDefinition.requestedByUserId ?? null,
    requestedBySystemComponent: input.options?.requestedBySystemComponent ?? null,
  ...
  if (!row) throw new JobControlPlaneError("JOB_CREATE_FAILED", "Canonical job could not be created");
...
export async function cancelQueuedJobInTransaction(input: {
  ...
  const safeReason = sanitizeJobErrorMessage(input.reason, 2000, "cancelled_by_request");
  const [job] = await input.query.select().from(workerJobs)
    .where(eq(workerJobs.id, input.jobId)).for("update").limit(1);
  assertJobMutationScope(job ?? null, input.scope);
  if (!job) throw new JobControlPlaneError("JOB_NOT_FOUND", "Job was not found");
  ...
  if (!["pending", "queued", "retry_scheduled"].includes(job.status)) {
    throw new JobControlPlaneError("ACTIVE_JOB_BLOCKED", "Active canonical work must finish before it can be cancelled");
  }

  const [existing] = await input.query.select().from(workerJobActions)
    .where(eq(workerJobActions.actionId, input.actionId)).limit(1);
  if (existing && (existing.workerJobId !== job.id || existing.command !== "cancel" || existing.reason !== safeReason.slice(0, 500))) {
    throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
  }
  if (existing && (existing.outcomeJson as Record<string, unknown>)?.accepted === true) return "ignored";
  if (!existing) {
    await input.query.insert(workerJobActions).values({
      id: randomUUID(), actionId: input.actionId, workerJobId: job.id, command: "cancel",
      actorId: input.actorId ?? null, reason: safeReason.slice(0, 500),
      expectedStatus: job.status, expectedAttempt: job.attempt,
      expectedFencingVersion: job.fencingVersion, authorizationScope: input.scope?.authorizationScope ?? "feature-189-transfer",
      outcomeJson: {},
    }).onConflictDoNothing();
  }
  const [winner] = await input.query.select().from(workerJobActions)
    .where(eq(workerJobActions.actionId, input.actionId)).limit(1);
  if (!winner || winner.workerJobId !== job.id || winner.command !== "cancel" || winner.reason !== safeReason.slice(0, 500)) {
    throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
  }

  const now = new Date();
  const updated = await input.query.update(workerJobs).set({
    status: "cancelled", statusReason: safeReason, leaseOwnerToken: null,
    leaseExpiresAt: null, finishedAt: now,
  }).where(and(
    eq(workerJobs.id, job.id), eq(workerJobs.status, job.status),
    eq(workerJobs.attempt, job.attempt), eq(workerJobs.fencingVersion, job.fencingVersion),
  )).returning({ id: workerJobs.id });
  if (updated.length === 0) throw new JobControlPlaneError("JOB_STATE_CONFLICT", "Job changed while cancelling");
  await input.query.update(workerJobOutbox).set({
    cancelledAt: now, failedReason: `cancelled:${safeReason}`,
    publisherLeaseTokenHash: null, publisherLeaseExpiresAt: null, updatedAt: now,
  }).where(and(eq(workerJobOutbox.workerJobId, job.id), isNull(workerJobOutbox.publishedAt), isNull(workerJobOutbox.cancelledAt)));
  const [attempt] = await input.query.select({ id: workerJobAttempts.id }).from(workerJobAttempts)
    .where(and(eq(workerJobAttempts.workerJobId, job.id), eq(workerJobAttempts.attempt, job.attempt))).limit(1);
  if (attempt) await input.query.update(workerJobAttempts).set({ finishedAt: now, terminalClass: "cancelled", recoveryReason: safeReason.slice(0, 500) }).where(eq(workerJobAttempts.id, attempt.id));
  const keyPrefix = `operator:${input.actionId}`;
  await appendJobEvent(input.query, {
    workerJobId: job.id, eventType: "OPERATOR_ACTION", attemptId: attempt?.id,
    eventIdempotencyKey: `${keyPrefix}:action`,
    payloadJson: { action: "cancel", reason: safeReason.slice(0, 500), actorId: input.actorId, targetAttempt: job.attempt, targetStatus: job.status },
  });
  await appendJobEvent(input.query, {
    workerJobId: job.id, eventType: "CANCEL_REQUESTED", attemptId: attempt?.id,
    eventIdempotencyKey: `${keyPrefix}:requested`, payloadJson: { reason: safeReason.slice(0, 500), actionId: input.actionId, actorId: input.actorId },
  });
  await appendJobEvent(input.query, {
    workerJobId: job.id, eventType: "CANCELLED", attemptId: attempt?.id,
    eventIdempotencyKey: `${keyPrefix}:cancelled`, payloadJson: { reason: safeReason.slice(0, 500), actionId: input.actionId, actorId: input.actorId },
  });
  await input.query.update(workerJobActions).set({ outcomeJson: { accepted: true, phase: "cancelled", jobId: job.id }, effectiveAt: now }).where(eq(workerJobActions.id, winner.id));
  return "cancelled";
}
...
export async function resumeReviewGatedJobInTransaction(input: {
  ...
  validateActionId(input.actionId);
  const safeReason = sanitizeJobErrorMessage(input.reason, 500, "authorized_resume");
  const [job] = await input.query.select().from(workerJobs)
    .where(eq(workerJobs.id, input.jobId)).for("update").limit(1);
  assertJobMutationScope(job ?? null, input.scope);
  if (!job) throw new JobControlPlaneError("JOB_NOT_FOUND", "Job was not found");
  if (input.expectedJobType && job.jobType !== input.expectedJobType) {
    throw new JobControlPlaneError("JOB_STATE_CONFLICT", "Job type cannot be resumed by this command");
  }
  if (job.status !== "retry_scheduled" || !job.operatorReviewRequired) {
    throw new JobControlPlaneError("OPERATOR_REVIEW_REQUIRED", "Only a review-gated retry-scheduled job can be resumed");
  }
  ...
  if (updated.length === 0) throw new JobControlPlaneError("JOB_STATE_CONFLICT", "Job changed while resuming");

  const [pendingOutbox] = await input.query.select({ id: workerJobOutbox.id })
    ...
    .orderBy(sql`${workerJobOutbox.createdAt} DESC`).limit(1);
  if (pendingOutbox) {
    await input.query.update(workerJobOutbox).set({
      nextAttemptAt: now,
      failedReason: null,
  ...
  const [attempt] = await input.query.select({ id: workerJobAttempts.id })
    .from(workerJobAttempts)
    .where(and(eq(workerJobAttempts.workerJobId, job.id), eq(workerJobAttempts.attempt, job.attempt)))
    .limit(1);
  await appendJobEvent(input.query, {
    workerJobId: job.id,
    eventType: "RECOVERED",
  ...
  await appendJobEvent(input.query, {
    ...
    payloadJson: { attempt: job.attempt, reason: safeReason, actionId: input.actionId },
  });
...
async function prepareOperatorAction(
  ...
  validateActionId(input.actionId);
  const normalizedReason = sanitizeJobErrorMessage(input.reason, 500, "unspecified");
  const existing = await repo.findAction(input.actionId);
  if (existing) {
    if (existing.workerJobId !== input.jobId || existing.command !== input.command || existing.reason !== normalizedReason) {
      throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
    }
    return false;
  }
  await repo.insertAction({
    ...
    authorizationScope: input.authorizationScope ?? "job-control-plane.internal",
    outcomeJson: {},
  });
  // A concurrent transaction may have won the unique action key race.
  const inserted = await repo.findAction(input.actionId);
  if (inserted && (inserted.workerJobId !== input.jobId || inserted.command !== input.command || inserted.reason !== normalizedReason)) {
    throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
  }
  return true;
}

async function finishOperatorAction(repo: TxRepo, actionId: string, outcome: Record<string, unknown>): Promise<void> {
  validateActionId(actionId);
  await repo.updateAction(actionId, { outcomeJson: redactJobPayload(outcome) as Record<string, unknown>, effectiveAt: new Date() });
}
...
function callbackReplayWindowMs(): number {
  const configured = Number(process.env.FEATURE_186_CALLBACK_REPLAY_WINDOW_SECONDS);
  const seconds = Number.isFinite(configured) ? Math.max(30, Math.min(configured, 86_400)) : 900;
  return seconds * 1000;
}
...
export async function recordAuthenticatedJobCallback(
  ...
  if (!input.signatureVerified) throw new JobControlPlaneError("CALLBACK_REJECTED", "Callback signature was not verified");
  if (!input.adapterNamespace || input.adapterNamespace.length > 120) throw new JobControlPlaneError("CALLBACK_INVALID", "Callback adapter namespace is invalid");
  ...
  if (!callbackKey || (providerEventId && replayKey) || callbackKey.length > 255) throw new JobControlPlaneError("CALLBACK_INVALID", "Callback requires exactly one bounded provider event or replay key");
  const occurredAt = new Date(input.occurredAt);
  if (!Number.isFinite(occurredAt.getTime())) throw new JobControlPlaneError("CALLBACK_INVALID", "Callback timestamp is invalid");
  const ageMs = Date.now() - occurredAt.getTime();
  const clockSkewMs = 5 * 60 * 1000;
  if (ageMs > callbackReplayWindowMs() || ageMs < -clockSkewMs) {
    throw new JobControlPlaneError("CALLBACK_REPLAY_WINDOW", "Callback timestamp is outside the accepted replay window");
  }
  ...
  return repository.transaction(async repo => {
    ...
    if (duplicate) return {
      ...
      originalDisposition: duplicate.disposition === "accepted" || duplicate.disposition === "rejected" ? duplicate.disposition : undefined,
      callbackIdempotencyKey: `${input.adapterNamespace}:${callbackKey}`,
    };

    const job = input.jobId ? await repo.findJob(input.jobId) : null;
    const correlated = Boolean(job && (!input.tenantId || job.tenantId === input.tenantId));
    ...
    if (!inserted) {
      ...
      return {
        disposition: "duplicate",
        originalDisposition: raced?.disposition === "accepted" || raced?.disposition === "rejected" ? raced.disposition : undefined,
        callbackIdempotencyKey: `${input.adapterNamespace}:${callbackKey}`,
      };
    }
    if (job && correlated) {
      await repo.insertEvent({
        workerJobId: job.id,
        eventType: disposition === "accepted" ? "CALLBACK_ACCEPTED" : "CALLBACK_REJECTED",
        eventIdempotencyKey: boundedEventKey("callback", input.adapterNamespace, callbackKey),
        payloadJson: { adapterNamespace: input.adapterNamespace, disposition },
      });
    }
    return { disposition, callbackIdempotencyKey: `${input.adapterNamespace}:${callbackKey}` };
  });
}
...
const DEFAULT_ADMISSION_LIMITS: Record<string, { perTenant: number; global: number }> = {
...
function admissionLimit(executionClass: string, scope: "perTenant" | "global"): number {
  const envName = `FEATURE_186_MAX_CONCURRENT_${scope === "perTenant" ? "PER_TENANT" : "GLOBAL"}_${executionClass.toUpperCase()}`;
  const configured = Number(process.env[envName]);
  if (Number.isInteger(configured) && configured > 0 && configured <= 100_000) return configured;
  return DEFAULT_ADMISSION_LIMITS[executionClass]?.[scope] ?? DEFAULT_ADMISSION_LIMITS.short[scope];
}

async function enforceAdmission(
  repo: TxRepo,
  input: { tenantId: string; executionClass: string; admissionMode?: CreateJobOptions["admissionMode"] },
): Promise<void> {
  ...
  if (!repo.lockAdmission || !repo.countActiveJobs || !repo.countActiveJobsGlobal) return;
...
export async function enforceCanonicalJobAdmissionInTransaction(
  ...
  if (input.admissionMode === "durable_queue") return;
  await query.execute(sql`SELECT pg_advisory_xact_lock(hashtextextended(${`feature-186-admission:global:${input.executionClass}`}, 0))`);
  await query.execute(sql`SELECT pg_advisory_xact_lock(hashtextextended(${`feature-186-admission:tenant:${input.tenantId}:${input.executionClass}`}, 0))`);
  ...
  if (tenantActive >= tenantLimit || globalActive >= globalLimit) {
    throw new JobControlPlaneError(
      "JOB_ADMISSION_BACKPRESSURE",
      "Job admission capacity is temporarily exhausted",
      { executionClass: input.executionClass, tenantActive, tenantLimit, globalActive, globalLimit },
    );
  }
}
...
async function assertNoActiveTenantTransferInTransaction(
  ...
  await query.execute(sql`SELECT 1 FROM "users" WHERE "id" = ${input.userId} FOR UPDATE`);
  const rows = await query.execute(sql`
    SELECT 1
    FROM "tenant_data_transfer_plans" p
    INNER JOIN "worker_jobs" j ON j."id" = p."operationId"
...
function assertJobMutationScope(job: WorkerJob | null, scope?: JobMutationScope): void {
  if (!scope || !job) return;
  if (job.tenantId !== scope.tenantId) {
    throw new JobControlPlaneError("JOB_NOT_FOUND", "Job is outside the authorized tenant scope");
  }
  if (scope.requestedByUserId !== undefined && job.requestedByUserId !== scope.requestedByUserId) {
    throw new JobControlPlaneError("JOB_NOT_FOUND", "Job is outside the authorized actor scope");
  }
}
  ...
  return {
    ...
    async getStatus(jobId: string, scope?: JobMutationScope) {
      return repository.transaction(async repo => {
        ...
        if (!job) return null;
        assertJobMutationScope(job, scope);
        const attemptDetails = await repo.findAttemptDetails?.(job.id, job.attempt);
        const now = new Date();
        const canonicalStatus = canonicalizeStoredStatus(job.status);
        const hasLease = Boolean(job.leaseExpiresAt && job.leaseExpiresAt.getTime() > now.getTime());
        const stale = Boolean(
          job.leaseExpiresAt
        ...
        return {
          ...
          errorMessage: (job.errorMessage ?? job.failureReason ?? job.statusReason)
            ? sanitizeJobErrorMessage(job.errorMessage ?? job.failureReason ?? job.statusReason ?? "unknown_error")
            : null,
          progress: redactJobPayload(job.progressJson ?? {}) as Record<string, unknown>,
          output: job.outputJson ? redactJobPayload(job.outputJson) as Record<string, unknown> : null,
          ...
          terminal: ["succeeded", "failed", "cancelled", "expired"].includes(canonicalStatus),
        };
      });
    },
     ...
     */
    async rejectUnsupportedDelivery(input: { jobId: string; reason: string; contractVersion?: string }): Promise<boolean> {
      return repository.transaction(async repo => {
        const job = await repo.findJob(input.jobId);
        if (!job || ["succeeded", "failed", "cancelled", "expired"].includes(job.status)) return false;
        const safeReason = sanitizeJobErrorMessage(input.reason, 500, "unsupported_delivery");
        ...
        await repo.insertEvent({
          ...
          payloadJson: { reason: safeReason, contractVersion: input.contractVersion ?? "unknown", source: "consumer" },
        });
        return true;
      });
    },
    ...
    async create(definition: JobDefinition, options: CreateJobOptions = {}): Promise<JobRef> {
      const normalizedDefinition = definition.idempotencyKey !== undefined
      ...
      const createOnce = () => repository.transaction(async repo => {
        if (normalizedDefinition.schedule && repo.findScheduleOccurrence) {
          ...
          if (existingOccurrence) {
            if (existingOccurrence.definitionHash !== definitionHash) {
              throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Schedule occurrence was already used for another job definition");
            }
        ...
        if (canonicalIdCollision) {
          throw new JobControlPlaneError("JOB_ID_CONFLICT", "Canonical job ID is already bound to another job");
        }
        ...
        if (normalizedDefinition.requestedByUserId !== undefined && repo.assertNoActiveTenantTransfer) {
        ...
        if (normalizedDefinition.schedule) {
          if (!repo.insertScheduleOccurrence || !repo.findScheduleOccurrence) {
            throw new JobControlPlaneError("SCHEDULE_PERSISTENCE_UNAVAILABLE", "Scheduled job persistence is not available");
          }
          ...
          if (!insertedOccurrence) {
            throw new JobControlPlaneError("SCHEDULE_OCCURRENCE_RACE", "Schedule occurrence was created concurrently");
          }
        }
        return { jobId: row.id, created: true };
      });
      try {
        ...
        if ((error as { code?: string })?.code === "SCHEDULE_OCCURRENCE_RACE" && normalizedDefinition.schedule && repository) {
          return repository.transaction(async repo => {
            ...
            if (winner.definitionHash !== definitionHash) throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Schedule occurrence was already used for another job definition");
    ...
    async claim(input: { jobId: string; runnerId: string; adapter: string; attemptId?: string }): Promise<LeaseContext | null> {
      return repository.transaction(async repo => {
        const job = await repo.findJob(input.jobId);
     ...
     */
    async holdQueued(jobId: string, operationKey: string, reason = "paused_by_domain"): Promise<boolean> {
      if (!operationKey.trim() || operationKey.length > 200) {
        throw new JobControlPlaneError("JOB_EXTERNAL_WAIT_INVALID", "External operation key is invalid");
      }
      const safeReason = sanitizeJobErrorMessage(reason, 500, "paused_by_domain");
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        if (!job) return false;
        if (job.status === "waiting_external") {
          const progress = job.progressJson && typeof job.progressJson === "object" && !Array.isArray(job.progressJson)
            ? job.progressJson as { externalWait?: { operationKey?: unknown } }
            : undefined;
        ...
        const updated = await repo.updateJob({
          ...
          values: {
            ...
            progressJson: {
              ...previousProgress,
              externalWait: { operationKey, reason: safeReason, resumeAfter: "9999-12-31T00:00:00.000Z" },
            },
            updatedAt: now,
          },
        });
        if (!updated) return false;
        await repo.cancelUnpublishedOutbox({ jobId, reason: safeReason, cancelledAt: now });
        await repo.insertEvent({
          workerJobId: jobId,
          eventType: "WAITING_EXTERNAL",
          eventIdempotencyKey: boundedEventKey("queued-hold", jobId, job.attempt, job.status, operationKey),
          payloadJson: { reason: safeReason, operationKey, source: "domain_pause" },
        });
        return true;
      });
    },
     ...
     */
    async markProviderSubmissionReview(jobId: string, operationKey: string, reason: string, now = new Date()): Promise<boolean> {
      if (!operationKey.trim() || operationKey.length > 200) {
        throw new JobControlPlaneError("JOB_EXTERNAL_WAIT_INVALID", "Provider operation key is invalid");
      }
      const safeReason = sanitizeJobErrorMessage(reason, 500, "provider_submission_ambiguous");
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        if (!job) return false;
        ...
        if (!updated) return false;
        await repo.cancelUnpublishedOutbox({ jobId, reason: "provider_submission_ambiguous", cancelledAt: now });
        await repo.insertEvent({
          workerJobId: jobId,
          eventType: "FAILED",
     ...
     */
    async reportLegacyStatus(lease: LeaseContext, legacyStatus: Record<string, unknown>): Promise<void> {
      ...
      const progress = Number.isFinite(rawProgress)
        ...
        : (String(legacyStatus.status ?? "").toLowerCase() === "completed" ? 100 : 0);
      const stageValue = legacyStatus.phase ?? legacyStatus.stage ?? legacyStatus.status ?? "running";
      const stage = String(stageValue).trim().slice(0, 100) || "running";
      const message = typeof legacyStatus.message === "string" ? legacyStatus.message.slice(0, 500) : undefined;
      const safeLegacyStatus = redactJobPayload(legacyStatus) as Record<string, unknown>;
      const eventDigest = createHash("sha256").update(JSON.stringify(safeLegacyStatus), "utf8").digest("hex").slice(0, 32);
      await repository.transaction(async repo => {
        const job = await repo.findJob(lease.jobId);
        await assertLeaseAttempt(repo, lease, job);
        const updated = await repo.updateJob({
          ...
          expectedFencingVersion: lease.fencingVersion,
          values: {
            progressJson: { progress, stage, ...(message ? { message } : {}), legacyStatus: safeLegacyStatus },
          },
        });
        if (!updated) throw new JobControlPlaneError("JOB_LEASE_STALE", "Job lease is no longer active");
        await repo.insertEvent({
    ...
    async waitForExternal(lease: LeaseContext, input: { operationKey: string; providerReference?: string; resumeAfter: string }): Promise<void> {
      ...
      const normalizedProviderReference = providerReference ? normalizeResultReference(providerReference) : null;
      if (normalizedProviderReference && normalizedProviderReference.length > 255) throw new JobControlPlaneError("JOB_EXTERNAL_WAIT_INVALID", "External provider reference is invalid");
      ...
      const safeExternalWait = {
        ...
        ...(normalizedProviderReference ? { providerReference: normalizedProviderReference } : {}),
      };
      await repository.transaction(async repo => {
        const job = await repo.findJob(lease.jobId);
        await assertLeaseAttempt(repo, lease, job);
        ...
        const updated = await repo.updateJob({
          ...
          values: {
            ...
            progressJson: { ...previousProgress, externalWait: safeExternalWait },
          },
        });
        if (!updated) throw new JobControlPlaneError("JOB_LEASE_STALE", "Job lease is no longer active");
        await repo.insertEvent({
          ...
          eventType: "WAITING_EXTERNAL",
          attemptId: lease.attemptId,
          eventIdempotencyKey: boundedEventKey("waiting-external", lease.attemptId, input.operationKey),
    ...
    async resumeExternal(
      ...
      if (inputJson !== undefined) validateBoundedPayload(inputJson, "externalResume.input");
      const normalizedResumeKey = resumeKey.trim();
      if (!normalizedResumeKey || normalizedResumeKey.length > 255) {
        throw new JobControlPlaneError("JOB_EXTERNAL_RESUME_INVALID", "External resume key is invalid");
      }
      const resumeDigest = createHash("sha256").update(normalizedResumeKey, "utf8").digest("hex").slice(0, 32);
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        const progress = job?.progressJson && typeof job.progressJson === "object" && !Array.isArray(job.progressJson)
          ? job.progressJson as { externalWait?: { operationKey?: unknown } }
          : undefined;
        const operationKey = typeof progress?.externalWait?.operationKey === "string" ? progress.externalWait.operationKey : "unknown";
        const operationDigest = createHash("sha256").update(operationKey, "utf8").digest("hex").slice(0, 32);
        ...
        const nextAttempt = advanceAttempt ? previousAttemptNumber + 1 : previousAttemptNumber;
        ...
        if (!updated) return false;
        const previousAttempt = await repo.findAttempt(jobId, previousAttemptNumber);
        const nextAttemptId = advanceAttempt ? randomUUID() : previousAttempt?.id;
        if (advanceAttempt && previousAttempt) {
          await repo.updateAttempt({ attemptId: previousAttempt.id, values: { finishedAt: new Date(), terminalClass: "retryable", recoveryReason: "external_resume_retry" } });
        }
        ...
        await repo.insertEvent({ workerJobId: jobId, eventType: "RECOVERED", attemptId: previousAttempt?.id, eventIdempotencyKey: eventKey, payloadJson: { runnerId, adapter, resumeKey: resumeDigest, previousAttempt: previousAttemptNumber, nextAttempt } });
        await repo.insertEvent({ workerJobId: jobId, eventType: "DISPATCH_REQUESTED", attemptId: nextAttemptId, eventIdempotencyKey: `dispatch-requested:external-resume:${jobId}:${nextAttempt}:${resumeDigest}`, payloadJson: { attempt: nextAttempt, reason: "external_resumed", previousAttempt: advanceAttempt ? previousAttemptNumber : undefined } });
        await repo.insertOutbox({
          workerJobId: jobId,
          attemptId: nextAttemptId,
          envelopeVersion: job.contractVersion,
          envelopeJson: { jobId, businessAttempt: nextAttempt, contractVersion: job.contractVersion, ...(nextAttemptId ? { attemptId: nextAttemptId } : {}), reason: "external_resumed", resumeKey: resumeDigest },
    ...
    async complete(lease: LeaseContext, result: JobResult): Promise<void> {
      ...
      const safeResult = { ...result, resultRef: resultRef ?? undefined, output: redactJobPayload(result.output ?? {}) as Record<string, unknown> };
      await guardedLeaseUpdate(repository, lease, "running", "succeeded", "COMPLETED", safeResult, {
        resultRef,
      ...
      await repository.transaction(async repo => {
        ...
        const updated = await repo.updateJob({
          ...
          values: {
            ...
            operatorReviewReason: error.class === "unknown" || error.operatorReviewRequired === true ? sanitizeJobErrorMessage(error.message, 500) : null,
            leaseOwnerToken: null,
            leaseExpiresAt: null,
        ...
        if (shouldRetry && nextAttemptId) {
          ...
          await repo.insertOutbox({
            ...
            attemptId: nextAttemptId,
            envelopeVersion: job.contractVersion,
            envelopeJson: { jobId: job.id, businessAttempt: nextAttempt, contractVersion: job.contractVersion, attemptId: nextAttemptId },
    ...
    async requestCancel(jobId: string, reason = "cancelled_by_request", actionId?: string, actorId?: number, scope?: JobMutationScope): Promise<void> {
      ...
      const safeReason = sanitizeJobErrorMessage(reason, 2000, "cancelled_by_request");
      await repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        assertJobMutationScope(job, scope);
        ...
        if (actionId && job) {
          ...
          if (existingAction && (existingAction.workerJobId !== jobId || existingAction.command !== "cancel" || existingAction.reason !== safeReason.slice(0, 500))) {
            throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
          }
        ...
        if (actionId && !(await prepareOperatorAction(repo, { actionId, jobId, command: "cancel", reason: safeReason, actorId, authorizationScope: scope?.authorizationScope, job }))) return;
        const updated = await repo.updateJob({
          jobId,
          expectedStatus: job.status,
          ...
          expectedAttempt: job.attempt,
          values: { statusReason: `cancel_requested:${safeReason}`, leaseOwnerToken: null, leaseExpiresAt: null },
        });
        if (!updated) throw new JobControlPlaneError("JOB_STATE_CONFLICT", "Job changed while requesting cancellation");
        if (actionId) await repo.insertEvent({ workerJobId: jobId, eventType: "OPERATOR_ACTION", eventIdempotencyKey: `${keyPrefix}:action`, payloadJson: { action: "cancel", reason: safeReason.slice(0, 500), actorId, targetAttempt: job.attempt, targetStatus: job.status } });
        await repo.insertEvent({ workerJobId: jobId, eventType: "CANCEL_REQUESTED", attemptId: (await repo.findAttempt(jobId, job.attempt))?.id, eventIdempotencyKey: `${keyPrefix}:requested`, payloadJson: { reason: safeReason.slice(0, 500), actionId, actorId } });
        if (actionId) await finishOperatorAction(repo, actionId, { accepted: true, phase: "requested", jobId });
      });
    },
    ...
    async finalizeCancel(jobId: string, reason = "cancelled_by_request", actionId?: string, actorId?: number, scope?: JobMutationScope): Promise<void> {
      ...
      await repository.transaction(async repo => {
        ...
        if (job.status === "cancelled") {
          if (actionId) await finishOperatorAction(repo, actionId, { accepted: true, phase: "cancelled", jobId });
          return;
        }
        ...
        const updated = await repo.updateJob({ jobId, expectedStatus: job.status, expectedTenantId: scope?.tenantId, expectedRequestedByUserId: scope?.requestedByUserId, expectedAttempt: job.attempt, values: { status: "cancelled", statusReason: safeReason, leaseOwnerToken: null, leaseExpiresAt: null, finishedAt: new Date() } });
        if (!updated) throw new JobControlPlaneError("JOB_STATE_CONFLICT", "Job changed while finalizing cancellation");
        await repo.cancelUnpublishedOutbox({ jobId, reason: `cancelled:${safeReason}`, cancelledAt: new Date() });
        const currentAttempt = await repo.findAttempt(jobId, job.attempt);
        if (currentAttempt) await repo.updateAttempt({ attemptId: currentAttempt.id, values: { finishedAt: new Date(), terminalClass: "cancelled", recoveryReason: safeReason.slice(0, 500) } });
        await repo.insertEvent({ workerJobId: jobId, eventType: "CANCELLED", attemptId: currentAttempt?.id, eventIdempotencyKey: `${keyPrefix}:cancelled`, payloadJson: { reason: safeReason.slice(0, 500), actionId, actorId } });
      });
    },
    ...
    async makeRetryDue(jobId: string, actionId?: string, actorId?: number, reason = "retry_due", scope?: JobMutationScope): Promise<boolean> {
      ...
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        const existingAction = actionId ? await repo.findAction(actionId) : null;
        if (existingAction) {
          if (existingAction.workerJobId !== jobId || existingAction.command !== "requeue" || existingAction.reason !== safeReason.slice(0, 500)) {
            throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
          }
          assertJobMutationScope(job, scope);
          return true;
        ...
        if (actionId && !(await prepareOperatorAction(repo, { actionId, jobId, command: "requeue", reason: safeReason, actorId, authorizationScope: scope?.authorizationScope, job }))) return true;
        ...
        const updated = await repo.updateJob({ jobId, expectedStatus: "retry_scheduled", expectedTenantId: scope?.tenantId, expectedRequestedByUserId: scope?.requestedByUserId, expectedAttempt: job.attempt, values: { status: "queued", nextRetryAt: null, operatorReviewRequired: false, operatorReviewReason: null } });
        if (!updated) return false;
        if (existingOutbox && !existingOutbox.publishedAt) {
          ...
          await repo.insertOutbox({
            ...
            envelopeVersion: job.contractVersion,
            envelopeJson: { jobId, businessAttempt: job.attempt, contractVersion: job.contractVersion, attemptId: currentAttempt?.id, reason: actionId ? "operator_requeue" : "retry_due" },
            dedupeKey: `job:${jobId}:attempt:${job.attempt}`,
            nextAttemptAt: new Date(),
          });
        }
        if (actionId) await repo.insertEvent({ workerJobId: jobId, eventType: "OPERATOR_ACTION", eventIdempotencyKey: `operator:${actionId}:action`, payloadJson: { action: "requeue", reason: safeReason.slice(0, 500), actorId, targetAttempt: job.attempt, targetStatus: job.status } });
        await repo.insertEvent({ workerJobId: jobId, eventType: "RECOVERED", eventIdempotencyKey: actionId ? `operator:${actionId}:requeue` : `retry-due:${jobId}:${job.attempt}`, payloadJson: { attempt: job.attempt, actionId, reason: safeReason.slice(0, 500) } });
        if (actionId) await finishOperatorAction(repo, actionId, { accepted: true, phase: "requeued", jobId });
        return true;
      });
    },
    ...
    async recoverCheckpoint(
      ...
      if (!actionId || !reason.trim() || reason.length > 500) {
        throw new JobControlPlaneError("JOB_ACTION_INVALID", "Checkpoint recovery action is invalid");
      }
      validateActionId(actionId);
      if (!/^[a-f0-9]{32,64}$/i.test(evidence.checkpointDigest)) {
        throw new JobControlPlaneError("JOB_RECOVERY_EVIDENCE_INVALID", "Checkpoint recovery evidence is invalid");
      }
      const safeReason = sanitizeJobErrorMessage(reason, 500, "checkpoint_recovery");
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        ...
        assertJobMutationScope(job, scope);
        if (!job || job.jobType !== "vertical_drama.story" || job.status !== "failed") return false;
        if (!job.operatorReviewRequired) return false;
        if (job.attempt >= job.maxAttempts) return false;

        if (!(await prepareOperatorAction(repo, {
          actionId,
          jobId,
        ...
        if (!updated) throw new JobControlPlaneError("JOB_STATE_CONFLICT", "Job changed while recovering checkpoint");

        const previousAttempt = await repo.findAttempt(jobId, job.attempt);
        if (previousAttempt) {
          await repo.updateAttempt({
            attemptId: previousAttempt.id,
            values: { finishedAt: now, terminalClass: "recovered", recoveryReason: normalizedReason },
          });
        }
        await repo.insertAttempt({
          id: nextAttemptId,
        ...
        const payload = {
          ...
          ...(evidence.completedEpisodeCount !== undefined ? { completedEpisodeCount: evidence.completedEpisodeCount } : {}),
        };
        await repo.insertEvent({
          workerJobId: jobId,
    ...
    /**
     * Recover a terminal, operator-reviewed job without creating a replacement
     * canonical job. This is the generic counterpart to checkpoint recovery
     * for domains whose durable progress is stored outside the control-plane
     * service. The action key and the new attempt/outbox are committed in the
     * same transaction so a repeated repair request cannot double-dispatch.
     */
    async recoverReviewGatedJob(
      jobId: string,
      actionId: string,
      reason: string,
      evidence: { disposition: "pre_submission_failure" | "provider_operation_resolved"; operationKey?: string },
      actorId?: number,
      scope?: JobMutationScope,
    ): Promise<boolean> {
      validateActionId(actionId);
      if (!reason.trim() || reason.length > 500) {
        throw new JobControlPlaneError("JOB_RECOVERY_EVIDENCE_INVALID", "Review recovery reason is invalid");
      }
      if (!evidence || !evidence.disposition || (evidence.operationKey !== undefined && (evidence.operationKey.length === 0 || evidence.operationKey.length > 200))) {
        throw new JobControlPlaneError("JOB_RECOVERY_EVIDENCE_INVALID", "Review recovery evidence is invalid");
      }
      const safeReason = sanitizeJobErrorMessage(reason, 500, "operator_review_recovery");
      const actionReason = `${safeReason}:${evidence.disposition}`;
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        const existingAction = await repo.findAction(actionId);
        if (existingAction) {
          if (existingAction.workerJobId !== jobId || existingAction.command !== "recover_review" || existingAction.reason !== actionReason) {
            throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
          }
          assertJobMutationScope(job, scope);
          return true;
        ...
        assertJobMutationScope(job, scope);
        if (!job || job.status !== "failed" || !job.operatorReviewRequired || job.attempt >= job.maxAttempts) return false;
        ...
        if (!updated) throw new JobControlPlaneError("JOB_STATE_CONFLICT", "Job changed while recovering operator review");

        if (previousAttempt) {
          await repo.updateAttempt({
            attemptId: previousAttempt.id,
            values: { finishedAt: now, terminalClass: "recovered", recoveryReason: safeReason },
          });
        }
        await repo.insertAttempt({
          id: nextAttemptId,
        ...
        const payload = {
          ...
          ...(evidence.operationKey ? { operationKey: evidence.operationKey } : {}),
        };
        await repo.insertEvent({
          workerJobId: jobId,
          eventType: "OPERATOR_ACTION",
        ...
        await repo.insertEvent({
          ...
          payloadJson: { attempt: nextAttempt, reason: "operator_review_recovery" },
        });
        await repo.insertOutbox({
          workerJobId: jobId,
          attemptId: nextAttemptId,
    ...
    async forceFail(jobId: string, reason: string, actionId: string, actorId?: number, scope?: JobMutationScope): Promise<void> {
      validateActionId(actionId);
      const safeReason = sanitizeJobErrorMessage(reason, 2000, "operator_force_fail");
      await repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        assertJobMutationScope(job, scope);
        const existingAction = await repo.findAction(actionId);
        if (existingAction) {
          if (existingAction.workerJobId !== jobId || existingAction.command !== "force_fail" || existingAction.reason !== safeReason.slice(0, 500)) {
            throw new JobControlPlaneError("IDEMPOTENCY_CONFLICT", "Action key was already used for another command or target");
          }
          return;
        }
        ...
        if (!(await prepareOperatorAction(repo, { actionId, jobId, command: "force_fail", reason: safeReason, actorId, authorizationScope: scope?.authorizationScope, job }))) return;
        const updated = await repo.updateJob({ jobId, expectedStatus: job.status, expectedTenantId: scope?.tenantId, expectedRequestedByUserId: scope?.requestedByUserId, expectedAttempt: job.attempt, values: { status: "failed", statusReason: "operator_force_fail", failureReason: safeReason, errorCode: "OPERATOR_FORCE_FAIL", errorMessage: safeReason, operatorReviewRequired: true, operatorReviewReason: safeReason.slice(0, 500), leaseOwnerToken: null, leaseExpiresAt: null, finishedAt: new Date() } });
        ...
        const currentAttempt = await repo.findAttempt(jobId, job.attempt);
        if (currentAttempt) await repo.updateAttempt({ attemptId: currentAttempt.id, values: { finishedAt: new Date(), terminalClass: "operator", recoveryReason: safeReason.slice(0, 500) } });
        await repo.insertEvent({ workerJobId: jobId, eventType: "OPERATOR_ACTION", attemptId: currentAttempt?.id, eventIdempotencyKey: `operator:${actionId}:action`, payloadJson: { action: "force_fail", reason: safeReason.slice(0, 500), actorId, targetAttempt: job.attempt, targetStatus: job.status } });
        ...
        await finishOperatorAction(repo, actionId, { accepted: true, phase: "force_failed", jobId });
      });
    },

    async failExternalWait(jobId: string, reason: string, operatorReviewRequired = true, now = new Date(), operationKey?: string, pollerLeaseTokenHash?: string): Promise<"failed" | "ignored"> {
      const safeReason = sanitizeJobErrorMessage(reason, 2000, "external_wait_timeout");
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        if (!job || job.status !== "waiting_external") return "ignored";
        if (operationKey) {
          const externalWait = job.progressJson && typeof job.progressJson === "object" && !Array.isArray(job.progressJson)
            ? (job.progressJson as { externalWait?: { operationKey?: string } }).externalWait
            : undefined;
          if (externalWait?.operationKey !== operationKey) return "ignored";
        }
        if (operationKey && pollerLeaseTokenHash && repo.assertProviderPollLease
          && !(await repo.assertProviderPollLease({ operationKey, pollerLeaseTokenHash }))) return "ignored";
        const updated = await repo.updateJob({ jobId, expectedStatus: "waiting_external", expectedAttempt: job.attempt, values: { status: "failed", statusReason: "external_wait_timeout", failureReason: safeReason, errorCode: "EXTERNAL_WAIT_TIMEOUT", errorMessage: safeReason, operatorReviewRequired, finishedAt: now } });
        if (!updated) return "ignored";
        const currentAttempt = await repo.findAttempt(jobId, job.attempt);
        if (currentAttempt) await repo.updateAttempt({ attemptId: currentAttempt.id, values: { finishedAt: now, terminalClass: "failed", recoveryReason: safeReason.slice(0, 500) } });
        await repo.cancelUnpublishedOutbox({ jobId, reason: "failed:external_wait_timeout", cancelledAt: now });
        await repo.insertEvent({ workerJobId: jobId, eventType: "TIMEOUT", attemptId: currentAttempt?.id, eventIdempotencyKey: `external-timeout:${jobId}:${job.attempt}`, payloadJson: { reason: safeReason.slice(0, 500) } });
    ...
    async completeExternal(jobId: string, resultRef: string, operationKey: string, now = new Date(), pollerLeaseTokenHash?: string): Promise<boolean> {
      const safeResultRef = normalizeResultReference(resultRef);
      if (!safeResultRef || safeResultRef.length > 255 || !operationKey.trim() || operationKey.length > 200) return false;
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        if (!job) return false;
        const externalWait = job.progressJson && typeof job.progressJson === "object" && !Array.isArray(job.progressJson)
        ...
        if (pollerLeaseTokenHash && repo.assertProviderPollLease
          && !(await repo.assertProviderPollLease({ operationKey, pollerLeaseTokenHash }))) return false;
        ...
        if (!updated) return false;
        const currentAttempt = await repo.findAttempt(jobId, job.attempt);
        if (currentAttempt) await repo.updateAttempt({ attemptId: currentAttempt.id, values: { finishedAt: now, terminalClass: "succeeded", recoveryReason: "provider_poll_completed" } });
        await repo.insertEvent({
          workerJobId: jobId,
          eventType: "COMPLETED",
          attemptId: currentAttempt?.id,
          eventIdempotencyKey: `provider-completed:${operationKey}`,
          payloadJson: { operationKey, resultRef: safeResultRef, source: "provider-poller" },
        });
        return true;
      });
    },
    ...
    async expireDeadline(jobId: string, now = new Date()): Promise<"expired" | "ignored"> {
      return repository.transaction(async repo => {
        ...
        if (job.status === "waiting_external" && externalWait?.operationKey?.startsWith("storyboard.pause:")) return "ignored";
        const policy = (job.retryPolicyJson ?? {}) as Record<string, unknown>;
    ...
    async recoverExpiredLease(jobId: string, now = new Date()): Promise<"recovered" | "ignored"> {
      return repository.transaction(async repo => {
        const job = await repo.findJob(jobId);
        if (!job || !["leased", "running", "waiting_external"].includes(job.status) || !job.leaseExpiresAt || job.leaseExpiresAt > now) return "ignored";
        const currentAttempt = await repo.findAttempt(jobId, job.attempt);
        const policy = (job.retryPolicyJson ?? {}) as Record<string, unknown>;
        const withinDeadline = !Number.isFinite(Number(policy.deadlineMs)) || now.getTime() < job.createdAt.getTime() + Number(policy.deadlineMs);
        ...
        const updated = await repo.updateJob({
          ...
          values: {
            ...
            errorMessage: "Execution lease expired before the attempt completed",
          },
        });
        if (!updated) return "ignored";
        ...
        if (shouldRetry && nextAttemptId) {
          ...
          await repo.insertOutbox({ workerJobId: jobId, attemptId: nextAttemptId, envelopeVersion: job.contractVersion, envelopeJson: { jobId, businessAttempt: nextAttempt, contractVersion: job.contractVersion, attemptId: nextAttemptId }, dedupeKey: `job:${jobId}:attempt:${nextAttempt}`, nextAttemptAt: nowPlus(retryDelay, now) });
        }
...
async function guardedLeaseUpdate(
  ...
  await repository.transaction(async repo => {
    ...
    if (!updated) throw new JobControlPlaneError("JOB_LEASE_STALE", "Job lease is no longer active");
    if (settlement) {
      await repo.insertSettlement({ workerJobId: lease.jobId, attemptId: lease.attemptId, settlementKey: settlement.settlementKey, settlementType: settlement.settlementType, payloadJson: redactJobPayload(payload) as Record<string, unknown> });
      await repo.insertEvent({
        workerJobId: lease.jobId,
        eventType: "SETTLEMENT_RECORDED",
        attemptId: lease.attemptId,
        eventIdempotencyKey: `settlement:${settlement.settlementKey}`,
        payloadJson: { settlementType: settlement.settlementType, settlementKey: settlement.settlementKey },
      });
    }
...
export function isRetryableError(error: unknown): boolean {
  if (error && typeof error === "object" && "class" in error && (error as { class?: unknown }).class === "retryable") return true;
  ...
  return ["TIMEOUTERROR", "ABORTERROR", "ECONNRESET", "ETIMEDOUT", "EAI_AGAIN", "ECONNREFUSED", "ENETUNREACH", "EHOSTUNREACH", "UNAVAILABLE", "TEMPORARY_UNAVAILABLE", "RETRYABLE"].includes(code)
 ...
 */
export function classifyJobError(error: unknown): "retryable" | "permanent" | "unknown" {
  if (error && typeof error === "object" && "class" in error) {
    const explicitClass = (error as { class?: unknown }).class;
    if (explicitClass === "retryable" || explicitClass === "permanent" || explicitClass === "unknown") return explicitClass;
  }
  if (isRetryableError(error)) return "retryable";
  const candidate = error as { code?: unknown; name?: unknown; status?: unknown; statusCode?: unknown } | null;
  const code = String(candidate?.code ?? candidate?.name ?? "").toUpperCase();
  const status = Number(candidate?.statusCode ?? candidate?.status);
  if ((Number.isInteger(status) && status >= 400 && status <= 499 && status !== 408 && status !== 429)
    || [
      "INVALID_INPUT",
      "INVALID_REQUEST",
      "INVALID_CREDENTIAL",

...
// A failed ledger write is an infrastructure/billing-integrity incident. It
// must never be routed to the user-purchase flow merely because the table name
// contains the word "credit".
const CREDIT_LEDGER_FAILURE_PATTERN =
  /(?:CREDIT_LEDGER_WRITE_FAILED|credit_transactions|skill_revenue_settlements|credit ledger)/i;
...
function inferModelKind(path: string, message: string): CreditModelKind {
  ...
  if (
    /(?:llm|chat|skill|story|draft|prompt|qualityqc|quality.review|script)/i.test(
      text
    )
  )
    return "llm";
...
export function classifyCreditFailure(input: {
  ...
  if (CREDIT_LEDGER_FAILURE_PATTERN.test(message)) {
    return {
  ...
  const source: CreditFailureSource =
    context.source === "provider" || provider
      ? "provider"
      : context.source === "user"
        ? "user"
        : "unknown";
  const modelKind =
    context.modelKind && context.modelKind !== "unknown"
      ? context.modelKind
      : inferModelKind(path, message);
  const requestedMatch = message.match(REQUIRED_CREDITS_PATTERN);
  const requestedCredits =
    finiteCredits(context.requestedCredits) ??

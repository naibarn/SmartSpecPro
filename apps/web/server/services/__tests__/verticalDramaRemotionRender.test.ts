...
import { workerArtifacts, workerJobs, verticalDramaEpisodes } from "../../../drizzle/schema";
...
function tableOfFrom(fromArg: unknown): "episode" | "workerJob" | "workerArtifact" | "unknown" {
      ...
      from: (fromArg: unknown) => ({
        where: () => ({
          limit: async () => {
            ...
            if (table === "workerJob") return workerJobRow ? [workerJobRow] : [];
      ...
      set: (patch: Record<string, unknown>) => ({
        where: async () => {
          if (tableOfFrom(table) === "episode" && "assemblyManifest" in patch) {
            episodeRow = { assemblyManifest: patch.assemblyManifest as Record<string, unknown> };
          }
        },
      }),
    }),
...
describe("buildVdCaptionLines", () => {
  ...
  it("renders ONLY the spoken text when a line has a speakerName — no 'Name: ' prefix (regression: speaker name was previously burned into the caption)", () => {
    const subtitles: RunAssemblyJobSubtitlesInput = {
      preset: "classic_box",
      lines: [{ startSec: 0, endSec: 2, speakerName: "ภูมิ", text: "อย่าแตะของเธอ" }],
    };
    const lines = buildVdCaptionLines(subtitles);
  ...
  it("returns an empty array when subtitles are absent or have no lines", () => {
    ...
    expect(buildVdCaptionLines({ preset: "no_subtitle_style", lines: [] })).toEqual([]);
  });
});
...
describe("resolveVdTextOverlayWindow", () => {
  ...
  it("resolves entireClip to [0, videoDurationSeconds]", () => {
    expect(resolveVdTextOverlayWindow({ ...base, entireClip: true }, 60)).toEqual({
      startSec: 0,
      endSec: 60,
    });
  });
  ...
  it("resolves endAnchored to a fixed-length window ending at videoDurationSeconds", () => {
    expect(
      resolveVdTextOverlayWindow(
        { ...base, kind: "end_card", endAnchored: true, durationSecForEndAnchor: 4 },
        60,
      ),
  ...
  it("passes through an already-concrete window unchanged", () => {
    expect(resolveVdTextOverlayWindow({ ...base, startSec: 10, endSec: 13 }, 60)).toEqual({
      startSec: 10,
      endSec: 13,
    });
  });
...
describe("buildVdRemotionTemplate", () => {
  ...
  it("maps every VdTextOverlayAssKind to a text layer within its resolved window", () => {
    const overlays: RunAssemblyJobTextOverlayEventInput[] = [
      { kind: "end_card", text: "END", endAnchored: true, durationSecForEndAnchor: 3, startSec: 0, endSec: 0 },
      { kind: "opener_recap", text: "Previously...", startSec: 0, endSec: 3 },
      { kind: "title_bumper", text: "EP 3: The Return", secondaryText: "ตอนที่ 3", startSec: 0, endSec: 2 },
      { kind: "episode_indicator", text: "EP 3", entireClip: true, startSec: 0, endSec: 0, variant: "top_left" },
      { kind: "character_intro", text: "Mai", secondaryText: "The Lead", startSec: 5, endSec: 8 },
      { kind: "time_setting", text: "3 days later", startSec: 20, endSec: 23 },
      { kind: "narrative_hook", text: "But little did she know...", startSec: 30, endSec: 33 },
      { kind: "watermark_text", text: "@brand", entireClip: true, startSec: 0, endSec: 0, variant: "bottom_right" },
      { kind: "age_badge", text: "18+", entireClip: true, startSec: 0, endSec: 0 },
    ];
    const { template } = buildVdRemotionTemplate({
      clips: [{ clipNumber: 1, url: "https://cdn.example.com/1.mp4", durationSec: 60 }],
    ...
    for (const overlay of overlays) {
      const layer = textLayers.find(l => l.id === `overlay-${overlay.kind}-${overlays.indexOf(overlay)}`);
      expect(layer).toBeTruthy();
      expect(layer && "content" in layer && layer.content).toBe(overlay.text);
    }
    const endCardLayer = textLayers.find(l => l.id.startsWith("overlay-end_card-"));
  ...
  it("composites the series IMAGE watermark as a full-timeline corner layer", () => {
    ...
    const layer = template.layers.find(l => l.id === "series-watermark-primary");
    ...
    expect(layer && "height" in layer && layer.height).toBeCloseTo(10 * (1080 / 1920), 5);
  });
  ...
  it("dual watermark: composites TWO independent full-timeline layers with distinct ids and anchors", () => {
    ...
    const primaryLayer = template.layers.find(l => l.id === "series-watermark-primary");
    const secondaryLayer = template.layers.find(l => l.id === "series-watermark-secondary");
  ...
  it("throws a typed error when the layer budget (40) is exceeded", () => {
    const manyOverlays: RunAssemblyJobTextOverlayEventInput[] = Array.from({ length: 40 }, (_, i) => ({
...
describe("submitVdRemotionAssembly", () => {
  it("enqueues a job and persists a pending compiledVideo state stamped remotion_queue, never dispatching Lane A in-process", async () => {
    ...
    const queueJob = vi.fn().mockResolvedValue({ created: true, job: { id: "job-1" } });
    const stageAsset = vi.fn().mockResolvedValue({ durationSec: 8, sha256: "abc" });
    ...
    expect(typeof (episodeRow.assemblyManifest as any).compiledVideo.renderSubmittedAt).toBe(
      "number",
    );
  });
  ...
  it("uses each staged asset's REAL bytes-sha256 in the asset manifest, not sha256(url)", async () => {
    episodeRow = { assemblyManifest: {} };
    const queueJob = vi.fn().mockResolvedValue({ created: true, job: { id: "job-2" } });
    ...
    await submitVdRemotionAssembly(
      {
        ...
        dialogueAudio: {
          segments: [{ audioUrl: "https://cdn.example.com/line1.mp3", startSec: 0, endSec: 2 }],
        },
...
describe("retimeSubtitleLinesToProbedClips", () => {
  ...
  it("places captions on the real timeline, not the planned one", () => {
    ...
    const lines = [
      ...
      line({ clipNumber: 1, clipLocalStartFrac: 0, clipLocalEndFrac: 0.5, startSec: 0, endSec: 4 }),
      // Planned: clip 9 [64,72) -> last quarter.
      line({ clipNumber: 9, clipLocalStartFrac: 0.75, clipLocalEndFrac: 1, startSec: 70, endSec: 72 }),
    ];
  ...
  it("is per-clip, so an uneven overrun does not accumulate wrongly", () => {
    ...
    const retimed = retimeSubtitleLinesToProbedClips(
      [line({ clipNumber: 2, clipLocalStartFrac: 0, clipLocalEndFrac: 1, startSec: 8, endSec: 16 })],
      clips
    );
  ...
  it("passes through lines with no clip attribution untouched", () => {
    ...
    const [result] = retimeSubtitleLinesToProbedClips([original], [
  ...
  it("preserves speaker and text while re-timing", () => {
    const [result] = retimeSubtitleLinesToProbedClips(
      [line({ speakerName: "ปราง", text: "มือถือเครื่องเดียว ทำไมต้องตามไม่เลิก" })],
      [{ clipNumber: 1, durationSec: 10 }]
    );
...
describe("reconcileVdRemotionAssembly", () => {
  ...
  it("writes status:'completed' + videoUrl from outputJson.outputUrl on success", async () => {
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-1" } } };
    workerJobRow = {
      id: "job-1",
      status: "completed",
  ...
  it("resolves the mp4 from the worker completion payload mirror, not outputJson.outputUrl", async () => {
    ...
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-6" } } };
    workerJobRow = {
      id: "job-6",
      status: "completed",
      outputJson: {
  ...
  it("prefers the already-servable publishedArtifacts sourceUrl over a bare storage key", async () => {
    ...
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-9" } } };
    workerJobRow = {
      id: "job-9",
      status: "completed",
      outputJson: {
        publishedArtifacts: [
          { sourceUrl: `/api/storage/files/${storageRef}`, publishedItemId: 650 },
        ],
  ...
  it("falls back to the published worker_artifacts mp4 row when the payload mirror is missing", async () => {
    ...
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-7" } } };
    workerJobRow = { id: "job-7", status: "completed", outputJson: { assignedAt: "x" } };
  ...
  it("still fails when the job completed with no artifact anywhere", async () => {
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-8" } } };
    workerJobRow = { id: "job-8", status: "completed", outputJson: { assignedAt: "x" } };
    workerArtifactRows = [];
  ...
  it("writes status:'failed' with the worker job's failureReason", async () => {
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-2" } } };
    workerJobRow = { id: "job-2", status: "failed", failureReason: "renderer crashed" };
  ...
  it("is a no-op while still queued and within the TTL", async () => {
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-3" } } };
    workerJobRow = { id: "job-3", status: "queued" };
    const submittedAt = Date.now() - (VD_REMOTION_QUEUED_TTL_MS - 60_000);
    const result = await reconcileVdRemotionAssembly(owner, "job-3", submittedAt);
    expect(result).toEqual({ reconciled: false });
    expect((episodeRow.assemblyManifest as any).compiledVideo.status).toBe("pending");
  });

  it("falls back to a failed compiledVideo state once the queued TTL elapses (no Lane B claim)", async () => {
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-4" } } };
    workerJobRow = { id: "job-4", status: "queued" };
    const submittedAt = Date.now() - (VD_REMOTION_QUEUED_TTL_MS + 60_000);
    const result = await reconcileVdRemotionAssembly(owner, "job-4", submittedAt);
    expect(result).toEqual({ reconciled: true, status: "failed" });
  ...
  it("treats a missing submittedAt as never-timed-out (stays pending)", async () => {
    episodeRow = { assemblyManifest: { compiledVideo: { status: "pending", pendingJobId: "job-5" } } };

...
describe("creditFailurePolicy", () => {
  it("keeps ordinary LLM user-credit failures user-only", () => {
    expect(
      ...
      route: "user_purchase",
      adminPriority: null,
      threshold: CREDIT_THRESHOLDS.llm,
    });
  });
  ...
  it("escalates an LLM request above 3000 credits", () => {
    expect(
      ...
      route: "admin_suspicious",
      adminPriority: "high",
      requestedCredits: 3001,
    });
  });
  ...
  it("does not classify unrelated errors as credit failures", () => {
    expect(
      classifyCreditFailure({ errorMessage: "Provider timed out" }).route
    ).toBe("none");
  });

  it("does not turn a credit-ledger database error into a purchase prompt", () => {
    expect(
      classifyCreditFailure({
        errorMessage:
          'CREDIT_LEDGER_WRITE_FAILED: skill settlement transaction; insert into "credit_transactions" failed',
        path: "verticalDramaCharacters.generateCharacterImage",
      })
    ).toMatchObject({ isCreditFailure: false, route: "none" });
  });
});

...
beforeEach(() => {
  ...
  (textExecutor.canHandle as any).mockReturnValue(true);
  (imageExecutor.canHandle as any).mockReturnValue(true);
  (videoExecutor.canHandle as any).mockReturnValue(true);
});
...
describe("Channel Parity Tests", () => {
  ...
  describe("Credit Parity", () => {
    ...
    it("chat deducts, team room calculates only (designed difference)", async () => {
      ...
      // deduct called once (for chat), calculate called once (for team_room)
  ...
  describe("Failure Parity", () => {
    ...
    it("executor not found -- same fallback for both channels", async () => {
      ...
      expect(chatResult.route.reason).toBe("executor_not_found");
      expect(teamResult.route.reason).toBe("executor_not_found");
      expect(chatResult.telemetry.executorId).toBe("unknown");
      expect(teamResult.telemetry.executorId).toBe("unknown");
    });
  });
});

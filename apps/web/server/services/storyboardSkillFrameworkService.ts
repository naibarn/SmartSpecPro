...
function storyboardManagedAssetUrl(storageKey: string | null | undefined): string | null {
  return storageKey
    ? `/api/storage/files/${encodeURIComponent(storageKey)}`
    : null;
}

function storyboardRepairActionId(runId: string, attempt: number, shotNumbers: number[]): string {
  const digest = crypto
    .createHash("sha256")
    .update(`storyboard-review:${runId}:${attempt}:${[...shotNumbers].sort((a, b) => a - b).join(",")}`, "utf8")
    .digest("hex")
    .slice(0, 32);
...
async function enrichStoryboardCharacterReferences(
  ...
  const existing = Array.isArray(normalized.skillInputs.character_reference_images)
  ...
  for (const value of existing) {
    ...
    const assetId = String((value as Record<string, unknown>).asset_id ?? (value as Record<string, unknown>).assetId ?? "").trim();
    if (!assetId || seen.has(assetId)) continue;
    seen.add(assetId);
    references.push(value as Record<string, unknown>);
  }
  for (const portrait of portraits) {
    if (portrait.mediaAssetId == null) continue;
...
async function ensureStoryboardControlPlaneJob(input: {
  ...
  const job = await createControlPlaneJob({
    ...
    definition: {
      ...
      retryPolicy: {
        // One control-plane attempt represents one sequential storyboard
        // pass. A bounded repair budget allows stop/resume and provider
        // repairs without turning retries into an unbounded loop.
        maxAttempts: 12,
        baseDelayMs: 5_000,
        maxDelayMs: 120_000,
        jitter: "bounded",
  ...
  await db.update(storyboardSkillRuns)
    .set({ workerJobId: job.jobId, updatedAt: new Date() })
    .where(and(eq(storyboardSkillRuns.id, input.runId), eq(storyboardSkillRuns.tenantId, input.tenantId), eq(storyboardSkillRuns.userId, input.userId)));
  return job.jobId;
}
...
export async function createStoryboardSkillDraft(input: {
  ...
  try {
    ...
    const enriched = await enrichStoryboardCharacterReferences(tx, normalized, tenantId, input.userId);
    const requestedFingerprint = buildStoryboardConfirmationFingerprint(enriched, skill);
    const [existing] = await tx
      ...
      .limit(1);
    if (existing) {
      assertStoryboardIdempotencyMatch(existing.run.confirmationFingerprint, requestedFingerprint);
      return {
    ...
    const enrichedFingerprint = buildStoryboardConfirmationFingerprint(enriched, skill);
    ...
    await tx.insert(storyboardSkillRuns).values({
      ...
      candidateHistory: [{ version: 1, idea: normalized.idea, source: "original" }],
    ...
    const enriched = await enrichStoryboardCharacterReferences(db, normalized, tenantId, input.userId);
    const requestedFingerprint = buildStoryboardConfirmationFingerprint(enriched, skill);
    const [existing] = await db
      .select({ project: storyboardSkillProjects, run: storyboardSkillRuns })
      .from(storyboardSkillRuns)
      .innerJoin(storyboardSkillProjects, eq(storyboardSkillProjects.id, storyboardSkillRuns.projectId))
      .where(and(
        eq(storyboardSkillRuns.tenantId, tenantId),
        eq(storyboardSkillRuns.userId, input.userId),
        eq(storyboardSkillRuns.idempotencyKey, input.idempotencyKey),
  ...
  try {
    ...
    if (existing) {
      ...
      if (input.draft !== undefined) {
        ...
        const requestedSkill = getStoryboardSkillSchema(requested.selectedSkillId);
        ...
        assertStoryboardIdempotencyMatch(
          ...
          buildStoryboardConfirmationFingerprint(requestedEnriched, requestedSkill),
        );
      }
      return {
    ...
    if (input.draft !== undefined) {
      ...
      const requestedEnriched = await db.transaction(tx =>
        enrichStoryboardCharacterReferences(tx, requested, tenantId, input.userId),
      );
      assertStoryboardIdempotencyMatch(
        existing.confirmationFingerprint,
        buildStoryboardConfirmationFingerprint(requestedEnriched, requestedSkill),
...
export async function getStoryboardSkillRun(input: {
  ...
  const controlPlaneEvents = run.workerJobId
    ? (await db
      ...
      .orderBy(desc(workerJobEvents.eventSequence), desc(workerJobEvents.createdAt))
      .limit(50))
      .reverse()
      .map(event => ({ ...event, payloadJson: redactStoryboardValue(event.payloadJson) as Record<string, unknown> }))
    : [];
  ...
  return {
    ...
    candidateHistory: redactStoryboardValue(run.candidateHistory) as typeof run.candidateHistory,
    ...
    shots: shots.map(shot => ({
      ...
      skillResponse: shot.skillResponse ? redactRecord(shot.skillResponse) : null,
      generationRequest: shot.generationRequest ? redactRecord(shot.generationRequest) : null,
      effectiveGenerationRequest: shot.effectiveGenerationRequest ? redactRecord(shot.effectiveGenerationRequest) : null,
      error: shot.error ? redactRecord(shot.error) : null,
      imageUrl: shot.imageAssetId ? assetUrlById.get(shot.imageAssetId) ?? null : null,
    })),
  };
}
...
export async function listStoryboardSkillRuns(input: {
  ...
  const rows = await db
    ...
    .where(and(
      ...
      and(...STORYBOARD_RECOVERY_INDEX_EXCLUDED_STATUSES.map(status => ne(storyboardSkillRuns.status, status))),
    ))
  ...
  const jobs = workerJobIds.length > 0
    ? await db
      .select({
        id: workerJobs.id,
        status: workerJobs.status,
  ...
  const summary = new Map<string, { total: number; completed: number; remaining: number }>();
  for (const shot of shots) {
    const current = summary.get(shot.runId) ?? { total: 0, completed: 0, remaining: 0 };
    current.total += 1;
  ...
  return rows.map(row => ({
    ...
    job: row.run.workerJobId ? jobById.get(row.run.workerJobId) ?? null : null,
...
export async function confirmStoryboardSkillRun(input: {
  ...
  if (run.status === "queued" || run.status === "running" || run.status === "succeeded") {
    const workerJobId = run.status === "queued"
      ? await ensureStoryboardControlPlaneJob({ runId: run.id, tenantId, userId: input.userId, workerJobId: run.workerJobId })
      : run.workerJobId;
  ...
  const confirmed = await db.transaction(async tx => {
    ...
    const [updatedRun] = await tx
      ...
      .where(and(
        ...
        eq(storyboardSkillRuns.confirmationFingerprint, input.confirmationFingerprint),
      ))
  ...
  if (!confirmed) {
    ...
    if (current?.status === "awaiting_confirmation") {
      throw new Error("Storyboard draft changed; refresh the estimate before confirming");
    }
    if (current && ["queued", "running", "succeeded"].includes(current.status)) {
      const workerJobId = current.status === "queued"
        ? await ensureStoryboardControlPlaneJob({ runId: run.id, tenantId, userId: input.userId, workerJobId: current.workerJobId })
        : current.workerJobId;
      return { runId: run.id, status: current.status, workerJobId, idempotent: true };
    }
    throw new Error("Storyboard run changed while confirming");
  }
  const workerJobId = await ensureStoryboardControlPlaneJob({ runId: run.id, tenantId, userId: input.userId, workerJobId: run.workerJobId });
  return { runId: run.id, status: "queued" as const, workerJobId, idempotent: false };
}
...
export async function pauseStoryboardSkillRun(input: {
  ...
  const result = await db.transaction(async tx => {
    const [existing] = await tx.select().from(storyboardSkillRuns).where(and(
      eq(storyboardSkillRuns.id, input.runId),
      eq(storyboardSkillRuns.tenantId, tenantId),
      eq(storyboardSkillRuns.userId, input.userId),
    )).limit(1);
    if (!existing) throw new Error("Storyboard run not found");
    if (isTerminalStoryboardRunStatus(existing.status)) return { run: existing, idempotent: true };
    if (existing.status === "cancel_requested") {
      throw new Error("Cancelled storyboard runs cannot be paused");
    }
    if (existing.status === "paused") return { run: existing, idempotent: true };
    const [project] = await tx.select({ id: storyboardSkillProjects.id })
      .from(storyboardSkillProjects)
      .where(and(
      ...
      .limit(1);
    if (!project) throw new Error("Storyboard project not found");
    const [run] = await tx.update(storyboardSkillRuns).set({
      status: "paused",
      error: { code: "PAUSED_BY_USER", message: "Generation paused by the user." },
      updatedAt: new Date(),
    }).where(and(
      eq(storyboardSkillRuns.id, existing.id),
      eq(storyboardSkillRuns.tenantId, tenantId),
      eq(storyboardSkillRuns.userId, input.userId),
      inArray(storyboardSkillRuns.status, ["awaiting_confirmation", "queued", "running", "partial"]),
    )).returning();
    if (!run) throw new Error("Storyboard run changed while pausing");
    await tx.update(storyboardSkillProjects).set({ status: "paused", updatedAt: new Date() }).where(and(
      eq(storyboardSkillProjects.id, run.projectId),
      eq(storyboardSkillProjects.tenantId, tenantId),
      eq(storyboardSkillProjects.userId, input.userId),
    ));
...
/** Resume keeps all valid completed shots and queues only the first missing one. */
export async function resumeStoryboardSkillRun(input: {
  ...
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const [run] = await db.select().from(storyboardSkillRuns).where(and(
    eq(storyboardSkillRuns.id, input.runId),
    eq(storyboardSkillRuns.tenantId, tenantId),
    eq(storyboardSkillRuns.userId, input.userId),
  )).limit(1);
  if (!run) throw new Error("Storyboard run not found");
  if (isTerminalStoryboardRunStatus(run.status)) throw new Error("Completed storyboard runs cannot be resumed");
  if (run.status === "cancel_requested") {
    throw new Error("Cancelled storyboard runs cannot be resumed");
  }
  if (run.status !== "paused" && run.status !== "partial") {
    return { runId: run.id, status: run.status, workerJobId: run.workerJobId, idempotent: true };
  }
  const controlPlane = run.workerJobId ? createJobControlPlane() : null;
  const controlStatus = run.workerJobId ? await controlPlane!.getStatus(run.workerJobId, {
    tenantId,
    requestedByUserId: input.userId,
    authorizationScope: "storyboard",
  }) : null;
  if (controlStatus && ["succeeded", "cancelled", "expired"].includes(controlStatus.status)) {
    throw new Error("Storyboard domain state cannot resume a terminal control-plane job");
  }
  ...
  const transition = await db.transaction(async tx => {
    const [current] = await tx.select().from(storyboardSkillRuns).where(and(
      eq(storyboardSkillRuns.id, input.runId),
      eq(storyboardSkillRuns.tenantId, tenantId),
      eq(storyboardSkillRuns.userId, input.userId),
    )).limit(1);
    ...
    const [project] = await tx.select({ id: storyboardSkillProjects.id })
      ...
      .limit(1);
    if (!project) throw new Error("Storyboard project not found");
    const shots = await tx.select().from(storyboardSkillShots).where(and(
      eq(storyboardSkillShots.runId, current.id),
      eq(storyboardSkillShots.tenantId, tenantId),
    ));
    if (current.error?.class === "unknown" || shots.some(shot => shot.error?.class === "unknown")) {
      throw new Error("Provider result requires operator review before resume");
    }
    for (const shot of shots) {
      if (shot.status === "paused" || shot.status === "suppressed") {
        await tx.update(storyboardSkillShots).set({
    ...
    const [updated] = await tx.update(storyboardSkillRuns).set({ status: "queued", error: null, updatedAt: new Date() }).where(and(
    ...
    await tx.update(storyboardSkillProjects).set({ status: "queued", updatedAt: new Date() }).where(and(
      eq(storyboardSkillProjects.id, current.projectId),
      eq(storyboardSkillProjects.tenantId, tenantId),
      eq(storyboardSkillProjects.userId, input.userId),
    ));
  ...
  if (!transition.changed) {
    return { runId: transition.run.id, status: transition.run.status, workerJobId: transition.run.workerJobId, idempotent: true };
  }
  const workerJobId = await ensureStoryboardControlPlaneJob({ runId: transition.run.id, tenantId, userId: input.userId, workerJobId: transition.run.workerJobId });
  if (transition.run.workerJobId && controlPlane) {
    if (controlStatus?.status === "waiting_external") {
      await controlPlane.resumeExternal(transition.run.workerJobId, "storyboard-resume", "postgres-pull", undefined, `resume:${transition.run.id}`, true);
    }
  }
  return { runId: transition.run.id, status: "queued" as const, workerJobId, idempotent: false };
}
...
export async function cancelStoryboardSkillRun(input: {
  ...
  const requested = await db.transaction(async tx => {
    const [existing] = await tx
      ...
      .limit(1);
    if (!existing) throw new Error("Storyboard run not found");
    if (isTerminalStoryboardRunStatus(existing.status)) return { run: existing, changed: false };
    if (existing.status === "cancel_requested") return { run: existing, changed: false };
    const [project] = await tx.select({ id: storyboardSkillProjects.id })
      .from(storyboardSkillProjects)
      .where(and(
        eq(storyboardSkillProjects.id, existing.projectId),
    ...
    const [run] = await tx
      ...
      .where(and(
        ...
        eq(storyboardSkillRuns.tenantId, tenantId),
        eq(storyboardSkillRuns.userId, input.userId),
        inArray(storyboardSkillRuns.status, ["awaiting_confirmation", "queued", "running", "paused", "partial"]),
      ))
      .returning({
        id: storyboardSkillRuns.id,
        projectId: storyboardSkillRuns.projectId,
  ...
  if (!requested.changed && requested.run.status !== "cancel_requested") {
    if (requested.run.status === "cancelled") {
      ...
      return {
        ...requested.run,
        idempotent: true,
        reviewId: projection.projectionStatus === "ready" ? projection.reviewId : null,
        projectionStatus: projection.projectionStatus,
      };
    }
    return { ...requested.run, idempotent: true };
  }
  const scope = { tenantId, requestedByUserId: input.userId, authorizationScope: "storyboard" as const };
  if (requested.run.workerJobId) {
    const controlPlane = createJobControlPlane();
    const status = await controlPlane.getStatus(requested.run.workerJobId, scope);
    if (status?.status !== "cancelled") {
      await controlPlane.cancel(
        requested.run.workerJobId,
        "storyboard_cancelled",
        `storyboard-cancel:${requested.run.id}`,
        input.userId,
        scope,
      );
    }
...
export async function retryStoryboardSkillShots(input: {
  ...
  const scope = { tenantId, requestedByUserId: input.userId, authorizationScope: "storyboard" as const };
  const controlPlane = run.workerJobId ? createJobControlPlane() : null;
  const controlStatus = run.workerJobId ? await controlPlane!.getStatus(run.workerJobId, scope) : null;
  if (controlStatus?.status === "failed" && controlStatus.operatorReviewRequired) {
    const unresolvedProviderOperation = retryableShots.some(shot => {
      const error = shot.error && typeof shot.error === "object" && !Array.isArray(shot.error)
    ...
    if (unresolvedProviderOperation) {
      throw new Error("Provider operation requires reconciliation before storyboard repair");
    }
  }
  const actionId = run.workerJobId
    ? storyboardRepairActionId(run.id, controlStatus?.attempt ?? 0, [...allowed])
    : null;
  ...
  if (run.workerJobId && controlPlane && actionId && controlStatus?.status === "failed" && controlStatus.operatorReviewRequired) {
    ...
    if (!reviewRecoveryAccepted) throw new Error("Storyboard control-plane review recovery was not accepted");
  }
  ...
  const domainRepairAccepted = await db.transaction(async tx => {
    const [currentRun] = await tx
      .select({ id: storyboardSkillRuns.id, projectId: storyboardSkillRuns.projectId, status: storyboardSkillRuns.status })
      .from(storyboardSkillRuns)
      .where(and(
        eq(storyboardSkillRuns.id, input.runId),
        eq(storyboardSkillRuns.tenantId, tenantId),
    ...
    const currentShots = await tx
      .select({ id: storyboardSkillShots.id, shotNumber: storyboardSkillShots.shotNumber, status: storyboardSkillShots.status, error: storyboardSkillShots.error })
      .from(storyboardSkillShots)
      .where(and(
    ...
    for (const shot of currentShots) {
      if (allowed.has(shot.shotNumber) && isRepairableStoryboardShot(shot.status, shot.error)) {
        await tx
          .update(storyboardSkillShots)
  ...
  if (run.workerJobId && controlPlane) {
    if (controlStatus?.status === "waiting_external") {
      await controlPlane.resumeExternal(run.workerJobId, "storyboard-retry", "postgres-pull", undefined, `retry:${run.id}:${[...allowed].sort((a, b) => a - b).join(",")}`, true);
      ...
      if (!requeued) throw new Error("Storyboard control-plane retry was not accepted");
    }
    if (reviewRecoveryAccepted) {
      ...
      if (refreshed?.status === "waiting_external") {
        await controlPlane.resumeExternal(run.workerJobId, "storyboard-retry", "postgres-pull", undefined, `repair:${run.id}:${[...allowed].sort((a, b) => a - b).join(",")}`, false);
      }
    }
  }
...
export async function updateStoryboardSkillDraft(input: {
  ...
  const enriched = await db.transaction(async tx => {
    ...
    const [existingRun] = await tx
      .select({ id: storyboardSkillRuns.id, normalizedSnapshot: storyboardSkillRuns.normalizedSnapshot })
      .from(storyboardSkillRuns)
      .where(and(
      ...
      .for("update")
      .limit(1);
    if (!existingRun) throw new Error("Only an awaiting-confirmation draft can be edited");
    const previousNormalized = normalizeStoryboardGlobalInput(existingRun.normalizedSnapshot);
    if (JSON.stringify(previousNormalized.characterIds) !== JSON.stringify(normalized.characterIds)) {
      throw new Error("Character bindings cannot be changed after draft creation");
    }
    const resolved = await enrichStoryboardCharacterReferences(tx, normalized, tenantId, input.userId);
    const resolvedFingerprint = buildStoryboardConfirmationFingerprint(resolved, skill);
    const [run] = await tx
      .update(storyboardSkillRuns)
      .set({
    ...
    await tx.delete(storyboardSkillShots).where(and(
      eq(storyboardSkillShots.runId, run.id),
      eq(storyboardSkillShots.tenantId, tenantId),
    ));
    ...
    await tx.update(storyboardSkillProjects)
...
export async function rebuildStoryboardSkillReviewProjection(input: {
  ...
  const completedClipCount = projection.tasks.filter(task => Boolean(task.url)).length;
...
export async function archiveStoryboardSkillProject(input: {
  ...
  if (archived.workerJobId && archived.runStatus === "cancel_requested") {
    ...
    if (status && !["cancelled", "succeeded", "failed", "expired"].includes(status.status)) {
      await controlPlane.cancel(
...
export async function listStoryboardCharacters(input: {
  ...
  const withPortraits = characters.map(character => ({
    ...
    portraitUrl: storyboardManagedAssetUrl(portraitByCharacter.get(character.id)?.storageKey),
  }));
...
async function updateDraftCharacterIds(input: {
  ...
  const [updated] = await input.db
    ...
    .returning({ id: storyboardSkillRuns.id });
  if (!updated) throw new Error("Characters can only be changed before confirmation");
...
export async function bindStoryboardProjectCharacter(input: {
  ...
  try {
    return await db.transaction(async tx => {
      ...
      const [project] = await tx
        .select({ activeRunId: storyboardSkillProjects.activeRunId, status: storyboardSkillProjects.status })
        .from(storyboardSkillProjects)
        .where(and(
          eq(storyboardSkillProjects.id, input.projectId),
          eq(storyboardSkillProjects.tenantId, tenantId),
        ...
        .limit(1);
      if (!project) throw new Error("Storyboard project not found");
      if (project.status === "archived") throw new Error("Archived storyboard projects cannot be changed");
      const [currentRun] = project.activeRunId
        ? await tx.select().from(storyboardSkillRuns).where(and(
          eq(storyboardSkillRuns.id, project.activeRunId),
          eq(storyboardSkillRuns.projectId, input.projectId),
          eq(storyboardSkillRuns.tenantId, tenantId),
          eq(storyboardSkillRuns.userId, input.userId),
      ...
      const [existing] = await tx
        .select({ id: storyboardSkillProjectCharacters.id, lookId: storyboardSkillProjectCharacters.lookId })
        .from(storyboardSkillProjectCharacters)
        .where(and(
          eq(storyboardSkillProjectCharacters.projectId, input.projectId),
        ...
        .limit(1);
      if (existing) {
        if (existing.lookId !== (input.lookId ?? null)) {
          await tx.update(storyboardSkillProjectCharacters)
            .set({ lookId: input.lookId ?? null, updatedAt: new Date() })
      ...
      const currentIds = currentRun
        ? normalizeStoryboardGlobalInput(currentRun.normalizedSnapshot).characterIds
        : [];
...
export async function unbindStoryboardProjectCharacter(input: {
  ...
  return db.transaction(async tx => {
    ...
    const [binding] = await tx
      ...
      .limit(1);
    if (!binding) {
      return { projectId: input.projectId, characterId: input.characterId, removed: false };
    }
    const [currentRun] = project.activeRunId
      ? await tx.select().from(storyboardSkillRuns).where(and(
        eq(storyboardSkillRuns.id, project.activeRunId),
    ...
    await updateDraftCharacterIds({
      ...
      characterIds: currentRun
        ? normalizeStoryboardGlobalInput(currentRun.normalizedSnapshot).characterIds.filter(id => id !== input.characterId)
        : [],
    });
    const [removed] = await tx.delete(storyboardSkillProjectCharacters)
      .where(and(

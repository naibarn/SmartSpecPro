 ...
 *
 * The whole clone runs inside ONE transaction so a mid-clone failure (e.g. a
 * DB error on the 40th character) never leaves a half-cloned cast behind —
 * either every table below is copied consistently, or none of it is.
 *
 * Transaction ownership (Stage 2.3 follow-up — orphan-row fix): this
 * function no longer unconditionally opens its OWN `db.transaction`. The
 * caller (`server/routers/verticalDramaSeries.ts`'s `create`) now runs the
 * series row INSERT and this clone inside the SAME outer transaction and
 * passes that transaction's handle as the optional second argument. That
 * way a clone failure rolls back the just-inserted series row too — there
 * is no longer a window where a sequel/special-edition row can persist with
 * `parentSeriesId` set and zero cloned cast (previously the insert and the
 * clone were two independently-committed operations, and the caller only
 * `debugError`'d a clone failure instead of surfacing or rolling it back).
 * When no `tx` is passed (e.g. this module's own unit tests, or any other
 * future caller that hasn't been updated), the function falls back to
 * opening its own `db.transaction`, preserving the original atomic-clone
 * behavior standalone.
 */
 ...
 * clones zero rows, not an error) — a genuine DB error still propagates
 * from here, and the CALLER decides what that means (the router's `create`
 * now treats it as a hard failure that rolls back the shared transaction —
 * see this file's header doc comment for the transaction-ownership
 * contract).
 *
 * `tx` — when provided, an ALREADY-OPEN transaction handle (typically the
 * same transaction the caller just inserted the child series row in) that
 * this function reuses instead of opening its own. When omitted, this
 * function opens its own `db.transaction` exactly like before this doc
 * comment's own header note — standalone-atomic, but independent of any
 * caller-side insert.
 */
export async function cloneSeriesCastForLineage(
  ...
  if (tx) return runCloneSeriesCastForLineage(tx, params);
  return db.transaction(t => runCloneSeriesCastForLineage(t, params));
}

async function runCloneSeriesCastForLineage(
  tx: DrizzleDB,
  params: CloneSeriesCastForLineageParams
): Promise<CloneSeriesCastForLineageSummary> {

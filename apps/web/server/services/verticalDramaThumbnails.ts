...
async function resolveMediaAssetUrls(
  ...
  const rows: Array<{ id: number; thumbnailUrl: string | null; originalUrl: string | null }> =
    await db
      .select({
        id: mediaAssets.id,
...
export async function resolveSeriesThumbnailUrls(
  ...
  const mediaAssetIds = Array.from(new Set(Array.from(candidateBySeriesId.values()).map(Number)))
    .filter((id) => Number.isFinite(id));
  const urlByAssetId = await resolveMediaAssetUrls(db, { tenantId, userId }, mediaAssetIds);
...
export async function resolveEpisodeThumbnailUrls(
  ...
  const rows: Array<{ id: number; approvedMediaAssetId: string | null }> = await db
    .select({
  ...
  for (const row of rows) {
    if (row.approvedMediaAssetId) candidateByEpisodeId.set(row.id, row.approvedMediaAssetId);
  }

  const mediaAssetIds = Array.from(new Set(Array.from(candidateByEpisodeId.values()).map(Number)))
    .filter((id) => Number.isFinite(id));
  const urlByAssetId = await resolveMediaAssetUrls(db, { tenantId, userId }, mediaAssetIds);

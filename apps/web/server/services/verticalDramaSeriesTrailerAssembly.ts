...
async function runTrailerJob(args: {
  ...
  try {
    // 1. Download narration audio, probe duration if not supplied.
    const audioPath = path.join(workDir, "narration.audio");
    await downloadToFile(args.audioUrl, audioPath, internalBaseUrl);
    let narrationDurationSeconds = args.audioDurationSeconds;
    if (!narrationDurationSeconds || narrationDurationSeconds <= 0) {
      narrationDurationSeconds = await probeDurationSeconds(audioPath);
    }
    if (!narrationDurationSeconds || narrationDurationSeconds <= 0) {
    ...
    await persistTrailerState(owner, {
      status: "completed",
      jobId: undefined,
      narrationAudioUrl: args.audioUrl,

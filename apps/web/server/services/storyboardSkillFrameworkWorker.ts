...
import { mediaAssets, storyboardSkillProjects, storyboardSkillRuns, storyboardSkillShots } from "../../drizzle/schema";
...
async function resolveReferenceUrls(input: {
  ...
  const rows = numericIds.length > 0
    ? await db.select({ id: mediaAssets.id, storageKey: mediaAssets.storageKey })
      .from(mediaAssets)
      .where(and(
...
async function loadRun(runId: string, scope: StoryboardWorkerScope) {
  ...
  if (!run) return null;
  const shots = await db.select().from(storyboardSkillShots).where(and(
...
async function setRunStatus(
  ...
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const allowedPreviousStatuses = status === "running"
  ...
  return db.transaction(async tx => {
    ...
    const [updated] = await tx.update(storyboardSkillRuns).set({
      status,
    ...
    await tx.update(storyboardSkillProjects).set({ status, updatedAt: new Date() }).where(and(
      eq(storyboardSkillProjects.id, projectId),
      eq(storyboardSkillProjects.tenantId, scope.tenantId),
      eq(storyboardSkillProjects.userId, scope.userId),
    ));
...
async function markShotGenerating(scope: StoryboardWorkerScope, runId: string, shotId: number): Promise<number | null> {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const [row] = await db.update(storyboardSkillShots)
    .set({ status: "generating", attempt: sql`${storyboardSkillShots.attempt} + 1`, updatedAt: new Date(), suppressedResult: false })
    .where(and(
      eq(storyboardSkillShots.id, shotId),
      eq(storyboardSkillShots.runId, runId),
      eq(storyboardSkillShots.tenantId, scope.tenantId),
      inArray(storyboardSkillShots.status, ["pending", "prompt_ready", "failed", "partial"]),
    ))
    .returning({ attempt: storyboardSkillShots.attempt });
  return row?.attempt ?? null;
}

async function bindProviderOperationKey(scope: StoryboardWorkerScope, runId: string, shotId: number, operationKey: string): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.update(storyboardSkillShots).set({ providerOperationKey: operationKey, updatedAt: new Date() }).where(and(
    eq(storyboardSkillShots.id, shotId),
    eq(storyboardSkillShots.runId, runId),
    eq(storyboardSkillShots.tenantId, scope.tenantId),
...
async function markSuppressed(input: { scope: StoryboardWorkerScope; runId: string; projectId: string; shotId: number; operationKey: string; message: string }) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.transaction(async tx => {
    ...
    await tx.update(storyboardSkillShots).set({
      ...
      error: { code: "STOPPED_RESULT_SUPPRESSED", message: input.message, providerOperationKey: input.operationKey },
      updatedAt: new Date(),
    }).where(and(
    ...
    if (run?.status === "queued" || run?.status === "running" || run?.status === "partial") {
      await tx.update(storyboardSkillRuns).set({ status: "paused", error: { code: "PAUSED_BY_USER", message: "Generation stopped; provider result was not accepted for continuity." }, updatedAt: new Date() }).where(and(
        eq(storyboardSkillRuns.id, input.runId),
        eq(storyboardSkillRuns.tenantId, input.scope.tenantId),
        eq(storyboardSkillRuns.userId, input.scope.userId),
      ));
...
async function markSucceeded(input: { scope: StoryboardWorkerScope; runId: string; projectId: string; shotId: number; assetId: number; operationKey: string; request: Record<string, unknown>; referenceAssetIds: string[]; videoPrompt: string }): Promise<boolean> {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  return db.transaction(async tx => {
    const [project] = await tx.select({ id: storyboardSkillProjects.id })
      ...
      .limit(1);
    if (!project) return false;
    const [run] = await tx.select({ status: storyboardSkillRuns.status }).from(storyboardSkillRuns).where(and(
      eq(storyboardSkillRuns.id, input.runId),
      eq(storyboardSkillRuns.tenantId, input.scope.tenantId),
      eq(storyboardSkillRuns.userId, input.scope.userId),
    )).limit(1);
    if (!run || isStoryboardExecutionStopped(run.status)) return false;
    const [updated] = await tx.update(storyboardSkillShots).set({
      status: "succeeded",
      imageAssetId: input.assetId,
      providerOperationKey: input.operationKey,
      effectiveGenerationRequest: input.request,
...
async function markFailure(input: { scope: StoryboardWorkerScope; runId: string; projectId: string; shotId: number; operationKey: string; error: ReturnType<typeof classifyStoryboardGenerationError>; prompt: string; request: Record<string, unknown>; providerSubmissionStarted?: boolean; creditSettledBeforeProvider?: boolean }) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const repairedPrompt = input.error.class === "policy"
    ? optimizeStoryboardPromptForConstraint(input.prompt, input.error.code)
    : input.prompt;
  ...
  await db.transaction(async tx => {
    ...
    await tx.update(storyboardSkillShots).set({
      ...
      error: stopped
        ? { code: "STOPPED_RESULT_SUPPRESSED", class: input.error.class, message: "Provider result was not accepted after the user stopped the storyboard.", providerOperationKey: input.operationKey }
        : {
          code: input.error.code,
          class: input.error.class,
          message: input.error.message,
          detail: input.error.detail,
          ...(input.error.statusCode ? { statusCode: input.error.statusCode } : {}),
          providerSubmissionStarted: input.providerSubmissionStarted ?? false,
          creditSettledBeforeProvider: input.creditSettledBeforeProvider ?? false,
          repairAction: input.error.class === "policy" ? "optimize_prompt" : "reuse_prompt",
          providerOperationKey: input.operationKey,
        },
    ...
    if (!stopped) {
      await tx.update(storyboardSkillRuns).set({
        status: "partial",
        error: {
          code: input.error.code,
          class: input.error.class,
      ...
      await tx.update(storyboardSkillProjects).set({ status: "partial", updatedAt: new Date() }).where(and(
        eq(storyboardSkillProjects.id, input.projectId),
        eq(storyboardSkillProjects.tenantId, input.scope.tenantId),
        eq(storyboardSkillProjects.userId, input.scope.userId),
      ));
...
export async function runStoryboardSkillBackground(input: WorkerInput): Promise<JobResult> {
  if (input.context.jobType !== "storyboard.skill.run") throw new Error("STORYBOARD_JOB_TYPE_MISMATCH");
  if (!Number.isSafeInteger(input.context.requestedByUserId) || input.context.requestedByUserId <= 0) {
  ...
  if (run.status === "awaiting_confirmation") throw new Error("STORYBOARD_RUN_NOT_CONFIRMED");
  if (run.status === "cancelled") {
    return { deferred: true, output: { runId: run.id, status: "cancelled" } };
  }
  if (isStoryboardExecutionStopped(run.status) && run.status !== "cancelled") {
    await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}`, resumeAfter: PAUSED_RESUME_AFTER });
    return { deferred: true, output: { runId: run.id, status: "paused" } };
  }
  const global = run.normalizedSnapshot as unknown as StoryboardGlobalInput;
  const plannedShots = planStoryboardShots(global);
  const startedStatus = await setRunStatus(run.id, run.projectId, "running", scope);
  if (!startedStatus) {
    await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}`, resumeAfter: PAUSED_RESUME_AFTER });
    return { deferred: true, output: { runId: run.id, status: "paused" } };
  }
  ...
  if (started.run.status === "cancelled") {
    return { deferred: true, output: { runId: run.id, status: "cancelled" } };
  }
  if (isStoryboardExecutionStopped(started.run.status) && started.run.status !== "cancelled") {
    await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}`, resumeAfter: PAUSED_RESUME_AFTER });
    return { deferred: true, output: { runId: run.id, status: "paused" } };
  }
  ...
  for (const shot of shots) {
    ...
    if (live.run.status === "cancelled") {
      return { deferred: true, output: { runId: run.id, status: "cancelled" } };
    }
    if (isStoryboardExecutionStopped(live.run.status) && live.run.status !== "cancelled") {
      await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}`, resumeAfter: PAUSED_RESUME_AFTER });
      return { deferred: true, output: { runId: run.id, status: "paused" } };
    }
    if (isReusableStoryboardImage(shot)) {
      previousImageAssetId = String(shot.imageAssetId);
      continue;
    }
    if (shot.status === "generating") {
      await markFailure({
        ...
        shotId: shot.id,
        operationKey: shot.providerOperationKey ?? `storyboard:${run.id}:shot:${shot.shotNumber}:ambiguous`,
        error: {
          class: "unknown",
          code: "IMAGE_OPERATION_AMBIGUOUS",
          message: "A provider operation was in progress when the worker lease was lost.",
      ...
      await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}:ambiguous:${shot.shotNumber}`, resumeAfter: PAUSED_RESUME_AFTER });
      return { deferred: true, output: { runId: run.id, status: "partial", shotNumber: shot.shotNumber } };
    }
    const shotError = asRecord(shot.error);
    const isAutomaticRetry =
      (shot.status === "failed" || shot.status === "partial") &&
      shotError.class === "transient";
    if ((shot.status === "failed" || shot.status === "partial") && !isAutomaticRetry) {
    ...
    const prompt = String(request.prompt ?? shot.generationPrompt ?? "").trim();
    ...
    try {
      ...
      const refs = [
        ...(previousImageAssetId ? [previousImageAssetId] : []),
        ...referenceAssetIds(global),
        ...(shot.referenceAssetIds ?? []),
      ].filter((value, index, all) => all.indexOf(value) === index).slice(0, 5);
      const referenceImageUrls = await resolveReferenceUrls({ tenantId: run.tenantId, userId: run.userId, assetIds: refs });
      await reportShotStage({
        reporter: input.reporter,
        lease: input.lease,
        shotNumber: shot.shotNumber,
      ...
      const response = await withLeaseHeartbeat({
        lease: input.lease,
        reporter: input.reporter,
        operation: () => mediaGenerationService.generateImage({
          prompt,
          ...
          onSubmissionReady: async () => {
            ...
            await deductCredits({
              ...
              sourceType: "skill",
              metadata: { runId: run.id, shotNumber: shot.shotNumber, attempt, operationKey, model: global.imageModelSelection.modelId },
            });
            creditSettled = true;
          },
          onSubmissionStarted: () => {
          ...
          referenceImageUrls,
          extraParams: global.imageModelSelection.quality ? { quality: global.imageModelSelection.quality } : undefined,
          publicUrl: getCachedPublicAppUrl(),
          auditContext: { userId: run.userId, tenantId: run.tenantId, skillRunId: creditKey, skillSlug: global.selectedSkillId, source: "storyboard_skill_framework", stage: `shot_${shot.shotNumber}`, operationKey },
        }, createInternalTokenFromAuth({ userId: run.userId, tenantId: run.tenantId }, ["media:generate"])),
      });
      await input.reporter.assertActive(input.lease);
      await reportShotStage({
        reporter: input.reporter,
        ...
        measured: { attempt, operationKey, providerSubmissionStarted, creditSettled },
      });
      const current = await loadRun(run.id, scope);
      if (!current || isStoryboardExecutionStopped(current.run.status)) {
        await markSuppressed({ scope, runId: run.id, projectId: run.projectId, shotId: shot.id, operationKey, message: "Provider returned after the user stopped the storyboard." });
        if (current?.run.status === "cancelled") {
          return { deferred: true, output: { runId: run.id, status: "cancelled", suppressedShotNumber: shot.shotNumber } };
        }
        await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}:provider:${shot.shotNumber}`, resumeAfter: PAUSED_RESUME_AFTER });
        return { deferred: true, output: { runId: run.id, status: "paused", suppressedShotNumber: shot.shotNumber } };
      }
      const durable = await durabilizeMediaGenerationResponse(response, { tenantId: run.tenantId, userId: run.userId, mediaType: "image", sourceType: "storyboard_skill_generated", identity: operationKey });
      const assetId = Number(durable.data?.[0]?.data?.mediaAssetId);
      if (!Number.isSafeInteger(assetId) || assetId <= 0) throw new Error("STORYBOARD_IMAGE_ASSET_MISSING");
      await reportShotStage({
        reporter: input.reporter,
        lease: input.lease,
        shotNumber: shot.shotNumber,
      ...
      const plannedShot = plannedShots.find(item => item.shotNumber === shot.shotNumber);
      const videoPrompt = buildStoryboardVideoPrompt({
        shot: plannedShot ?? { shotNumber: shot.shotNumber, beat: shot.beat, context: shot.context, continuity: "", dialogueLines: [] },
        imageAssetId: String(assetId),
        videoModelId: global.videoModelSelection.modelId,
        language: global.language,
      });
      const accepted = await markSucceeded({ scope, runId: run.id, projectId: run.projectId, shotId: shot.id, assetId, operationKey, request: { ...request, prompt, reference_images: refs.map(asset_id => ({ asset_id })) }, referenceAssetIds: refs, videoPrompt });
      if (!accepted) {
        await markSuppressed({ scope, runId: run.id, projectId: run.projectId, shotId: shot.id, operationKey, message: "Provider returned after the user stopped the storyboard." });
        await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}:race:${shot.shotNumber}`, resumeAfter: PAUSED_RESUME_AFTER });
        return { deferred: true, output: { runId: run.id, status: "paused", suppressedShotNumber: shot.shotNumber } };
      }
      await reportShotStage({
        reporter: input.reporter,
      ...
      previousImageAssetId = String(assetId);
    } catch (error) {
      ...
      await reportShotStage({
        ...
        measured: { attempt, operationKey, class: recoveryError.class, providerSubmissionStarted, creditSettled },
      });
      await markFailure({ scope, runId: run.id, projectId: run.projectId, shotId: shot.id, operationKey, error: recoveryError, prompt, request, providerSubmissionStarted, creditSettledBeforeProvider: creditSettled });
      if (recoveryError.class === "transient") {
        ...
        return { deferred: true, output: { runId: run.id, status: "partial", failedShotNumber: shot.shotNumber, failureClass: recoveryError.class } };
      }
  ...
  const allSucceeded = final.shots.length > 0 && final.shots.every(shot => shot.status === "succeeded" && shot.imageAssetId && !shot.suppressedResult);
  if (!allSucceeded) {
  const partialStatus = await setRunStatus(run.id, run.projectId, "partial", scope);
    if (!partialStatus) {
      await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}`, resumeAfter: PAUSED_RESUME_AFTER });
      return { deferred: true, output: { runId: run.id, status: "paused" } };
    }
    await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}:incomplete`, resumeAfter: PAUSED_RESUME_AFTER });
    return { deferred: true, output: { runId: run.id, status: "partial" } };
  }
  try {
    const { rebuildStoryboardSkillReviewProjection } = await import("./storyboardSkillFrameworkService");
    const projection = await rebuildStoryboardSkillReviewProjection({ userId: run.userId, tenantId: run.tenantId, runId: run.id });
    if (projection.projectionStatus !== "ready") throw new Error("STORYBOARD_REVIEW_PROJECTION_PENDING");
  } catch {
    await setRunStatus(run.id, run.projectId, "partial", scope, {
      ...
      // evidence gates.
      class: "projection",
      message: "The storyboard review projection is not ready and requires repair.",
    });
    ...
    return { deferred: true, output: { runId: run.id, status: "partial", projectionPending: true } };
  }
  const succeededStatus = await setRunStatus(run.id, run.projectId, "succeeded", scope);
  if (!succeededStatus) {
    await input.reporter.waitForExternal(input.lease, { operationKey: `storyboard.pause:${run.id}`, resumeAfter: PAUSED_RESUME_AFTER });
    return { deferred: true, output: { runId: run.id, status: "paused" } };
  }
  return { output: { runId: run.id, status: "succeeded", shotCount: final.shots.length } };
}

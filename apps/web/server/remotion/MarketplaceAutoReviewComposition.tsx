 ...
 *
 * Phase 2 of planning/remotion-migration/plan.md — supports ONLY the
 * schema-default preset combination (see `remotionCompositionService.ts`'s
 * `assertRemotionPresetSupport`): `fade`/`none` transitions,
 * `smooth_reveal` shot animation, `stagger_rise` on-screen-text motion, and
 * `classic_box` burned-in subtitles. This does not aim for pixel parity
 * with the HyperFrames CSS/GSAP renderer — it is a real, visible, timed
...
} from "../services/remotionCompositionService";
...
/**
 * Loads the staged Thai-capable font (see `remotionRuntimeAdapter.ts`'s
 * `resolveAndStageRenderFont`, served over the render workspace's local
 * asset server) via the browser `FontFace` API, and blocks frame capture
 * (`delayRender`/`continueRender` — Remotion's documented pattern for
 * custom/local fonts, see https://www.remotion.dev/docs/fonts) until the
 * font has actually loaded into `document.fonts`. Renders nothing itself;
 * `fontFamily` is then usable as a plain CSS `font-family` value by sibling
 * elements once loading completes.
 *
 * No-op (immediately unblocks capture) when `fontFaceUrl` is `null` — the
 * soft-degrade case where no Thai font could be resolved at render time
 * (see `resolveAndStageRenderFont`'s diagnostic warning). In that case
 * elements referencing `fontFamily` simply fall back to the browser's
 * default font.
 */

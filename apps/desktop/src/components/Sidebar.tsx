...
const sharedMenuItems = getMenuItemsByGroup('desktop', 'user', 'main');

export const defaultNavigationItems: NavigationItem[] = sharedMenuItems.map(item => ({
  id: item.id,
  label: item.label,
  icon: item.icon,
}));
    ...
    <div style={{ width: 240, padding: 12, borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", height: "100vh" }}>
      ...
      <nav style={{ display: "grid", gap: 6, flex: 1 }}>
        {sharedMenuItems.map(item => (
          <NavLink key={item.id} to={item.path} end={item.path === '/'} style={linkStyle}>
            {item.label}
          </NavLink>
        ))}

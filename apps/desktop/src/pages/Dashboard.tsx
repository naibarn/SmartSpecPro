...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    if (!token) {
      // No Python backend token — just show web credits
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Try loading from Python backend (optional — may not be running)
      ...
      const [summaryRes, usageRes, llmUsageRes, transactionsRes] = results;
      ...
      if (llmUsageRes.status === "fulfilled" && llmUsageRes.value.ok) {
        setLLMUsage(await llmUsageRes.value.json());
      }
      ...
      // Don't set error — web credits section still works
    } finally {
      setLoading(false);
    }
  };

...
impl ComfyConnectionProfile {
    pub fn validate(&self) -> Result<(), String> {
        ...
        match self.transport {
            ComfyTransportKind::LocalStdio => {
                let command = self
                    .command
                    .as_deref()
                    .ok_or("comfy_profile_command_missing")?;
                validate_command(command)
                    .map_err(|_| "comfy_profile_command_invalid".to_string())?;
                if self.endpoint.is_some() {
                    return Err("comfy_local_stdio_endpoint_forbidden".into());
                }
            }
            ComfyTransportKind::SelfHostedStdioBridge => {
                self.endpoint
                    ...
                    .ok_or("comfy_bridge_endpoint_missing")?;
                let command = self
                    .command
                    .as_deref()
                    .ok_or("comfy_profile_command_missing")?;
            ...
            ComfyTransportKind::SelfHostedHttpMcp => {
                validate_http_endpoint(self.endpoint.as_deref(), false)?
            }
            ComfyTransportKind::ComfyCloud => {
                validate_http_endpoint(self.endpoint.as_deref(), true)?
            }
        ...
        if self.credential_kind != ComfyCredentialKind::None
    ...
    pub fn projection(&self) -> ComfyProfileProjection {
        ComfyProfileProjection {
            ...
            credential_configured: self.credential_kind == ComfyCredentialKind::None
...
pub fn resolve_bridge_args(args: &[String], endpoint: &str) -> Result<Vec<String>, String> {
    ...
    Ok(args
        .iter()
        .map(|arg| arg.replace("{endpoint}", endpoint))
        .collect())
}
...
impl ComfyProfileStore {
    pub fn load(app_data_dir: &Path) -> Result<Self, String> {
        ...
        if !path.exists() {
            return Ok(Self {
                data: PersistedProfiles::default(),
                path: Some(path),
            });
        }
        let raw =
            fs::read_to_string(&path).map_err(|_| "comfy_profiles_read_failed".to_string())?;
        let mut data: PersistedProfiles =
            serde_json::from_str(&raw).map_err(|_| "comfy_profiles_invalid".to_string())?;
        ...
        if data.schema_version != PROFILE_SCHEMA_VERSION {
            return Err("comfy_profiles_schema_unsupported".into());
        }
        ...
        if let Some(active) = &data.active_profile_id {
            if !data
                .profiles
                .iter()
                .any(|profile| profile.profile_id == *active && profile.enabled)
            {
    ...
    pub fn profiles(&self) -> impl Iterator<Item = &ComfyConnectionProfile> {
        self.data.profiles.iter()
    }
    pub fn active_profile(&self) -> Option<&ComfyConnectionProfile> {
        self.data.active_profile_id.as_ref().and_then(|id| {
            self.data
                .profiles
                .iter()
    ...
    pub fn projections(&self) -> Vec<ComfyProfileProjection> {
        self.data
            .profiles
            .iter()
            .map(ComfyConnectionProfile::projection)
            .collect()
    }
    ...
    pub fn activate(&mut self, profile_id: &str) -> Result<(), String> {
        let profile = self
            ...
            .profiles
            .iter()
            .find(|item| item.profile_id == profile_id && item.enabled)
            .ok_or("comfy_profile_not_found")?;
    ...
    pub fn disable(&mut self, profile_id: &str) -> Result<(), String> {
        ...
        if self.data.active_profile_id.as_deref() == Some(profile_id) {
    ...
    pub fn record_probe(&mut self, profile_id: &str, status: &str) -> Result<(), String> {
        ...
        let profile = self
            ...
            .find(|item| item.profile_id == profile_id)
            .ok_or("comfy_profile_not_found")?;
        profile.last_probe_at = Some(
            time::OffsetDateTime::now_utc()
                .format(&time::format_description::well_known::Rfc3339)
                .map_err(|_| "comfy_probe_time_failed")?,
        );
    ...
    fn persist(&self) -> Result<(), String> {
        ...
        let bytes = serde_json::to_vec_pretty(&self.data)
            .map_err(|_| "comfy_profiles_encode_failed".to_string())?;
...
fn validate_http_endpoint(endpoint: Option<&str>, cloud: bool) -> Result<(), String> {
    ...
    if cloud {
        if value != "https://cloud.comfy.org/mcp" {
            return Err("comfy_cloud_endpoint_not_allowlisted".into());
        }
        return Ok(());
    }
    ...
    if !(https || loopback_http) || value.contains(['?', '#', '\\', '@']) {
        return Err("comfy_http_endpoint_not_allowed".into());
    }
    Ok(())
}
...
mod tests {
    ...
    fn profile(transport: ComfyTransportKind) -> ComfyConnectionProfile {
        ComfyConnectionProfile {
            ...
            display_name: "Local Comfy".into(),
            transport,
            endpoint: None,
            command: Some("comfy-mcp".into()),
            args: Vec::new(),
    ...
    fn validates_all_connection_modes_without_secret_values() {
        ...
        let mut bridge = profile(ComfyTransportKind::SelfHostedStdioBridge);
        bridge.endpoint = Some("https://comfy.example.test/mcp".into());
        bridge.args = vec!["--endpoint".into(), "{endpoint}".into()];
        assert!(bridge.validate().is_ok());
        bridge.args = vec!["--endpoint".into()];
        assert_eq!(
            bridge.validate().expect_err("bridge target placeholder"),
            "comfy_bridge_endpoint_placeholder_missing"
        );
        let mut http = profile(ComfyTransportKind::SelfHostedHttpMcp);
        http.command = None;
        http.endpoint = Some("https://comfy.example.test/mcp".into());
        assert!(http.validate().is_ok());
        let mut cloud = profile(ComfyTransportKind::ComfyCloud);
        cloud.command = None;
        cloud.endpoint = Some("https://cloud.comfy.org/mcp".into());
        cloud.credential_kind = ComfyCredentialKind::ApiKey;
        cloud.credential_ref = Some("keychain:comfy/cloud".into());
        assert!(cloud.validate().is_ok());
        let mut ssh = profile(ComfyTransportKind::SshTunnel);
        ssh.command = None;
        ssh.endpoint = Some("http://127.0.0.1:8188/mcp".into());
        ssh.args = vec![
            "-N".into(),
            "-o".into(),
            ...
            "-L".into(),
            "127.0.0.1:8188:127.0.0.1:8188".into(),
            "user@example.test".into(),
        ];
        ssh.credential_kind = ComfyCredentialKind::SshKeychainRef;
        ssh.credential_ref = Some("keychain:ssh/comfy".into());
        assert!(ssh.validate().is_ok());
    }

    #[test]
    fn rejects_untrusted_endpoints_and_projects_only_redacted_metadata() {
        let mut ssrf_profile = profile(ComfyTransportKind::SelfHostedHttpMcp);
        ssrf_profile.command = None;
        ssrf_profile.endpoint = Some("http://169.254.169.254/mcp".into());
        assert_eq!(
            ssrf_profile.validate().expect_err("SSRF endpoint"),
            "comfy_http_endpoint_not_allowed"
        );
        let mut profile2 = profile(ComfyTransportKind::SelfHostedHttpMcp);
        profile2.command = None;
        profile2.endpoint = Some("https://user:secret@comfy.example.test/mcp".into());
        assert_eq!(
            profile2.validate().expect_err("endpoint userinfo"),
            "comfy_http_endpoint_not_allowed"
        );
    ...
    fn active_profile_is_single_and_disable_clears_it() {
        ...
        store
            .upsert(profile(ComfyTransportKind::LocalStdio))
            .expect("upsert");
        store.activate("profile-1").expect("activate");
        assert_eq!(
            store.active_profile().map(|item| item.profile_id.as_str()),
            Some("profile-1")
        );

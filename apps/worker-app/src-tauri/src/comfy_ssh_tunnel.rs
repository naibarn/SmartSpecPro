...
pub fn validate_args(args: &[String]) -> Result<(), String> {
    if args.is_empty()
        || args
            .iter()
            .any(|arg| arg.is_empty() || arg.len() > 512 || arg.chars().any(|ch| ch.is_control()))
    {
    ...
    if args.iter().any(|arg| {
        arg == "-R"
            ...
            || arg.contains("StrictHostKeyChecking=no")
    }) {
    ...
    for (index, arg) in args.iter().enumerate() {
        ...
        if arg == "-L" {
            let spec = args
                .get(index + 1)
                .ok_or("comfy_ssh_forwarding_target_missing")?;
        ...
        if arg == "-o"
            && args
                .get(index + 1)
                .is_some_and(|value| value == "ExitOnForwardFailure=yes")
        {
        ...
        if arg == "-o"
            && args
                .get(index + 1)
                .is_some_and(|value| value == "StrictHostKeyChecking=yes")
        {
            has_host_key_check = true;
        }
        if arg == "StrictHostKeyChecking=yes" {
            has_host_key_check = true;
        }
        if arg.starts_with("UserKnownHostsFile=") && arg.len() > "UserKnownHostsFile=".len() {
    ...
    if !has_forward
        ...
        || !has_known_hosts
    {
        return Err("comfy_ssh_forwarding_not_fail_closed".into());
    }
    Ok(())
}
...
fn validate_forward_spec(spec: &str) -> Result<(), String> {
    ...
    if parts.len() != 4
        ...
        || parts[3].parse::<u16>().is_err()
    {
        return Err("comfy_ssh_forwarding_target_invalid".into());
    }
    if parts[1].parse::<u16>().unwrap_or(0) < 1 || parts[3].parse::<u16>().unwrap_or(0) < 1 {
        return Err("comfy_ssh_forwarding_port_invalid".into());
    }
    Ok(())
}
...
pub fn open_with_identity(args: &[String], private_key: &str) -> Result<SshTunnelProcess, String> {
    validate_args(args)?;
    if private_key.trim().is_empty() || private_key.len() > 64 * 1024 {
        return Err("comfy_ssh_identity_invalid".into());
    }
    let path = std::env::temp_dir().join(format!(
        "smartaihub-comfy-ssh-{}-{}.key",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map_err(|_| "comfy_ssh_identity_invalid")?
            .as_nanos()
    ));
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(&path, fs::Permissions::from_mode(0o600))
            .map_err(|_| "comfy_ssh_identity_permissions_failed".to_string())?;
    }
...
fn open_inner(
    ...
    key_path: Option<PathBuf>,
    validate: bool,
) -> Result<SshTunnelProcess, String> {
    if validate {
        validate_args(args)?;
    }
...
mod tests {
    ...
    fn requires_restricted_forwarding_and_fail_closed_option() {
        ...
        assert!(super::validate_args(&[
            "-N".into(),
            "-L".into(),
            "0.0.0.0:8188:127.0.0.1:8188".into()
        ])
        .is_err());
        assert!(
            super::validate_args(&["-N".into(), "-R".into(), "8188:127.0.0.1:8188".into()])
                .is_err()
        );
    }
}

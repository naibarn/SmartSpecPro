...
pub fn validate_provider_url(profile: &LocalLlmProviderProfile) -> Result<Url, String> {
    let url =
        Url::parse(profile.base_url.trim()).map_err(|_| "provider URL is invalid".to_string())?;
...
pub fn resolve_model<'a>(
    registry: &'a LocalLlmRegistry,
    request: &LocalLlmRequest,
) -> Result<(&'a LocalLlmProviderProfile, &'a LocalLlmModelRecord), String> {
    let model = registry
        .models
        .iter()
        .find(|item| {
        ...
        .ok_or_else(|| "local model is unavailable".to_string())?;
    let provider = registry
        .providers
        .iter()
        .find(|item| item.enabled && item.local_provider_id == request.local_provider_id)
...
pub fn build_openai_chat_payload(
    ...
    if request.stream
        && model
            .capabilities
            .iter()
            .all(|cap| cap != "llm.chat" && cap != "llm.completion")
    {
    ...
    if let Some(parameters) = request.parameters.as_object() {
        for (key, value) in parameters {
            if matches!(
                key.as_str(),
                "temperature" | "top_p" | "maxTokens" | "max_tokens" | "response_format"
            ) {
...
pub async fn execute_openai_compatible(
    ...
    if cancel.load(Ordering::Relaxed) {
        return Err("canceled before provider send".into());
    }
    let base = validate_provider_url(profile)?;
    let endpoint = base
        .join("/v1/chat/completions")
        .map_err(|_| "provider endpoint is invalid".to_string())?;
    ...
    if cancel.load(Ordering::Relaxed) {
        return Err("canceled after provider response".into());
    }
...
mod tests {
    ...
    fn validates_local_url_and_redacts_credentials_from_payload() {
        ...
        remote.base_url = "http://example.test".into();
        assert!(validate_provider_url(&remote).is_err());
        let model = LocalLlmModelRecord {
            local_provider_id: "p".into(),
            local_model_id: "m".into(),
            provider_model_id: "llama3".into(),
        ...
        let request = LocalLlmRequest {
            ...
            model_ref: "wllm_ref".into(),
            local_provider_id: "p".into(),
            local_model_id: "m".into(),
            inventory_revision: 1,
            messages: vec![json!({"role":"user","content":"hello"})],
            parameters: json!({"apiKey":"must-not-forward","temperature":0.2}),
            stream: false,
        };
    ...
    fn normalizes_completion_and_rejects_empty_input() {
        ...
        assert!(build_openai_chat_payload(
            &request,
            &LocalLlmModelRecord {
                local_provider_id: "p".into(),
                local_model_id: "m".into(),
        ...
        .is_err());
        let valid = LocalLlmRequest {
            messages: vec![json!({"role":"user","content":"hello"})],
            ..request
        };
        let result = normalize_openai_response(
            &valid,

...
impl ComfyHttpMcpTransport {
    pub fn new(
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|_| "comfy_http_client_failed".to_string())?;
        Ok(Self {
            client,
            endpoint,
            bearer_token,
    ...
    pub async fn call(&mut self, id: u64, method: &str, params: Value) -> Result<Value, String> {
        if method.trim().is_empty() {
            return Err("comfy_mcp_method_missing".into());
        }
        ...
        let response = request
            .send()
            .await
            .map_err(|_| "comfy_mcp_http_unreachable".to_string())?;
        if response.status() == StatusCode::UNAUTHORIZED {
            return Err("comfy_mcp_http_unauthorized".into());
        }
        if response.status() == StatusCode::FORBIDDEN {
            return Err("comfy_mcp_http_forbidden".into());
        }
        if response.status() == StatusCode::NOT_FOUND {
            self.session_id = None;
            return Err("comfy_mcp_http_session_not_found".into());
        }
        if !response.status().is_success() {
            return Err("comfy_mcp_http_failed".into());
        }
        ...
        let body = response
            .bytes()
            .await
            .map_err(|_| "comfy_mcp_http_invalid_response".to_string())?;
        if body.len() > MAX_MCP_RESPONSE_BYTES {
            return Err("comfy_mcp_http_response_too_large".into());
        }
    ...
    async fn notify(&mut self, method: &str, params: Value) -> Result<(), String> {
        ...
        if !response.status().is_success() {
            return Err("comfy_mcp_http_notification_failed".into());
        }
        if let Some(session_id) = response
            .headers()
            .get("Mcp-Session-Id")
        ...
        for page in 0..MAX_MCP_TOOL_PAGES {
            if let Some(cursor_value) = cursor.as_ref() {
                if !seen_cursors.insert(cursor_value.clone()) {
                    return Err("comfy_mcp_http_tools_list_cursor_loop".into());
                }
            }
            let params = cursor
                .as_ref()
            ...
            if response.get("error").is_some() {
                return Err("comfy_mcp_http_tools_list_rejected".into());
            }
            if let Some(page_tools) = response
                .get("result")
                ...
                .and_then(Value::as_array)
            {
                ...
                if tools.len() > MAX_MCP_TOOLS {
                    return Err("comfy_mcp_http_tool_count_limit".into());
                }
            }
            cursor = response
                .get("result")
    ...
    pub async fn call_tool(&mut self, tool_name: &str, arguments: Value) -> Result<Value, String> {
        ...
        let response = self
            ...
            .await?;
        if response.get("error").is_some() {
            return Err("comfy_mcp_tool_error".into());
        }
        Ok(response.get("result").cloned().unwrap_or(response))
    }

    pub async fn run_workflow_with_lifecycle(
        &mut self,
        arguments: Value,
        cancel: Arc<AtomicBool>,
    ) -> Result<Value, String> {
        self.run_workflow_with_lifecycle_for_tool(arguments, None, cancel, |_| {})
            .await
    }

    pub async fn run_workflow_with_lifecycle_and_callback<F>(
        &mut self,
        arguments: Value,
        cancel: Arc<AtomicBool>,
        on_execution_id: F,
    ...
    where
        F: Fn(&str) + Send + Sync,
    {
        self.run_workflow_with_lifecycle_for_tool(arguments, None, cancel, on_execution_id)
            .await
    }

    pub async fn run_workflow_with_lifecycle_for_tool<F>(
        &mut self,
        arguments: Value,
        ...
        F: Fn(&str) + Send + Sync,
    {
        ...
        let run_tool = requested_tool
            ...
            .or_else(|| {
                ...
                .iter()
                .find(|candidate| tool_names.iter().any(|name| *name == **candidate))
                .copied()
            })
        ...
        let submission = self
            .call(
                next_id,
                "tools/call",
                serde_json::json!({"name": run_tool, "arguments": arguments}),
            )
            .await?;
        if submission.get("error").is_some() {
            return Err("comfy_mcp_tool_error".into());
        }
        let Some(execution_id) = find_string_field(
            &submission,
            &[
                "executionId",
                "execution_id",
        ...
        .iter()
        .find(|candidate| tool_names.iter().any(|name| *name == **candidate));
        let Some(status_tool) = status_tool else {
            return Ok(submission);
        };
        for _ in 0..720 {
            if cancel.load(Ordering::Relaxed) {
                if let Some(cancel_tool) = [
                    "cancel_job",
                    "cancel_workflow",
                    "cancel_execution",
                    "cancel",
            ...
            {
                return Ok(
                    serde_json::json!({ "executionId": execution_id, "status": "completed", "submission": submission, "statusResult": status, "output": Value::Null }),
                );
            }
            match find_status(&status).as_deref() {
                Some("completed" | "complete" | "succeeded" | "success" | "done") => {
                    let output = if let Some(output_tool) = [
                        "fetch_outputs",
                        "get_output",
                        "get_job_output",
                    ...
                    return Ok(
                        serde_json::json!({ "executionId": execution_id, "status": "completed", "submission": submission, "statusResult": status, "output": output }),
                    );
                }
                Some("failed" | "error" | "canceled" | "cancelled" | "expired") => {
                    return Err("comfy_mcp_execution_terminal".into())
                }
...
fn lifecycle_arguments(tools: &[Value], tool_name: &str, execution_id: &str) -> Value {
    ...
    .iter()
    .find(|candidate| properties.is_some_and(|items| items.contains_key(**candidate)))
    .copied()
    .unwrap_or_else(|| {
...
fn parse_sse_json(body: &[u8]) -> Result<Value, String> {
    let text =
        std::str::from_utf8(body).map_err(|_| "comfy_mcp_http_invalid_response".to_string())?;
...
pub fn validate_endpoint(endpoint: &str) -> Result<(), String> {
    ...
    if !allowed
        || endpoint.contains(['?', '#', '\\', '@'])
        || endpoint.contains("169.254.")
        || endpoint.contains("127.0.0.1.evil")
    {

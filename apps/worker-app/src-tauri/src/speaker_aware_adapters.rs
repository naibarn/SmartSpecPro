...
pub const BUNDLED_SPEAKER_AWARE_RUNNER_RELATIVE_PATH: &str =
...
pub fn validate_policy(policy: &AdapterPolicy) -> Result<(), String> {
    if policy.contract_version != SPEAKER_AWARE_CONTRACT_VERSION {
        return Err("invalid_contract: unsupported speaker-aware policy version".into());
    }
    if !(250..=60_000).contains(&policy.max_scan_window_ms) {
        return Err("invalid_contract: maxScanWindowMs out of bounds".into());
    }
    if !(1..=8).contains(&policy.max_concurrent_processes) {
        return Err("invalid_contract: maxConcurrentProcesses out of bounds".into());
    }
    for (name, stage) in [
        ("vad", &policy.vad),
        ...
        if stage.required && !stage.enabled_adapters.contains(&stage.primary) {
            return Err(format!(
                "invalid_contract: {name} required primary adapter is not enabled"
            ));
        }
        if matches!(stage.fallback_policy, FallbackPolicy::AllowListed)
            && stage.fallback_allow_list.is_empty()
        {
            return Err(format!(
                "invalid_contract: {name} fallback allow-list is empty"
            ));
        }
        if stage
            .fallback_allow_list
            .iter()
            .any(|adapter| !stage.enabled_adapters.contains(adapter))
        {
            return Err(format!(
                "invalid_contract: {name} fallback adapter is disabled"
            ));
        }
    }
    Ok(())
...
pub fn resolve_adapter(
    ...
    if status_for(&stage.primary).is_some_and(|item| item.status == AdapterStatus::Ready) {
        return AdapterResolution {
            adapter_id: Some(stage.primary.clone()),
            status: "ready".into(),
            fallback_from: None,
            reason: "primary adapter ready".into(),
        };
    }
    match stage.fallback_policy {
        FallbackPolicy::Deny => AdapterResolution {
            adapter_id: None,
            status: "blocked".into(),
            fallback_from: Some(stage.primary.clone()),
            reason: "primary adapter unavailable and fallback denied".into(),
        },
        FallbackPolicy::ReportUnknown => AdapterResolution {
            adapter_id: None,
            status: "unknown".into(),
            fallback_from: Some(stage.primary.clone()),
            reason: "primary adapter unavailable and policy is report-only".into(),
        },
        FallbackPolicy::AllowListed => stage
            .fallback_allow_list
            .iter()
            .find(|id| status_for(id).is_some_and(|item| item.status == AdapterStatus::Ready))
            .map(|id| AdapterResolution {
                adapter_id: Some(id.clone()),
                status: "fallback".into(),
                fallback_from: Some(stage.primary.clone()),
                reason: "explicit allow-listed fallback".into(),
            })
            .unwrap_or_else(|| AdapterResolution {
                adapter_id: None,
                status: "blocked".into(),
                fallback_from: Some(stage.primary.clone()),
                reason: "no ready adapter in allow-list".into(),
            }),
    }
}

pub fn probe_allowlisted_command(
    command: &str,
    model_path: Option<&Path>,
    gpu_required: bool,
) -> AdapterCapability {
    let adapter_id = match command {
        "silero-onnx" => AdapterId::SileroOnnx,
        "firered-vad" => AdapterId::FireRedOnnx,
        "ten-vad" => AdapterId::TenVad,
        "webrtc-vad" => AdapterId::WebRtcVad,
        "pyannote" => AdapterId::PyannoteDiarization,
        "mediapipe-face" => AdapterId::MediaPipeFace,
        "person-body" => AdapterId::PersonBody,
        "active-speaker-fusion" => AdapterId::ActiveSpeakerFusion,
        _ => {
            return AdapterCapability {
                adapter_id: AdapterId::WebRtcVad,
                version: "unknown".into(),
                status: AdapterStatus::Incompatible,
                runtime: None,
                device: "unknown".into(),
                model_checksum: None,
                remediation_key: Some("unknown_adapter".into()),
            }
        }
    };
    if gpu_required && std::env::var("CUDA_VISIBLE_DEVICES").ok().as_deref() == Some("") {
        return AdapterCapability {
            adapter_id,
            version: "unknown".into(),
            status: AdapterStatus::GpuUnavailable,
            runtime: Some(command.into()),
            device: "cuda".into(),
            model_checksum: None,
            remediation_key: Some("gpu_required".into()),
        };
    }
    if model_path.is_some_and(|path| !path.is_file()) {
        return AdapterCapability {
            adapter_id,
            version: "unknown".into(),
            status: AdapterStatus::MissingModel,
            runtime: Some(command.into()),
            device: if gpu_required { "cuda" } else { "cpu" }.into(),
            model_checksum: None,
            remediation_key: Some("install_model".into()),
        };
    }
    ...
    AdapterCapability {
        ...
        device: if gpu_required { "cuda" } else { "cpu" }.into(),
        model_checksum: None,
        remediation_key: if ready {
            None
        } else {
            Some("install_runtime".into())
        },
    }
}
...
pub fn configure_bundled_runner(
    resource_dir: &Path,
    ...
    if let Some(app_data_dir) = app_data_dir {
        let (manifest_path, _) =
            crate::runtime_manifest::runtime_pack_paths(resource_dir, app_data_dir);
...
pub fn probe_configured_runner() -> Result<String, String> {
    let command = configured_runner().ok_or_else(|| {
        "workflow_capability_blocked: speaker-aware runner is not configured".to_string()
    })?;
    let mut probe = Command::new(&command);
    #[cfg(target_os = "windows")]
    {
    ...
    let output = probe.arg("--version").output().map_err(|error| {
        format!("workflow_capability_blocked: speaker-aware runner unavailable: {error}")
    })?;
    ...
    loop {
            ...
            .map_err(|error| format!("speaker_aware_runner_failed: {error}"))?
        {
            ...
            if !output_path.is_file() {
                return Err(
                    "speaker_aware_runner_failed: runner did not create output artifact".into(),
                );
            }
            let bytes = fs::read(output_path).map_err(|error| {
                format!("speaker_aware_runner_failed: output unreadable: {error}")
            })?;
            let value: serde_json::Value = serde_json::from_slice(&bytes)
                .map_err(|error| format!("invalid_contract: runner output is not JSON: {error}"))?;
            let object = value
                .as_object()
                .ok_or_else(|| "invalid_contract: runner output must be an object".to_string())?;
            if object
                .get("contractVersion")
                .and_then(serde_json::Value::as_str)
                != Some(SPEAKER_AWARE_CONTRACT_VERSION)
            {
...
mod tests {
    ...
    fn stage(
        ...
        let mut enabled = vec![primary.clone()];
        enabled.extend(list.clone());
        AdapterStagePolicy {
            enabled_adapters: enabled,
            primary,
            fallback_policy: fallback,
            fallback_allow_list: list,
            required: true,
        }
    }
    #[test]
    fn deny_does_not_fallback() {
        let resolution = resolve_adapter(
            &stage(AdapterId::SileroOnnx, FallbackPolicy::Deny, vec![]),
            &[],
        );
        assert_eq!(resolution.status, "blocked");
        assert_eq!(resolution.adapter_id, None);
    }
    #[test]
    fn only_allow_listed_ready_adapter_can_fallback() {
        let policy = stage(
            AdapterId::SileroOnnx,
            FallbackPolicy::AllowListed,
            vec![AdapterId::WebRtcVad],
        );
        let caps = vec![AdapterCapability {
            adapter_id: AdapterId::WebRtcVad,
            version: "1".into(),
            status: AdapterStatus::Ready,
            runtime: Some("webrtc-vad".into()),
            device: "cpu".into(),
            model_checksum: None,
            remediation_key: None,
        }];
        let resolution = resolve_adapter(&policy, &caps);
        assert_eq!(resolution.status, "fallback");
        assert_eq!(resolution.adapter_id, Some(AdapterId::WebRtcVad));
    }
    #[test]
    fn invalid_policy_is_rejected() {
        let invalid = AdapterPolicy {
            contract_version: "old".into(),
            vad: stage(AdapterId::SileroOnnx, FallbackPolicy::Deny, vec![]),
            diarization: stage(AdapterId::PyannoteDiarization, FallbackPolicy::Deny, vec![]),
            face: stage(AdapterId::MediaPipeFace, FallbackPolicy::Deny, vec![]),
            person: stage(AdapterId::PersonBody, FallbackPolicy::Deny, vec![]),
            active_speaker: stage(AdapterId::ActiveSpeakerFusion, FallbackPolicy::Deny, vec![]),
            max_scan_window_ms: 1000,
    ...
    fn adapter_ids_match_web_contract_casing() {
        assert_eq!(
            serde_json::to_string(&AdapterId::ActiveSpeakerFusion).unwrap(),
            "\"ActiveSpeakerFusion\""
        );
        assert_eq!(
            serde_json::to_string(&AdapterId::MediaPipeFace).unwrap(),
            "\"MediaPipeFace\""
        );
    }

...
The approved plan becomes a music caption: instrumentation/palette, energy progression, emotional intent, tempo/key preference, sparse arrangement around dialogue and instrumental/no-vocals intent. `compile_music_caption` MUST emit `displayCaption` (in localized user language for UI review) and `modelInstruction` (compiled strictly in English for MiniMax-Music3 prompt conditioning to match its instruction encoder training). Technical mixing values stay in DSP, not promises embedded in music prose.
...
- Cancellation stops the owned job/process group safely (using process group isolation such as `preexec_fn=os.setsid` / `killpg` on Unix sidecar processes and explicit PyTorch `torch.cuda.empty_cache()` VRAM reclamation), records terminal state and prevents late publication/apply. Do not kill unrelated Worker/IDE processes. If the backend cannot interrupt a kernel safely, drain/quarantine the result and report cancellation truthfully.

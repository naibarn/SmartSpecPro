...
- Python orchestrator/runtime hardening suite: `DEBUG=false PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 UV_CACHE_DIR=/tmp/uv-cache uv run --project python-backend pytest -p pytest_asyncio.plugin -p pytest_cov --noconftest python-backend/tests/unit/test_agency_orchestrator_runtime.py python-backend/tests/unit/test_agency_service.py::TestAgencyServiceExecuteRun::test_execute_run_orchestrator_passes_whitelist_and_retrieval_scope python-backend/tests/unit/test_agency_service.py::TestAgencyPreviewPersistencePolicy::test_execute_run_stream_orchestrator_passes_whitelist_and_retrieval_scope python-backend/tests/unit/test_agency_service.py::TestAgencyServiceExecuteRun::test_execute_run_passes_retrieval_scope_mode_to_tool_resolution python-backend/tests/unit/test_agency_service.py::TestAgencyPreviewPersistencePolicy::test_execute_run_stream_passes_retrieval_scope_mode_to_tool_resolution python-backend/tests/unit/test_agency_tools.py::TestToolResolution::test_resolve_filters_external_retrieval_tools_for_library_only_scope python-backend/tests/unit/test_agency_tools.py::TestToolResolution::test_resolve_keeps_web_search_when_scope_allows_fallback` → pass (`7` tests)
- Template provenance and audit honesty suite: `bash -lc 'source ~/.nvm/nvm.sh && cd /home/dev/projects/SmartSpecPro && npm --prefix apps/web test -- server/services/agencyExperienceTemplateService.test.ts server/routers/__tests__/agency.test.ts'` and `DEBUG=false PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 UV_CACHE_DIR=/tmp/uv-cache uv run --project python-backend pytest -p pytest_asyncio.plugin -p pytest_cov --noconftest python-backend/tests/unit/test_agency_audit.py python-backend/tests/unit/test_agency_orchestrator_runtime.py python-backend/tests/unit/test_agency_service.py::TestAgencyServiceExecuteRun::test_execute_run_orchestrator_passes_whitelist_and_retrieval_scope python-backend/tests/unit/test_agency_service.py::TestAgencyPreviewPersistencePolicy::test_execute_run_stream_orchestrator_passes_whitelist_and_retrieval_scope python-backend/tests/unit/test_agency_service.py::TestAgencyServiceExecuteRun::test_execute_run_passes_retrieval_scope_mode_to_tool_resolution python-backend/tests/unit/test_agency_service.py::TestAgencyPreviewPersistencePolicy::test_execute_run_stream_passes_retrieval_scope_mode_to_tool_resolution` → pass (`21` web tests + `14` python tests)
...
- Retrieval-scope hardening is improved: both agent-only runs and orchestrator-backed agent nodes now drop direct external retrieval tools during Python tool resolution for `library_only`, but centralized enforcement across every external-access path is still deferred.
- Credit reconciliation no longer reports a misleading `0.0`/`0.0` match, but true gateway-total reconciliation is still deferred until run-level credit totals are exposed from the billing path.
...
3. Wire actual run-level credit totals into agency reconciliation so pending audit events can become true match/mismatch checks.

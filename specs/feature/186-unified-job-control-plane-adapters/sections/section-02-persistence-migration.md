...
Verify SQL shape, legacy status preservation and explicit alias projection, nullable idempotency behavior, existing assignment-sequence compatibility, uniqueness/FK constraints, action/callback/settlement dedupe, transfer item-key/snapshot/checkpoint constraints, bounded error/review projection, rerun safety, dry-run no-write behavior, resume cursor, and quarantine of ambiguous legacy rows. Verify that deletion/retention cannot cascade-delete required lifecycle evidence without the approved archive/redaction workflow.

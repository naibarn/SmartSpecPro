...
| 02 Persistence | migration SQL shape, legacy status preservation, rerun-safe backfill, quarantine, and Feature 189 integration constraints | Drizzle additions and additive migration/backfill |
...
| 06 Monitor | tenant/admin scope, cursor, redaction, action idempotency, and the Feature 189 cancellation/fencing integration boundary | canonical monitor and safe action integration |
...
- Feature 189 transfer tests are owned and run from the Feature 189 plan; Feature 186 retains only the fake control-plane cancellation/fencing contract.
...
The transfer and System Admin test cases above are completion criteria for
Feature 189. Feature 186 owns only the reusable fake cancellation/fencing,
outbox-cancellation, settlement, and reconciler contract that those tests
consume; it does not own transfer handlers, item semantics, or transfer UI.

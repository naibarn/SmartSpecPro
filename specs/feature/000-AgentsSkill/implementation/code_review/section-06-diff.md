diff --git a/deep_plan/skills/orchestra/SKILL.md b/deep_plan/skills/orchestra/SKILL.md
new file mode 100644
index 0000000..c8a73a8
--- /dev/null
+++ b/deep_plan/skills/orchestra/SKILL.md
@@ -0,0 +1,443 @@
+---
+name: orchestra
+description: >
...
+| `references/artifact-management.md` | Only on fresh session setup or archive — Step 0 |
+
+---
+
+## STOP Conditions
...
+- Read `references/artifact-management.md` if `orchestra/` already exists (check for stale session and archive if needed).
...
+| `large` | deep-plan-codex chain | Read `references/skill-pack-integration.md`. Create requirements spec. Write expected artifact paths to `orchestra/backlog.md`. Print handoff instruction. STOP. |
...
+
+**Contracts are frozen after they are written — they are never modified mid-session.** See `references/artifact-management.md` for the enforcement rule.
+
+**Wave grouping:**
...
+
+**Circular dependency check:** Before finalizing, verify no pending tasks form a cycle. If all pending tasks are blocked by each other with no ready tasks — STOP (see STOP conditions table in Step 2).
+
+---
+
+## Step 4: Dispatch
...
+
+**Build Task Packets:** For each agent in the current wave, construct a Task Packet following `references/sub-agent-dispatch.md`. The packet must include all 8 required sections: TASK, DOMAIN, FILES, CONTEXT, CONSTRAINTS, CONTRACT, OUTPUT, QUALITY GATE.
+
...
+|-------|----------|--------|
+| `green` | Short session, few decisions, simple task | Continue normally |
...
+4. Notify user:
+   ```
+   🔴 CONTEXT CRITICAL: Snapshot saved.
+     orchestra/snapshot.json and orchestra/snapshot.md updated.
+   To resume after /clear: /orchestra resume
+   To continue here: type "continue"
+   ```
+
...
+
+When Claude resumes after a `/clear` or context compaction, the detailed instructions from this file are lost. The `orchestra/` files are the source of truth.
+
+**Recovery procedure:**
+

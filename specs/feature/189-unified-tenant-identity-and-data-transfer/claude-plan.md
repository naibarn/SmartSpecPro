...
Feature 186 is complete and is consumed as a dependency. This plan owns only
identity, authorization, tenant moves, transfer handlers, preview/approval,
transfer execution, and transfer UI. It must call Feature 186 control-plane
ports for canonical job creation, claim/lease, cancellation, outbox, settlement,
and reconciliation; it must not import a transport or duplicate lifecycle
logic. The initial transfer migration is
`0316_feature_189_tenant_identity_and_data_transfer.sql`, with the repeat-safe
backfill completion migration
`0317_feature_189_tenant_identity_backfill_completion.sql`; Feature 186
migrations `0303` through `0315` are not modified or reused.
...
- No flag is enabled until the rollout manifest contains numeric selection,
  preview, batch, deadline, rate, transaction, query, and canary budgets plus
  named owner/evidence fields.
...
Create `specs/feature/189-unified-tenant-identity-and-data-transfer/rollout-manifest.md` as the implementation/rollout manifest containing:
...
Add a durable `tenant_identity_actions` phase/fence record keyed by
action-idempotency key and user. It stores the source/target snapshot, phase,
fencing version, authorization decision, safe outcome, and timestamps. While a
move fence is open, protected job admission/dispatch for that source user
returns `TENANT_MOVE_IN_PROGRESS` or a bounded defer result and creates no
transport side effect. A crashed move remains resumable by the same action
key.
...
Add a dedicated `adminTenants.moveUserTenant` service/procedure under existing
admin service/router ownership. It must require `role = admin`, reject
`domain_admin` even if the request host matches the target tenant, and validate
an explicit confirmation token/idempotency key and warning acknowledgment.

The service uses two short transactions with the control-plane cancellation
work between them:

1. Lock the user and validate the source/active-target snapshot, then commit
   before any control-plane or adapter call. Enumerate only this user's
   verified canonical `pending`, `queued`, and `retry_scheduled` jobs and
   cancel/fence them through the control plane; never flush a shared broker
   queue. Any `leased`, `running`, or `waiting_external` job blocks the move
   until resolved or explicitly reviewed. If cancellation partially succeeds,
   retain the evidence for the same action-key retry. The open identity fence
   rejects or defers new job admission for this user.
2. Re-lock and revalidate the snapshot. In the final transaction update only
   `currentTenantId` and approved audit fields, set credits to zero immediately
   through the existing credit/ledger boundary without deleting or rewriting
   prior transactions, revoke all active sessions/tokens, and append the
   auditable action with actor, old/new tenant, reason, credit reset, and
   revocation result. Re-enumerate queueable and active jobs immediately before
   this transaction; a newly active job blocks the commit. Close the identity
   fence only after the binding transaction commits.

Preserve email, openId, password/OAuth identity, and account ID. Leave old
tenant-owned data/files and transaction/usage history in the old tenant. Return
a stable idempotent outcome on repeat requests.
...
Approval also opens a source-user transfer fence in that transaction. New
queueable admission for the source user returns `TRANSFER_IN_PROGRESS` or a
bounded defer result until the operation reaches a terminal state. This closes
the race between approval/cancellation and a newly submitted job.
...
Expose guarded `resume`, `resolveItem`, and `cancel` commands with action idempotency keys. `resolveItem` accepts only an explicit `retry` after the operator has repaired the declared conflict or an explicit `skip`; it never means overwrite or merge. Resume may transition only `paused_on_error` or an approved resumable state to `resuming`/`running`. It must:
...
- `preview`: input `{ sourceUserId, targetUserId, resourceSelections, requestIdempotencyKey }`; output `{ previewId, previewFingerprint, sourceUserId, targetUserId, tenantId, counts, items, nextCursor, queueActions, exclusions, generatedAt, expiresAt }`.
...
- `resume`, `resolveItem`, and `cancel`: each accepts the operation/item identifier as applicable plus an action idempotency key and returns the original or newly applied guarded outcome. `resolveItem` accepts only `retry` after the declared conflict is repaired or `skip`; it cannot overwrite or merge. Item resolution is the only public path for retrying or intentionally skipping an individual item.
...
- resume/resolve-item/cancel actions, each authorized and idempotent;
...
apps/web/drizzle/0316_feature_189_tenant_identity_and_data_transfer.sql # conductor-only schema migration
apps/web/drizzle/0317_feature_189_tenant_identity_backfill_completion.sql # repeat-safe backfill completion

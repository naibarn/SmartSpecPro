...
The executor receives only the canonical `worker_jobs.id` from Feature 186,
loads the approved plan from PostgreSQL, and never trusts tenant, user, handler,
adapter, or routing values from the transport envelope. It must not import or
call BullMQ, Celery, Beat, Redis, provider APIs, or transport cancellation
directly. All queue and lifecycle actions go through Feature 186 ports.
...
Derive an item key from canonical `operationId`/`worker_jobs.id`, tenant, source user, target user, resource kind/source ID, handler version, and logical action. Before a side effect, lock/recheck item state, ownership, dependency, target conflict, current attempt/lease context, and durable destination/result marker. A transferred/skipped item is not executed again. No overwrite/merge is allowed. Process deterministic parent-before-child batches; persist item outcomes, `queue_cancelled` dispositions, progress, and resume cursor atomically after each bounded batch. Storage/domain partial writes use durable settlement markers keyed by item and result digest.

## Pause and resume

System/database/transport failure pauses the operation at `paused_on_error` with safe reason and durable cursor, while the canonical job is `retry_scheduled` with `operatorReviewRequired` and no automatic outbox dispatch. `resume` is an idempotent guarded action that rechecks ownership/dependencies, creates the approved dispatch signal, and continues the same operation/job. `resolveItem` may explicitly choose `retry` after the declared conflict is repaired or `skip`; it never means overwrite or merge. Resume skips `transferred`/intentional `skipped`, retries only bounded `retryable_error`, and leaves `conflict`/`permanent_error` visible until authorized resolution/skip. It never creates a replacement operation or new paid key. The source-user transfer fence remains open during pause and is released only by a guarded terminal outcome; new queueable admission returns `TRANSFER_IN_PROGRESS` while it is open.

Set `completed` when all selected items settle by transfer/approved skip, `completed_with_conflicts` when unresolved conflict/unsupported/permanent items remain, `failed` for unrecoverable operation-level failure with operator review, and `cancelled` only through guarded cancellation. Close the source-user transfer fence only after the terminal state and item outcomes are durable. Map these projections to canonical worker-job terminal events without creating a second lease/retry/status source.
...
Extend the Feature 186 reconciler to find stale transfer leases, paused jobs,
incomplete checkpoints, missing queue-cancellation evidence, and unresolved
storage/domain settlement. A review-gated transfer is never automatically
dispatched even when `nextRetryAt` is due. Reconcile bounded and tenant-scoped;
transport outage leaves durable state intact. Never bypass conflicts or create
replacement jobs automatically.

...
- `preview({ sourceUserId, targetUserId, resourceSelections, requestIdempotencyKey })` → preview ID/fingerprint, users/tenant, counts, first cursor page, queue actions, exclusions, timestamps.
...
- `resume`, `resolveItem`, `cancel` → guarded original/new outcome with action idempotency key; `resolveItem` is limited to explicit retry-after-repair or skip and never overwrites/merges. Convenience aliases must not create separate mutation semantics or bypass the action ledger.
...
Every mutation returns a stable error envelope with `code`, safe localized
message, correlation ID, and whether the result is `created`, `replayed`, or
`rejected`. Required codes are `TENANT_MISMATCH`, `FORBIDDEN`, `PREVIEW_STALE`,
`PREVIEW_EXPIRED`, `IDEMPOTENCY_CONFLICT`, `ACTIVE_JOB_BLOCKED`,
`UNSUPPORTED_RESOURCE`, `RESOURCE_CONFLICT`, `JOB_STATE_CONFLICT`,
`OPERATOR_REVIEW_REQUIRED`, `BACKPRESSURE`, and `TRANSFER_CANCELLED`. Cursors
are signed, scope-bound, expiry-bound, and rejected when filters or tenant
context differ from the cursor payload.

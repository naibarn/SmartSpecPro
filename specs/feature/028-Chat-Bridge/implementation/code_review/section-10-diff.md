diff --git a/apps/web/server/routers/__tests__/telegram.bridge.test.ts b/apps/web/server/routers/__tests__/telegram.bridge.test.ts
new file mode 100644
index 0000000..58047be
--- /dev/null
+++ b/apps/web/server/routers/__tests__/telegram.bridge.test.ts
@@ -0,0 +1,619 @@
...
+  });
+});
diff --git a/apps/web/server/routers/telegram.ts b/apps/web/server/routers/telegram.ts
index 5d59c3d..c8ca962 100644
--- a/apps/web/server/routers/telegram.ts
+++ b/apps/web/server/routers/telegram.ts
@@ -15,10 +15,11 @@ import {
   users,
   telegramLinkTokens,
   telegramConnections,
...
+        ),
+      );
+    boundConversationCount = countResult?.cnt ?? 0;
+  }
+
   return {
...
@@ -596,6 +639,33 @@ const unlinkTelegram = protectedProcedure.mutation(async ({ ctx }) => {
     ...remainingPrefs
   } = currentPrefs;
...
+
   ...
   await db
     .update(users)
@@ -664,6 +734,421 @@ const updateTelegramPreferences = protectedProcedure
     return { success: true };
   });
...
+      .where(
+        and(
+          eq(conversations.id, parseInt(conversationId, 10)),
...
+
 export const telegramRouter = router({
   // Admin endpoints
   getTelegramSettings,
@@ -676,4 +1161,13 @@ export const telegramRouter = router({
   checkTelegramStatus,
   unlinkTelegram,

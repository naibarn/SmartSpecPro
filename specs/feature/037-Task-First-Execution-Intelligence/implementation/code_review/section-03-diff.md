diff --git a/apps/web/drizzle/0063_open_harrier.sql b/apps/web/drizzle/0063_open_harrier.sql
new file mode 100644
index 00000000..3b952f6b
--- /dev/null
+++ b/apps/web/drizzle/0063_open_harrier.sql
@@ -0,0 +1,45 @@
...
diff --git a/apps/web/drizzle/meta/0063_snapshot.json b/apps/web/drizzle/meta/0063_snapshot.json
new file mode 100644
index 00000000..69b5957d
--- /dev/null
+++ b/apps/web/drizzle/meta/0063_snapshot.json
@@ -0,0 +1,19569 @@
+{
+  "id": "b9bc4522-4dbb-4747-9e8d-03948f64d803",
+  "prevId": "7108a074-a5d8-466b-ab77-ae7d26f4c16f",
+  "version": "7",
+  "dialect": "postgresql",
+  "tables": {
+    "public.agencies": {
...
index 80e08fc3..028560b1 100644
--- a/apps/web/drizzle/meta/_journal.json
+++ b/apps/web/drizzle/meta/_journal.json
@@ -442,6 +442,13 @@
       "when": 1773211330429,
       "tag": "0062_warm_johnny_storm",
       "breakpoints": true
+    },
+    {
+      "idx": 63,
...
index 2e222dd9..57d3a70e 100644
--- a/apps/web/drizzle/schema.ts
+++ b/apps/web/drizzle/schema.ts
@@ -4955,3 +4955,61 @@ export const automationTemplates = pgTable("automation_templates", {
...
+]);
diff --git a/apps/web/server/services/modelResolver.test.ts b/apps/web/server/services/modelResolver.test.ts
new file mode 100644
index 00000000..5b2c9891
--- /dev/null
+++ b/apps/web/server/services/modelResolver.test.ts
@@ -0,0 +1,164 @@
...
+  });
+});
diff --git a/apps/web/server/services/modelResolver.ts b/apps/web/server/services/modelResolver.ts
new file mode 100644
index 00000000..c8b530b4
--- /dev/null
+++ b/apps/web/server/services/modelResolver.ts
@@ -0,0 +1,112 @@
+/**
+ * Model Resolver
+ *
...
+  return snapshot;
+}
diff --git a/apps/web/server/services/taskExecutionPlanner.test.ts b/apps/web/server/services/taskExecutionPlanner.test.ts
new file mode 100644
index 00000000..2b660ad7
--- /dev/null
+++ b/apps/web/server/services/taskExecutionPlanner.test.ts
@@ -0,0 +1,194 @@
+import { describe, it, expect } from "vitest";
+import {
+  classifyTaskType,
...
+  });
+});
diff --git a/apps/web/server/services/taskExecutionPlanner.ts b/apps/web/server/services/taskExecutionPlanner.ts
new file mode 100644
index 00000000..da74159d
--- /dev/null
+++ b/apps/web/server/services/taskExecutionPlanner.ts
@@ -0,0 +1,141 @@
+/**
...
+  };
+
+  if (budgetClass) plan.budgetClass = budgetClass;
+  if (input.executionPolicy?.disallowedModels?.length) {
+    plan.disallowedModels = input.executionPolicy.disallowedModels;
+  }
+
+  // Attach context for auditability
+  if (input.skillSlug || input.conversationModel || input.sourceType) {
+    plan.context = {};
+    if (input.skillSlug) plan.context.skillSlug = input.skillSlug;
+    if (input.conversationModel) plan.context.conversationModel = input.conversationModel;
+    if (input.sourceType) plan.context.sourceType = input.sourceType;
+  }
+
+  return plan;
+}

diff --git a/apps/web/drizzle/0062_warm_johnny_storm.sql b/apps/web/drizzle/0062_warm_johnny_storm.sql
new file mode 100644
index 0000000..aec77fc
--- /dev/null
+++ b/apps/web/drizzle/0062_warm_johnny_storm.sql
@@ -0,0 +1,8 @@
...
diff --git a/apps/web/drizzle/meta/0062_snapshot.json b/apps/web/drizzle/meta/0062_snapshot.json
new file mode 100644
index 0000000..bb5fb5d
--- /dev/null
+++ b/apps/web/drizzle/meta/0062_snapshot.json
@@ -0,0 +1,19214 @@
+{
+  "id": "7108a074-a5d8-466b-ab77-ae7d26f4c16f",
+  "prevId": "c122b061-d477-4bb4-8ae6-7da218391bb4",
+  "version": "7",
+  "dialect": "postgresql",
+  "tables": {
+    "public.agencies": {
...
index dcdcb89..80e08fc 100644
--- a/apps/web/drizzle/meta/_journal.json
+++ b/apps/web/drizzle/meta/_journal.json
@@ -435,6 +435,13 @@
       "when": 1773152198837,
       "tag": "0061_flawless_epoch",
       "breakpoints": true
+    },
+    {
+      "idx": 62,
...
@@ -2446,6 +2469,21 @@ export const skills = pgTable("skills", {
...
diff --git a/apps/web/server/services/capabilityRegistry.test.ts b/apps/web/server/services/capabilityRegistry.test.ts
new file mode 100644
index 0000000..805202f
--- /dev/null
+++ b/apps/web/server/services/capabilityRegistry.test.ts
@@ -0,0 +1,151 @@
...
+ * Skill execution policy comes from skill.md frontmatter.
+ */
+
+/**
...
+
+/**
+ * Skill execution policy — describes how a skill wants its model resolved.
+ *
+ * Modes:
+ * - `requirements`: resolve from capabilities (default)
+ * - `fixed`: use a specific model only
+ * - `hybrid`: prefer fixed model, but fall back to requirements-based resolution
+ */
+export interface SkillExecutionPolicy {
+  mode: "requirements" | "fixed" | "hybrid";
+
+  /** Capability requirements for the model (used in requirements/hybrid modes) */
+  requirements?: CapabilityRequirements;
+
+  /** Fixed model ID (used in fixed/hybrid modes) */
+  fixedModel?: string;
+
+  /** Whether the conversation model can override this policy */
+  allowConversationOverride?: boolean;
+
+  /** Preferred strategy hint: "cheapest", "fastest", "best" */
+  preferredStrategy?: "cheapest" | "fastest" | "best";
+
+  /** Ordered list of preferred model IDs (tried first if they meet requirements) */
+  preferredProfiles?: string[];
+
+  /** Models that may be used if preferred ones are unavailable */
+  allowedFallbackProfiles?: string[];
+
+  /** Models that must NOT be used */
+  disallowedModels?: string[];
+
+  /** Budget class hint for cost-aware routing */
+  budgetClass?: "economy" | "standard" | "premium";
+
+  /** Whether tenant admin can override this policy */
+  overrideableByTenant?: boolean;
+
+  /** What to do when no model matches: "error" | "use_default" */
...
+ */
+export const DEFAULT_EXECUTION_POLICY: SkillExecutionPolicy = {
+  mode: "requirements",
+  requirements: {},
...
+    // Hybrid: put fixed model first if it's in the candidates
...
+  return candidates;
+}
diff --git a/apps/web/server/services/skillRegistry.ts b/apps/web/server/services/skillRegistry.ts
index 67c3088..b045ba7 100644
--- a/apps/web/server/services/skillRegistry.ts
+++ b/apps/web/server/services/skillRegistry.ts

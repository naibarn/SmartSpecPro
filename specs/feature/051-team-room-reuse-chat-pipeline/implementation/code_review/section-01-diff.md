diff --git a/apps/web/server/services/__tests__/roomIntentRouter.enhanced.test.ts b/apps/web/server/services/__tests__/roomIntentRouter.enhanced.test.ts
new file mode 100644
index 00000000..120bd0c5
--- /dev/null
+++ b/apps/web/server/services/__tests__/roomIntentRouter.enhanced.test.ts
@@ -0,0 +1,229 @@
...
index 7737f4b4..308e78c2 100644
...
diff --git a/apps/web/server/services/roomIntentRouter.ts b/apps/web/server/services/roomIntentRouter.ts
index dcd51fff..cd2ab9c4 100644
--- a/apps/web/server/services/roomIntentRouter.ts
+++ b/apps/web/server/services/roomIntentRouter.ts
@@ -1,6 +1,6 @@
 import { classifyIntent } from "./skillIntentClassifier";
 import { detectSkill } from "./skillDetector";
-import { TEAM_DISCUSSION_SKILL_ID } from "./internalSkills";
+const FALLBACK_CONTENT_SKILL_ID = "general-article-writer";
 
 export type RoomIntentOrigin = "human_user" | "assistant" | "system";
 export type RoomIntentContext = "room_message" | "run_turn" | "work_item";
@@ -55,12 +55,24 @@ export async function routeRoomIntent(input: RoomIntentRouterInput): Promise<Roo
     };
   }
 
+  // For assistant/run_turn: still attempt skill detection before falling back
+  // to team-discussion. This allows proper skill matching (e.g. language, domain).
   if (input.origin !== "human_user") {
+    const assistantDetection = await detectSkill(normalized, undefined, undefined, input.userId);

...
def _extract_and_validate_manifest(content: str) -> None:
    ...
    if content.startswith("---"):
        try:
            ...
            logger.warning("skill_yaml_parse_failed", error=str(e))
    ...
    if manifest:
        try:
            ...
            logger.error(
                "skill_manifest_validation_failed",
                error=str(e),
                manifest_name=manifest.get("name") if isinstance(manifest, dict) else None,
            )
            raise HTTPException(
                status_code=400, detail=f"Skill manifest validation failed: {str(e)}"
            )
        except Exception as e:
            logger.error("skill_manifest_validation_error", error=str(e))
            raise HTTPException(
                status_code=400, detail=f"Failed to validate skill manifest: {str(e)}"
            )
...
async def update_skill(
    ...
    try:
        ...
        if skill_update.content is not None:
            ...
            _extract_and_validate_manifest(skill_update.content)

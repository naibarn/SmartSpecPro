"""Video Generation node executor using the Media Studio pipeline (Kie AI)."""
import os
...
class VideoExecutor:
    """
    Executor for Video Generation nodes.

    Uses the same media generation pipeline as Media Studio:
    Node.js tRPC media.generateVideo -> credit check -> Python media API -> Kie AI -> credit deduction

    Models are loaded dynamically from the media_models DB table (type=video).
    The frontend renders dynamic input fields based on each model's configJson.inputFields
    (e.g. duration, resolution, aspect_ratio).  All values set by the user are forwarded
    to the tRPC payload so the Node.js pipeline can pass them to the external provider.
    """
    ...
    async def execute(
        ...
        """
        Execute video generation via the Node.js tRPC media pipeline.
        ...
        Returns:
            dict with videoUrl and metadata outputs
        """
        config = data.config
        inputs = data.inputs
        ...
        if not prompt:
            return {"videoUrl": None, "metadata": {"error": "Video generation requires a prompt"}}

        model = inputs.get("model") or config.get("model", "")
        if not model:
            return {"videoUrl": None, "metadata": {"error": "Video generation requires a model"}}
        ...
        standard_fields = {
            "prompt", "model", "duration", "aspect_ratio", "aspectRatio",
            "resolution", "fps", "referenceImages", "modelCreditCost",
        }
        ...
        if not user_token:
            logger.warning("video_executor_no_token", execution_id=context.execution_id)
            return {"videoUrl": None, "metadata": {"error": "No authentication token available"}}
        ...
        if duration is not None:
            trpc_input["duration"] = int(duration) if isinstance(duration, (int, float, str)) else duration
        if aspect_ratio:
            trpc_input["aspectRatio"] = aspect_ratio
        if resolution:
            trpc_input["resolution"] = resolution
        if fps is not None:
            trpc_input["fps"] = int(fps) if isinstance(fps, (int, float, str)) else fps
        if reference_images:
        ...
        # --- Call Node.js tRPC media.generateVideo endpoint ----------------------
        # Full pipeline: rate limit -> credit check -> Kie AI -> credit deduction
        try:
            async with httpx.AsyncClient(timeout=300.0) as client:
                response = await client.post(
                    f"{NODEJS_INTERNAL_URL}/trpc/media.generateVideo",
                    json=trpc_payload,
                    headers={
                ...
                if response.status_code != 200:
                    ...
                    try:
                        ...
                        error_msg = (
                            ...
                            or f"Video generation failed (HTTP {response.status_code})"
                        )
                    except Exception:
                        error_msg = f"Video generation failed (HTTP {response.status_code})"

                    return {"videoUrl": None, "metadata": {"error": error_msg}}
            ...
            logger.error("video_executor_timeout", execution_id=context.execution_id)
            return {"videoUrl": None, "metadata": {"error": "Video generation timed out (300s)"}}
        except Exception as exc:
            logger.exception("video_executor_unexpected_error", execution_id=context.execution_id)
            return {"videoUrl": None, "metadata": {"error": f"Video generation failed: {str(exc)}"}}
